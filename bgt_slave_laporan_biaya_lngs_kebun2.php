...
while ($rKode = mysql_fetch_assoc($qKodeOrg)) {
	$a += 1;
	$dtJjg[$rKode['tahunbudget']] += $rKode['thntnm'];
	$dtJmlhKg[$rKode['tahunbudget']] += $rKode['thntnm'];
}
...
#exit(mysql_error($conn));
($qNoakunRupiah2 = mysql_query($sNoakunRupiah2)) || true;

while ($rNoakunRupiah2 = mysql_fetch_assoc($qNoakunRupiah2)) {
	$dtNoakunRup2[$thnBudget] += $rNoakunRupiah2['aknKpala'];
}
	...
	foreach ($dtNoakun as $barisNoakun) {
		...
		if ($brs == 1) {
			...
			foreach ($dtThnBudget as $lstThaTnm) {
				if (('12601' <= substr($barisNoakun, 0, 5)) && (substr($barisNoakun, 0, 5) <= '12605')) {
				...
				else if ('12606' <= substr($barisNoakun, 0, 5)) {
