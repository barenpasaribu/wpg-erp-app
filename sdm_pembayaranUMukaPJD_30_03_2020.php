...
while ($bar = mysql_fetch_object($res)) {
	...
	if( (real)$bar->dibayar == 0 ){
		echo "<img src='images/puzz.png' style='cursor:pointer;' title='click to get value' onclick=\"document.getElementById('bayar".$no."').value='".number_format($bar->uangmuka, 2, '.', ',')."'\">\r\n\t\t     ";             
	}
	...
	if( trim($dissa) == '' ){
		echo "<img src='images/save.png' title='Save' class=resicon onclick=saveBayarPJD('".$no."','".$bar->notransaksi."')>\r\n\t\t      ";
	}	
