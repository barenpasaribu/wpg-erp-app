...
switch ($proses) {
    case 'showDetail':
        ...
        $where = "notransaksi='".$param['notransaksi']."' and kodeorg='".$param['kodeorg']."' and tipetransaksi='".$param['tipetransaksi']."' and noakun2a='".$param['noakun']."'";
        $cols = 'kode,keterangan1,noakun,noaruskas,matauang,kurs,keterangan2,jumlah,'.'kodekegiatan,kodeasset,kodebarang,nik,kodecustomer,kodesupplier,kodevhc,orgalokasi,nodok,hutangunit1';
    ...
    case 'getInvoice':
        ...
        while ($rPo = mysql_fetch_assoc($qPo)) {
            if ($rPo['noinvoice'] == $belumlunas[$rPo['noinvoice']]) {
                ...
                $qVp = 'select a.noinv,b.posting,b.novp,c.noakun,c.kurs,c.matauang,c.jumlah from '.$dbname.'.keu_vp_inv a left join '.$dbname.'.keu_vpht b on a.novp=b.novp left join '.$dbname.".keu_vpdt c on a.novp=c.novp where a.noinv='".$rPo['noinvoice']."' and b.posting=1";
                $cekVp = fetchData($qVp);
                $kursPpn = 1;
                $kursInv = 1;
                ...
                $qJmlh = mysql_query($sJmlh) || exit(msyql_error($conn));
                ...
                if (0 == $rPo['posting'] || empty($cekVp)) {
                    ...
                    if ($totalInvoice <= $rJmlh['jmlhKas']) {
                        ...
                        $sakun = 'select noakun from '.$dbname.'.keu_vpdt a left join '.$dbname.'.keu_vp_inv b on a.novp=b.novp '."where b.noinv='".$rPo['noinvoice']."' and left(noakun,3)='211'";
                        $qakun = mysql_query($sakun);
                        $rakun = mysql_fetch_assoc($qakun);
                        $dat .= "<tr class='rowcontent' onclick=\"setPo('".$rPo['noinvoice']."','".$rCek['jmlhinvoice']."','".$rakun['noakun']."','".$rPo['keterangan']."','".$rPo['kodesupplier']."','".$rPo['nopo']."')\" style='pointer:cursor;'><td>".$no.'</td>';
