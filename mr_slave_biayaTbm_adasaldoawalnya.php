...
// ambil periode awal
$str="SELECT periode FROM ".$dbname.".keu_saldobulanan
    WHERE kodeorg ".$kodeorg." order by periode limit 1";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
    $periodeawal=$res['periode'];
    $tahunawal=substr($periodeawal,0,4);
    $bulanawal=substr($periodeawal,4,2);
}
...
// preview excel pdf    
    ...
    // ambil saldo awal
    $str="SELECT substr(noakun,1,5) as noakun, periode, sum(awal".$bulanawal.") as sawal FROM ".$dbname.".keu_saldobulanan
        WHERE kodeorg ".$kodeorg." and periode = '".$periodeawal."' group by substr(noakun,1,5)";
    $query=mysql_query($str) or die(mysql_error($conn));
    while($res=mysql_fetch_assoc($query))
    {
        $arr[$res['noakun']]['sawal']=$res['sawal'];
    }    
    ...
    $str="SELECT tahuntanam, substr(noakun,1,5) as noakun, sum(debet) as debet, sum(kredit) as kredit FROM ".$dbname.".keu_jurnalsum_blok_vw
        WHERE substr(kodeorg,1,4) ".$kodeorg." and periode <= '".$periode."' and noakun like '126%' group by tahuntanam, substr(noakun,1,5)";
    $query=mysql_query($str) or die(mysql_error($conn));
    while($res=mysql_fetch_assoc($query))
    {
        $ttoke=false;
...
//    echo "</pre>";
    ...
    $tab.="<table cellpadding=1 cellspacing=1 border=".$brdr." class=sortable >
        ...
        $tab.="<td align=center ".$bg." >Saldo Awal</td>";
        if(!empty($tt))foreach($tt as $tata){
        ...
        if(!empty($tt))foreach($tt as $tata){
            if($tata==0)$tab.="<td align=right ".$bg." ></td>";
            else $tab.="<td align=right colspan=2 ".$bg." >".number_format($luas[$tata])."</td>";
        }
    ...
    if(!empty($akuntb))foreach($akuntb as $akun){
        ...
        <td align=right>".number_format($arr[$akun]['sawal'])."</td>";
        $total=0;
        $totat['sawal']+=$arr[$akun]['sawal'];
        $total+=$arr[$akun]['sawal'];
        $totin['sawal']+=$arr[$akun]['sawal'];
        if(!empty($tt))foreach($tt as $tata){
            $qwe=$akun.$tata;
            @$perha=$arr[$qwe]['rp']/$luas[$tata];
            if($tata==0){
                $tab.="<td align=right style='cursor:pointer;' title='Click untuk melihat detail' onclick=\"lihatDetail('".$akun."','".$pt."','".$unit."','".$periode."',event);\">".number_format($arr[$qwe]['rp'])."</td>";
            }else{
    ...
    $tab.="<td align=right ".$bg.">".number_format($totat['sawal'])."</td>";
    $total=0;
    $total+=$totat['sawal'];
    if(!empty($tt))foreach($tt as $tata){
    ...
    <td align=right ".$bg.">".number_format($totin['sawal'])."</td>";
    $total=0;
    $total+=$totin['sawal'];
    if(!empty($tt))foreach($tt as $tata){
