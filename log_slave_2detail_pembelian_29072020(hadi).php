...
case 'preview':
	...
	foreach ($data as $row => $dt) {
		...
		$afdC = false;
		$blankC = false;
		$sList = 'select distinct a.tanggal,a.matauang,b.kodebarang,b.satuan,b.nopo,b.jumlahpesan,b.nopp,b.hargasatuan from ' . $dbname . '.log_poht a left join ' . $dbname . '.log_podt b on a.nopo=b.nopo where a.kodesupplier=\'' . $dt['kodesupplier'] . '\' and b.nopo!=\'NULL\' and a.tanggal between \'' . date('Y-m-d', strtotime($_POST['tglDr'])) . '\' and \'' . date('Y-m-d', strtotime($_POST['tanggalSampai'])) . '\' ';
