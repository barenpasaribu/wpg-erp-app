...
class PDF extends FPDF
{
    ...
    public function Footer()
    {
        ...
        break;
    case 'getKodeorg':
        $pabrik = $_POST['kdPabrik'];   //substr($_POST['kdPabrik'],0,3);
        ...
        if (2 == $tipeIntex) {
			...
			//$sOrg = 'SELECT namaorganisasi,kodeorganisasi FROM '.$dbname.".organisasi WHERE tipe='KEBUN' and induk ='".$pabrik."' order by namaorganisasi asc";
        } else {
            if (0 == $tipeIntex) {
                $sOrg = 'SELECT a.supplierid, a.namasupplier FROM '.$dbname.".log_5supplier a INNER JOIN log_5klsupplier b ON a.kodekelompok = b.kode WHERE a.kodetimbangan != 'NULL' AND b.istbs = 1 AND a.kodekelompok <> 'S001' AND a.kodekelompok <> 'S002' AND a.kodekelompok <> 'S003' AND
                a.kodekelompok <> 'S004' AND a.kodekelompok <> 'S005' AND a.kodekelompok <> 'S006' order by a.namasupplier asc";
        ...
        if (3 != $tipeIntex) {
            ...
            while ($rOrg = mysql_fetch_assoc($qOrg)) {
                if (0 != $tipeIntex) {
                    ...
                    $optorg .= '<option value="'.$rOrg['supplierid'].'" selected="">'.$rOrg['namasupplier']. ' - '.$rOrg['supplierid'].'</option>';                    
                    //$optorg .= '<option value="'.$rOrg['supplierid'].'" selected="">'.$rOrg['supplierid']. ' - '.$rOrg['namasupplier'].'</option>';
