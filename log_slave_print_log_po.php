...
$grandtotal = $rlopoht->nilaipo;
if($grandtotal == 0){
	$grandtotal = (($rlopoht->subtotal-$rlopoht->nilaidiskon)+$rlopoht->ppn)-$rlopoht->pph+$rlopoht->ongkosangkutan+$rlopoht->ongkirimppn+$rlopoht->misc;
	$queryUpdate = "UPDATE log_poht SET nilaipo='".$grandtotal."' where nopo='".$_GET['column']."'";
	mysql_query($queryUpdate);
	saveLog($queryUpdate);
}
$pdf->Cell(29, 5, $_SESSION['lang']['grnd_total'], 0, 0, 'L', 1);
$pdf->Cell(26, 5, $rlopoht->matauang . ' ' . number_format($grandtotal, $desiomal, '.', ','), 0, 1, 'R', 1);
//terbilang($totalAll['totalSemua'], 2)
$pdf->Cell(35, 4, 'Terbilang: '.terbilang($grandtotal,2).' Rupiah', 0, 1, 'L');
