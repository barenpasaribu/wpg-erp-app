...
$col1 = 'noakun,jumlah,noaruskas,matauang,kode,nik,kodesupplier';
...
switch ($proses) {
    case 'pdf':
        ...
        if (!empty($resH[0]['nobayar'])) {
            $pdf->Cell($width, $height, 'No. Pembayaran : '.$resH[0]['nobayar'], 0, 1, 'L', 1);
        }
        ...
        if (!empty($resH[0]['tanggalposting'])) {
            $pdf->Cell($width, $height, $_SESSION['lang']['tanggalposting'].' : '.tanggalnormal($resH[0]['tanggalposting']), 0, 1, 'L', 1);
        }
        ...
        $pdf->Cell($width, $height, $_SESSION['lang']['cgttu'].' : '.$resH[0]['cgttu'].$nogiro, 0, 1, 'L', 1);
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 9);
            ...
            foreach ($row as $key => $cont) {
                if ('nomor' === $key) {
                    ...
                    if ('debet' === $key || 'kredit' === $key) {
                        ...
                        if ('noakun' === $key) {
                            if ('121' === substr($cont, 0, 3) && 1 === $resH[0]['hutangunit']) {
