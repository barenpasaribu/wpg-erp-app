...
if ('' !== $pt) {
    $whr .= " and kodeorg like '".$pt."%'";
}

if ('all' !== $customer) {
    $whr .= " and kodecustomer = '".$customer."'";
}

$str = 'select * from '.$dbname.".aging_piutang_vw\r\n      where tanggal > '2012-12-31' and (nilaiinvoice > dibayar or dibayar is NULL)\r\n   ".$whr.' order by namacustomer asc';
saveLog($str);
$res = mysql_query($str);
$no = 0;
if (@mysql_num_rows($res) < 1) {
    ...
    while ($bar = mysql_fetch_assoc($res)) {
        if ($kodecustomer !== $bar['kodecustomer']) {
            $kodecustomer = $bar['kodecustomer'];
            $i = 'select count(kodecustomer) as rowdt from '.$dbname.".aging_piutang_vw where tanggal > '2012-12-31' and (nilaiinvoice > dibayar or dibayar is NULL) and kodecustomer='".$kodecustomer."' order by tanggal asc";
            $n = mysql_query($i);
        ...
        $nopokontrak = $bar['noorder'];
        $nilaipo = $bar['nilaipo'];
        $nilaikontrak = $bar['nilaikontrak'];
        ...
        ++$row;
        if ($row === $rowd) {
            echo "<thead><tr>\r\n                                  <td colspan=6 align=center width=20><b>".$_SESSION['lang']['subtotal'].' '.$namacustomer."</b></td>\r\n                                  <td align=right><b>";
