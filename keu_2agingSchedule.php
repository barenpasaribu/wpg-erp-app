...
echo open_body();
echo "<script language=javascript1.2 src='js/keu_2agingSchedule.js'></script>\r\n";
...
//}
$str = 'select DISTINCT(kodesupplier), namasupplier from '.$dbname.".aging_sch_vw where kodeorg like '".$kodeorg."%' order by namasupplier";
$res = mysql_query($str);
$optsupplier = "<option value=''>".$_SESSION['lang']['all'].'</option>';
while ($bar = mysql_fetch_object($res)) {
    $optsupplier .= "<option value='".$bar->kodesupplier."'>".$bar->namasupplier.'</option>';
}

echo "<fieldset>\r\n     <legend>".$_SESSION['lang']['usiahutang']."</legend>\r\n\t ".$_SESSION['lang']['pt'].' : '."<select id=pt style='width:200px;'>".$optpt."</select>\r\n\t <select id=gudang style='width:150px;' hidden>".$optgudang."</select>\r\n  Supplier <select id=kodesupplier style='width:150px;'>".$optsupplier."</select> <input type=\"text\" value=\"".($tanggalpivot = date('d-m-Y')."\" class=\"myinputtext\" id=\"tanggalpivot\" name=\"tanggalpivot\" onmousemove=\"setCalendar(this.id)\" onkeypress=\"return false;\"  maxlength=\"10\" style=\"width:100px;\" />\r\n\t <button class=mybutton onclick=getUsiaHutang()>".$_SESSION['lang']['proses']."</button>\r\n\t </fieldset>");
CLOSE_BOX();
