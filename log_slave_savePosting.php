...
if (isTransactionPeriod()) {
    ...
    $res = mysql_query($str);
    while ($bar = mysql_fetch_object($res)) {
		$induknyaGudang = $bar->induk;
    }
	$induknyaUntukUnit= $_POST['untukunit'];
    /*
	$str = 'select induk from '.$dbname.".organisasi where kodeorganisasi='".$_POST['untukunit']."'";
    $res = mysql_query($str);
    while ($bar = mysql_fetch_object($res)) {
		$induknyaUntukUnit = $bar->induk;
    }
	*/
	// ----------------------------------

	$periode = $_SESSION['gudang'][$gudang]['tahun'].'-'.$_SESSION['gudang'][$gudang]['bulan'];
    $str = 'select tutupbuku from '.$dbname.".setup_periodeakuntansi where periode='".$periode."' and kodeorg='".$induknyaGudang."'";
    $res = mysql_query($str);
    $close = 0;
    while ($bar = mysql_fetch_object($res)) {
        $close = $bar->tutupbuku;
    ...
    if ($gudangx!='' && $induknyaGudang != substr($gudangx, 0, 4)) {
    ...
    if ($tipetransaksi=='1') {
        ...
        $updmaster = 'update '.$dbname.'.log_5masterbarangdt set saldoqty='.$newsaldoakhirqty.",\r\n                                hargalastin=".$newhargarata." where kodegudang='".$gudang."' and kodebarang='".$kodebarang."' and left(kodeorg,3)='".substr($kodept,0,3)."'";
...
//            exit('Error: Account no. for material or supplier not available yet for '.$notransaksi);
        ...
        $kodeJurnal = 'INVM1';
        $queryJ = selectQuery($dbname, 'keu_5kelompokjurnal', 'nokounter', "left(kodeorg,3)='".substr($kodept,0,3)."' and kodekelompok='".$kodeJurnal."' ");
        $tmpKonter = fetchData($queryJ);
        $konter = addZero($tmpKonter[0]['nokounter'] + 1, 3);
        $nojurnal = str_replace('-', '', tanggalsystem($tanggal)).'/'.$induknyaGudang.'/'.$kodeJurnal.'/'.$konter;
        ...
        $noUrut = 1;
        $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunbarang, 'keterangan' => 'Pembelian barang '.$namabarang.' '.$jumlah.' '.$satuan, 'jumlah' => $nilaitotal, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => $supplier, 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => $nopo, 'kodeblok' => '', 'revisi' => '0'];
        ++$noUrut;
        $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunspl, 'keterangan' => 'Pembelian barang '.$namabarang.' '.$jumlah.' '.$satuan, 'jumlah' => -1 * $nilaitotal, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => $supplier, 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => $nopo, 'kodeblok' => '', 'revisi' => '0'];
        ++$noUrut;
...
//                $headErr .= 'Insert Header Error : '.addslashes(mysql_error($conn))."\n";
            ...
            if ($headErr=='') {
                ...
                if ($detailErr=='') {
                    $updJurnal = updateQuery($dbname, 'keu_5kelompokjurnal', ['nokounter' => $konter], "left(kodeorg,3)='".substr($kodept,0,3)."' and kodekelompok='".$kodeJurnal."'");
    ...
    if ($tipetransaksi=='6') {
        ...
        $instmaster = ' insert into '.$dbname.".log_5masterbarangdt(\r\n                                kodeorg, kodebarang, saldoqty, hargalastin, hargalastout, \r\n                                stockbataspesan, stockminimum, lastuser,kodegudang) values(\r\n                                '".$kodept."','".$kodebarang."',".$newsaldoakhirqty.','.$newhargarata.",\r\n                                0,0,0,".$user.",'".$gudang."'\r\n                                )";
        $updmaster = 'update '.$dbname.'.log_5masterbarangdt set saldoqty='.$newsaldoakhirqty.",\r\n                                hargalastout=".$newhargarata." where kodegudang='".$gudang."' and kodebarang='".$kodebarang."' and left(kodeorg,3)='".substr($kodept,0,3)."'";
        $kodekl = substr($supplier, 0, 4);
        $str = 'select noakun from '.$dbname.".log_5klsupplier where kode='".$kodekl."'";
        $res = mysql_query($str);
        $akunspl = '';
        ...
        $dataRes['header'] = ['nojurnal' => $nojurnal, 'kodejurnal' => $kodeJurnal, 'tanggal' => tanggalsystem($tanggal), 'tanggalentry' => date('Ymd'), 'posting' => 1, 'totaldebet' => $nilaitotal, 'totalkredit' => -1 * $nilaitotal, 'amountkoreksi' => '0', 'noreferensi' => $notransaksi, 'autojurnal' => '1', 'matauang' => 'IDR', 'kurs' => '1', 'revisi' => '0'];
        $noUrut = 1;
        $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunspl, 'keterangan' => 'ReturSupplier '.$namabarang.' '.$jumlah.' '.$satuan, 'jumlah' => $nilaitotal, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => $supplier, 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => $nopo, 'kodeblok' => '', 'revisi' => '0'];
        ++$noUrut;
        $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunbarang, 'keterangan' => 'ReturSupplier '.$namabarang.' '.$jumlah.' '.$satuan, 'jumlah' => -1 * $nilaitotal, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => $supplier, 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => $nopo, 'kodeblok' => '', 'revisi' => '0'];
        ++$noUrut;
        $updflagststussaldo = 'update '.$dbname.'. log_transaksidt set statussaldo=1,hargarata='.$newhargarata.',jumlahlalu='.$cursaldo."\r\n   where notransaksi='".$notransaksi."' and kodebarang='".$kodebarang."' and kodeblok='".$blok."'";
        if ((substr($kodebarang, 0, 3) < '400' || substr($kodebarang, 0, 1)=='9') && trim($akunbarang)!='') {
            $insHead = insertQuery($dbname, 'keu_jurnalht', $dataRes['header']);
...
//                $headErr .= 'Insert Header Error : '.addslashes(mysql_error($conn))."\n";
        ...
        if ($tipetransaksi=='2') {
            ...
            $ptGudang = '';
            $str = 'select induk from '.$dbname.".organisasi where kodeorganisasi='".$induknyaGudang."'";
            $res = mysql_query($str);
            while ($bar = mysql_fetch_object($res)) {
                $ptGudang = $bar->induk;
            }
            ...
            if ($ptGudang != $ptpengguna) {
                $str = 'select akunpiutang from '.$dbname.".keu_5caco where kodeorg='".$induknyaGudang."' and jenis='inter'";
                $res = mysql_query($str);
                $akunspl = '';
                while ($bar = mysql_fetch_object($res)) {
                ...
                if ($pengguna != $induknyaGudang) {
                    $str = 'select akunpiutang from '.$dbname.".keu_5caco where kodeorg='".$induknyaGudang."' and jenis='intra'";
                    $res = mysql_query($str);
                    $akunspl = '';
                    while ($bar = mysql_fetch_object($res)) {
                        $akunspl = $bar->akunpiutang;
            ...
            if ($induknyaGudang==$pengguna) {
                ...
                $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunbarang, 'keterangan' => $keterangan, 'jumlah' => $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => $kodemesin, 'nodok' => '', 'kodeblok' => $blok, 'revisi' => '0'];
                ++$noUrut;
                $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunpekerjaan, 'keterangan' => $keterangan, 'jumlah' => -1 * $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => $kodekegiatan, 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => $kodemesin, 'nodok' => '', 'kodeblok' => $blok, 'revisi' => '0'];
                ++$noUrut;
...
//                        $headErr .= 'Insert Header Error : '.addslashes(mysql_error($conn))."\n";
                ...
                $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunbarang, 'keterangan' => $keterangan, 'jumlah' => $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => '', 'kodeblok' => '', 'revisi' => '0'];
                ++$noUrut;
                $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $inter, 'keterangan' => $keterangan, 'jumlah' => -1 * $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => '', 'kodeblok' => '', 'revisi' => '0'];
                if ((substr($kodebarang, 0, 3) < '400' || substr($kodebarang, 0, 1)=='9') && '' != trim($akunbarang)) {
                    $insHead = insertQuery($dbname, 'keu_jurnalht', $dataRes['header']);
                    if (!mysql_query($insHead)) {
//                        $headErr .= 'Insert Header Error : '.addslashes(mysql_error($conn))."\n";
...
//                        $headErr .= 'Insert Header Error : '.addslashes(mysql_error($conn))."\n";
                ...
                if ($tipetransaksi=='7') {
                    ...
                    $akunspl = '';
                    if ($ptGudang != $ptpengguna) {
                        $str = 'select akunhutang from '.$dbname.".keu_5caco where left(kodeorg,4)='".$induknyaGudang."' and jenis='inter'";
                        $res = mysql_query($str);
                        $akunspl = '';
                        while ($bar = mysql_fetch_object($res)) {
                        ...
                        if ($pengguna != $induknyaGudang) {
                            $str = 'select akunhutang from '.$dbname.".keu_5caco where left(kodeorg,4)='".$induknyaGudang."' and jenis='intra'";
                            $res = mysql_query($str);
                            $akunspl = '';
                            while ($bar = mysql_fetch_object($res)) {
                                $akunspl = $bar->akunhutang;
                    ...
                    if ($pengguna == $induknyaGudang) {
                        ...
                        $kodeJurnal = 'INVK1';
                        $queryJ = selectQuery($dbname, 'keu_5kelompokjurnal', 'nokounter', "kodeorg='".substr($gudang, 0, 3)."' and kodekelompok='".$kodeJurnal."' ");
                        $tmpKonter = fetchData($queryJ);
                        $konter = addZero($tmpKonter[0]['nokounter'] + 1, 3);
                        $nojurnal = str_replace('-', '', tanggalsystem($tanggal)).'/'.$induknyaGudang.'/'.$kodeJurnal.'/'.$konter;
                        $header1pemilik = $nojurnal;
                        ...
                        $keterangan = substr($keterangan, 0, 150);
                        $noakunmutasi = '1210299';
                        $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $inter, 'keterangan' => $keterangan, 'jumlah' => $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => '', 'kodeblok' => '', 'revisi' => '0'];
                        ++$noUrut;
                        $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $noakunmutasi, 'keterangan' => $keterangan, 'jumlah' => $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => '', 'kodeblok' => '', 'revisi' => '0'];
                        ++$noUrut;
                        $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunbarang, 'keterangan' => $keterangan, 'jumlah' => -1 * $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => '', 'kodeblok' => '', 'revisi' => '0'];
                        ++$noUrut;
                        $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $inter, 'keterangan' => $keterangan, 'jumlah' => -1 * $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => '', 'nodok' => '', 'kodeblok' => '', 'revisi' => '0'];
                        if ((substr($kodebarang, 0, 3) < '400' || substr($kodebarang, 0, 1)=='9') && '' != trim($akunbarang)) {
                            $insHead = insertQuery($dbname, 'keu_jurnalht', $dataRes['header']);
...
//                                $headErr .= 'Insert Header Error : '.addslashes(mysql_error($conn))."\n";
                    ...
                    if ($tipetransaksi=='5') {
						...
						if($hargarata==0){
							$sqlUpRata="UPDATE log_5masterbarangdt SET hargalastin=hargalastout  WHERE hargalastin>0 AND left(kodeorg,3)='".substr($kodept,0,3)."' AND kodebarang='".$kodebarang."' and kodegudang='".$gudang."'";
                        ...
                        $updmaster = 'update '.$dbname.'.log_5masterbarangdt set saldoqty='.$newsaldoakhirqty.",\r\n                            hargalastout=".$newhargarata." where kodegudang='".$gudang."' and kodebarang='".$kodebarang."' and left(kodeorg,3)='".substr($kodept,0,3)."'";
                            ...
                            $sqlSelectNmris="SELECT nomris FROM log_transaksiht WHERE  notransaksi='".$notransaksi."' LIMIT 1 ";

                            //echo "warning: ".$sqlSelectRata;
                            //exit();

                            $resSelectNmris=mysql_query($sqlSelectNmris);
                            while($r = mysql_fetch_object($resSelectNmris)) {
                                $notransmris= $r->nomris;
                            }
                        $statpost = 1;
                        $updtmtht = "update log_transaksiht set post='".$statpost."' where nomris='".$notransmris."'";
                        $pengguna = $induknyaUntukUnit;
                        $ptpengguna = '';
                        $str = 'select induk from '.$dbname.".organisasi where kodeorganisasi='".$pengguna."'";
                        $res = mysql_query($str);
                        ...
                        $akunspl = '';
                        if ($ptGudang != $ptpengguna) {
                            $str = 'select akunhutang from '.$dbname.".keu_5caco where kodeorg='".$induknyaGudang."' and jenis='inter'";
                            $res = mysql_query($str);
                            $akunspl = '';
                            while ($bar = mysql_fetch_object($res)) {
                                $akunspl = $bar->akunhutang;
                        ...
                        if ($induknyaGudang==$pengguna) {
                            ...
                            $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunpekerjaan, 'keterangan' => $keterangan, 'jumlah' => $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => $kodekegiatan, 'kodeasset' => $kodeasset, 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => $kodemesin, 'nodok' => '', 'kodeblok' => $blok, 'revisi' => '0'];
                            ++$noUrut;
                            $dataRes['detail'][] = ['nojurnal' => $nojurnal, 'tanggal' => tanggalsystem($tanggal), 'nourut' => $noUrut, 'noakun' => $akunbarang, 'keterangan' => $keterangan, 'jumlah' => -1 * $jumlah * $hargarata, 'matauang' => 'IDR', 'kurs' => '1', 'kodeorg' => $induknyaGudang, 'kodekegiatan' => '', 'kodeasset' => '', 'kodebarang' => $kodebarang, 'nik' => '', 'kodecustomer' => '', 'kodesupplier' => '', 'noreferensi' => $notransaksi, 'noaruskas' => '', 'kodevhc' => $kodemesin, 'nodok' => '', 'kodeblok' => $blok, 'revisi' => '0'];
                            ++$noUrut;
                            if ((substr($kodebarang, 0, 3) < '400' || substr($kodebarang, 0, 1)=='9') && trim($akunbarang)!='') {
                                $insHead = insertQuery($dbname, 'keu_jurnalht', $dataRes['header']);
                                if (!mysql_query($insHead)) {
                                    ...
                                    if ($detailErr=='') {
                                        ...
                                        if (!mysql_query($strupdate)) {
                                            ...
                                            if (!mysql_query($updmaster)) {
                                             ...
                                             if (!mysql_query($updtmtht)) {
                                                $errGudang = ' Gagal update Mris';
                                            }
