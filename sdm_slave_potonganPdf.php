...
$pdf->Cell(20, 5, $_SESSION['lang']['potongan'], 1, 0, 'C', 1);

$pdf->Cell(40, 5, $_SESSION['lang']['keterangan'], 1, 1, 'C', 1);

$arrNmtp = ['0', 'Staff', 3 => 'SKU', 4 => 'SKUP'];
...
if ('KANWIL' == $_SESSION['empl']['tipelokasitugas']) {
    ...
    $str = 'select * from '.$dbname.".sdm_potongandt where periodegaji='".$tgl."' ".'and kodeorg in (select distinct kodeunit from '.$dbname.".bgt_regional_assignment where regional='".$_SESSION['empl']['regional']."')\r\n                      and tipepotongan='".$tppot."'  order by nik asc";

} else {

    $str = 'select * from '.$dbname.".sdm_potongandt where periodegaji='".$tgl."' "."and kodeorg='".$_SESSION['empl']['lokasitugas']."'\r\n                      and tipepotongan='".$tppot."'  order by nik asc";
...
while ($res = mysql_fetch_assoc($re)) {
    ...
    $pdf->Cell(60, 5, $rKry['namakaryawan'], 1, 0, 'L', 1);

    $pdf->Cell(15, 5, $arrNmtp[$rKry['tipekaryawan']], 1, 0, 'L', 1);

    $pdf->Cell(25, 5, $res['kodeorg'], 1, 0, 'L', 1);

    $pdf->Cell(20, 5, number_format($res['jumlahpotongan']), 1, 0, 'R', 1);
...
$pdf->Cell(20, 5, number_format($totPot), 1, 0, 'R', 1);
