...
$sShow = 'select * from '.$dbname.'.guidance_english order by kode asc,tentang,modul,isi limit '.$offset.','.$limit.' ';
$qShow = mysql_query($sShow) || exit(mysql_error($conns));
while ($row = mysql_fetch_assoc($qShow)) {
    ++$no;
    echo '<script>loadNData_en()</script>';
    echo "<td><img src=images/edit.png class=resicon  title='Edit' onclick=\"editRow_en('".$row['kode']."','".$row['tentang']."','".$row['modul']."','".str_replace(["\r", "\n"], '\\n', $row['isi'])."','".$row['tujuan']."');\" ></td>";
    ...
    echo "<td><img src=images/delete1.jpg class=resicon  title='Delete' onclick=\"delData_en('".$row['kode']."','".$row['tentang']."','".$row['modul']."','".str_replace(["\r", "\n"], '\\n', $row['isi'])."')></td></tr>";
}
