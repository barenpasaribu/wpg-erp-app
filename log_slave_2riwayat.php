...
// FA 20191124 - nambahi $cjumlah untuk qty sdh diterima di gudang

switch ($proses) {
	case 'getPO':
		$str = "select * from (".
			getQuery("pp").
			") x where x.nopp like '%".$nmPO."%'";
		...
		while ($rSupplier = mysql_fetch_assoc($qSupplier)) {
			$no += 1;
			echo '<tr class=rowcontent onclick=setDataPO(\'' . $rSupplier['nopp'] . '\')>' . "\r\n" .
				'                         <td>' . $no . '</td>' . "\r\n" .
...
//	else {
		...
		// FA 20191122 - Buka untuk semua unit dalam 1 PT jika HO (hardcoded huruf akhir H)
		$cLokasi= $_SESSION['empl']['lokasitugas'];
		if (substr($_SESSION['empl']['lokasitugas'],3,1)=='H'){
			$cLokasi = substr($cLokasi,0,3);
		}
		$sortLokasi = 'a.nopp like \'%' . $cLokasi . '%\'';
...
//	}
		...
		$offset = $page * $limit;
		$sLim = 'SELECT count(*) as jmlhrow FROM ' . $dbname . '.log_prapodt a left join ' . $dbname . '.log_prapoht b on a.nopp=b.nopp where ' . $sortLokasi . ' order by a.nopp desc';
		#exit(mysql_error());
		($query2 = mysql_query($sLim)) || true;
		while ($jsl = mysql_fetch_object($query2)) {
			$jlhbrs = $jsl->jmlhrow;
		}
		$sql = "select a.*,b.*, a.keterangan as keterangan2 FROM $dbname.log_prapodt a ".
			"left join $dbname.log_prapoht b on a.nopp=b.nopp ".
			"where " .
			$sortLokasi . "order by b.tanggal desc, a.nopp desc ".
			"limit " . $offset . "," . $limit;
		...
		if (0 < $row) {
			...
			while ($res = mysql_fetch_assoc($query2)) {
				...
				if (!is_null($rDet['nopo']) || ($rDet['nopo'] != '')) {
					...
					$stat = 1;
					$nopo = $rPo2['nopo'];
					$statPp = $_SESSION['lang']['disetujui'].", tgl: ".tanggalnormal($tgl1); //FA- 20191124
				}
				else {
					$tGl1 = substr($res['tanggal'], 0, 4);
					$tGl2 = substr($res['tanggal'], 5, 2);
				...
				if ($rDet['nopo'] != '') {
					$sRapb = 'select notransaksi,tanggal,jumlah from ' . $dbname . '.log_transaksi_vw ' . "\r\n" . '                                                    where nopo=\'' . $rDet['nopo'] . '\' and kodebarang=\'' . $res['kodebarang'] . '\'';
					#exit(mysql_error());
...
//	else {
		...
		$sortLokasi = 'and a.nopp like \'%' . $cLokasi . '%\'';

//		$sortLokasi = 'and a.nopp like \'%' . $_SESSION['empl']['lokasitugas'] . '%\'';
//	}

		if (($nopp == '') && ($tglSdt == '') && ($statusPP == '') && ($periode == '') && ($lokBeli == '') && ($_POST['supplier_id'] == '') && ($_POST['txNmbrg'] == '') && ($_POST['purchsrId'] == '')) {
			$tglSkrng = date('Y-m');
