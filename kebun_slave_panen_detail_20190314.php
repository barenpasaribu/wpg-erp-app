...
switch ($proses) {
	...
	case 'updUpah':
		...
		$str = "select tahuntanam, norma, sum(hasilkerja-brondolan) as tothasilkerja, 
			...
			where substr(notransaksi,1,8) = '".$subnotrans."' and nik = '".$param['nik']."' group by substr(notransaksi,1,8)";
		...
		$isgabunghasil = 0;
		if (($tothasilkerja<=$xnorma) && ($tothasilkerja+$hasilKg>$xnorma)){
			$isgabunghasil = 1;
...
*/
		...
		// Jika bukan hari Minggu 
			...
			if ($isgabunghasil == 0) {
				...
				where kodeorg='".$_SESSION['empl']['regional']."' and tahuntanam='".$param['tahuntanam']
				."' and (hasilpanen between 0 and ".round($jumlahlbhbasis, 0, PHP_ROUND_HALF_UP).') order by hasilkg desc';                
			} else {
				$sPremi = 'select distinct * from '.$dbname.".kebun_5premipanen 
				where kodeorg='".$_SESSION['empl']['regional']."' and tahuntanam='".$xtahuntanam
				."' and (hasilpanen between 0 and ".round($jumlahlbhbasis, 0, PHP_ROUND_HALF_UP).') order by hasilkg desc';                
			}
			$qPremi = mysql_query($sPremi) ;
			while ($rPremi = mysql_fetch_assoc($qPremi)) {
