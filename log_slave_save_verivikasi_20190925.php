...
case 'refresh_data':
	...
	if (mysql_query($str)) {
		...
		while ($bar = mysql_fetch_object($res)) {
			...
			$jmlHari = $jd2 - $jd1;
			$optPur = '<option value=\'\'></option>';
			$klq = 'select karyawanid,namakaryawan from ' . $dbname . '.`datakaryawan` where  bagian=\'HO_PUR\' and (tanggalkeluar is NULL or tanggalkeluar="0000-00-00") order by namakaryawan asc ';
			...
			if (($bar->realisasi == '0') || ($bar->realisasi == '')) {
				echo "\r\n\t\t\t\t\t\t\t" . '  <td align=right>' . "\r\n" . '                              ' . "\r\n\t\t\t\t\t\t\t" . '  ' . "\t" . '<input type=text id=realisasi_' . $no . ' name=realisasi_' . $no . ' onkeypress=\'return angka_doang(event)\' class=\'myinputtextnumber\'  value=' . $bar->jumlah . ' style=\'width:60px;\' />' . "\r\n\t\t\t\t\t\t\t" . '  </td>';
			}
			else {
				echo '<td align=right>' . "\r\n" . '                               ' . "\t" . '<input type=text id=realisasi_' . $no . ' name=realisasi_' . $no . ' onkeypress=\'return angka_doang(event)\' class=\'myinputtextnumber\' ' . $read_only2 . ' value=' . $bar->realisasi . ' style=\'width:60px;\' />' . "\r\n\t\t\t\t\t\t\t" . '  </td>';
			}
