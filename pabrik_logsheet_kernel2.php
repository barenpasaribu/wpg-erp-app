...
if( isset($_POST['btnSubmit']) ){
	...
	{	
		...
		$qry="insert into ".$dbname.".tmp_kernel2ht values('".$nama_file."','".$kdOrg."','".$date."'); ";
		...
		{ 
			...
			$tgl=$data->val($i, 1);	
			$jam=$data->val($i, 2);				
			$air=preg_replace("/[^0-9.]/", "", $data->val($i, 3));
			$caco3=preg_replace("/[^0-9.]/", "", $data->val($i, 4));			
			$abu=preg_replace("/[^0-9.]/", "", $data->val($i, 5));
			...
			// menyimpan data xls ke db tmp_absensidt
			 $qry1="insert into ".$dbname.".tmp_kernel2dt values('".$nama_file."','".$tgl."','".$jam."','".$air."','".$caco3."','".$abu."',current_timestamp); ";				
		...
		*/
		...
		// */
		// $totx= $res1['totx'];
		$squery="SELECT * FROM tmp_kernel2dt  where namafile='".$nama_file."' ";
...
if( $_POST['btnSubmitData']!=""){
	...
	if( $total_data > 0){
		...
		// get data karyawan
		$squery="SELECT a.*,b.kodeorganisasi as kodeorg FROM tmp_kernel2dt a,tmp_kernel2ht b  where a.namafile=b.namafile and a.namafile='".$namafile."' ";
		// echo "<pre>";
		// print_r($squery);
		// die();
		$hasil = mysql_query($squery);
		while ($res = mysql_fetch_array($hasil)) { 
			...
			$queryCekAbsensiDT = "SELECT * FROM pabrik_kernel2 WHERE kodeorg = '".$kodeorg."' and tgl = '".$tgl."' and jam = '".$jam."'  ";
			...
			if($jumlahCekAbsensiDT > 0){
				$sDetIns = "UPDATE ".$dbname.".pabrik_kernel2 
				 ...
				 $sDetIns="insert into ".$dbname.".power_kernel2 values('".$namafile."','".$tgl."','".$jam."','".$air."','".$caco3."','".$abu."','".$kdOrg."',current_timestamp)"; 
			}
		...
		$emptyQueryTable2 = "TRUNCATE TABLE tmp_kernel2dt";
...
OPEN_BOX('', 'Pabrik Logsheet Kernel 2 Upload</b>');
