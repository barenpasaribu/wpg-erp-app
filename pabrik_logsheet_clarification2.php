...
if( isset($_POST['btnSubmit']) ){
	...
	{	
		...
		$qry="insert into ".$dbname.".tmp_klarification2ht values('".$nama_file."','".$kdOrg."','".$date."'); ";
		...
		{ 
			...
			$sc1_start=preg_replace("/[^0-9.]/", "", $data->val($i, 3));
			$sc1_stop=preg_replace("/[^0-9.]/", "", $data->val($i, 4));	
			$sc2_start=preg_replace("/[^0-9.]/", "", $data->val($i, 3));
			$sc2_stop=preg_replace("/[^0-9.]/", "", $data->val($i, 4));	
			$sc3_start=preg_replace("/[^0-9.]/", "", $data->val($i, 3));
			$sc3_stop=preg_replace("/[^0-9.]/", "", $data->val($i, 4));	
			$sc4_start=preg_replace("/[^0-9.]/", "", $data->val($i, 3));
			$sc4_stop=preg_replace("/[^0-9.]/", "", $data->val($i, 4));	
			$sc5_start=preg_replace("/[^0-9.]/", "", $data->val($i, 3));
			$sc5_stop=preg_replace("/[^0-9.]/", "", $data->val($i, 4));	
			...
			// menyimpan data xls ke db tmp_absensidt
			 $qry1="insert into ".$dbname.".tmp_klarification2dt values('".$nama_file."','".$tgl."','".$shift_nik."','".$sc1_start."','".$sc1_stop."','".$sc2_start."','".$sc2_stop."','".$sc3_start."','".$sc3_stop."','".$sc4_start."','".$sc4_stop."','".$sc5_start."','".$sc5_stop."',current_timestamp); ";				
		...
		*/
		...
		// */
		// $totx= $res1['totx'];
		$squery="SELECT * FROM tmp_klarification2dt  where namafile='".$nama_file."' ";
		...
		while ($res = mysql_fetch_array($hasil)) { 
			...
			$sc1_start = $res['sc1_start'];
			$sc1_stop = $res['sc1_stop'];
			$sc2_start = $res['sc2_start'];
			$sc2_stop = $res['sc2_stop'];
			$sc3_start = $res['sc3_start'];
			$sc3_stop = $res['sc3_stop'];
			$sc4_start = $res['sc4_start'];
			$sc4_stop = $res['sc4_stop'];
			$sc5_start = $res['sc5_start'];
			$sc5_stop = $res['sc5_stop'];
...
if( $_POST['btnSubmitData']!=""){
	...
	if( $total_data > 0){
		...
		// get data karyawan
		$squery="SELECT a.*,b.kodeorganisasi as kodeorg FROM tmp_klarification2dt a,tmp_klarification2ht b  where a.namafile=b.namafile and a.namafile='".$namafile."' ";
		// echo "<pre>";
		// print_r($squery);
		// die();
		$hasil = mysql_query($squery);
		while ($res = mysql_fetch_array($hasil)) { 
			...
			$queryCekAbsensiDT = "SELECT * FROM pabrik_klarification2 WHERE kodeorg = '".$kodeorg."' and tgl = '".$tgl."' and shift_nik = '".$shift_nik."'  ";
			...
			if($jumlahCekAbsensiDT > 0){
				$sDetIns = "UPDATE ".$dbname.".pabrik_kernel2 
							SET
								sc1_start = '".$sc1_start."',
								sc1_stop='".$sc1_stop."',
								sc2_start = '".$sc2_start."',
				 ...
				 $sDetIns="insert into ".$dbname.".pabrik_klarification2 values('".$namafile."','".$tgl."','".$shift_nik."','".$sc1_start."','".$sc1_stop."','".$sc2_start."','".$sc2_stop."','".$sc3_start."','".$sc3_stop."','".$sc4_start."','".$sc4_stop."','".$sc5_start."','".$sc5_stop."','".$kdOrg."',current_timestamp)"; 
			}
		...
		$emptyQueryTable2 = "TRUNCATE TABLE tmp_klarification2dt";
...
OPEN_BOX('', 'Pabrik Logsheet Klarification 2 Upload</b>');
