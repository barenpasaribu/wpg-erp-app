...
<script type=\"application/javascript\" src=\"js/vhc_penggantianKomponen.js?v=".mt_rand()."\"></script>\r\n<script>\r\n jdl_ats_0='";
...
OPEN_BOX();
$svhc = 'select kodevhc,jenisvhc,tahunperolehan,detailvhc from '.$dbname.'.vhc_5master  order by kodevhc';
$qvhc = mysql_query($svhc);
while ($rvhc = mysql_fetch_assoc($qvhc)) {
    $optVhc .= "<option value='".$rvhc['kodevhc']."'>".$rvhc['kodevhc'].'['.$rvhc['tahunperolehan'].'] ['.$rvhc['detailvhc'].']</option>';
}
$svhc2 = 'select m.kodeorg, o.namaorganisasi from '.$dbname.'.vhc_5master m inner join organisasi o on m.kodeorg=o.kodeorganisasi group by m.kodeorg';
$qvhc2 = mysql_query($svhc2);
while ($rvhc2 = mysql_fetch_assoc($qvhc2)) {
    $optOrg .= "<option value='".$rvhc2['kodeorg']."'>".$rvhc2['namaorganisasi'].'</option>';
}
for ($t = 0; $t < 24; ++$t) {
    if (strlen($t) < 2) {
        $t = '0'.$t;
...
// $i = "select karyawanid,namakaryawan,nik,kodejabatan from $dbname.datakaryawan 
// where 
// lokasitugas in (select kodeunit from $dbname.bgt_regional_assignment where regional='".$_SESSION['empl']['regional']."' and kodeunit like '%RO%') and 
// tipekaryawan!='5' and 
// kodejabatan in (select kodejabatan from $dbname.sdm_5jabatan 
// where alias like '%Kepala Mekanik%' or alias like '%Pelaksana Bengkel%' or alias like '%Mekanik%' or 
// alias like '%Asisten Bengkel%') ";

$i = "select d.karyawanid,d.namakaryawan,d.nik,d.kodejabatan,s.namajabatan from  datakaryawan  d 
inner join sdm_5jabatan s on d.kodejabatan = s.kodejabatan 
where 
s.alias like '%Kepala Mekanik%' or s.alias like '%Pelaksana Bengkel%' or s.alias like '%Mekanik%' or 
s.alias like '%Asisten Bengkel%'  order by namakaryawan ";
$n = mysql_query($i);
while ($d = mysql_fetch_assoc($n)) {
    $optKar .= "<option value='".$d['karyawanid']."'>".$d['nik'].' - '.$d['namakaryawan'].' ('.$d['namajabatan'].')</option>';
}
echo "\r\n";
...
<select id=namaMekanik1 name=namaMekanik1 tipe=mechanic style=width:150px; onchange='cekMekanik(this);'>".$optKar."</select></td>\r\n</tr>\r\n<tr>\r\n\t<td>".$_SESSION['lang']['namamekanik']." 2</td>\r\n\t<td>:</td>\r\n\t<td>
<select id=namaMekanik2 name=namaMekanik2 tipe=mechanic style=width:150px; onchange='cekMekanik(this);'>".$optKar."</select></td>\r\n</tr>\r\n<tr>\r\n\t<td>".$_SESSION['lang']['namamekanik']." 3</td>\r\n\t<td>:</td>\r\n\t<td>
<select id=namaMekanik3 name=namaMekanik3 tipe=mechanic style=width:150px; onchange='cekMekanik(this);'>".$optKar."</select></td>\r\n</tr>\r\n<tr>\r\n\t<td>".$_SESSION['lang']['namamekanik']." 4</td>\r\n\t<td>:</td>\r\n\t<td>
<select id=namaMekanik4 name=namaMekanik4 tipe=mechanic style=width:150px; onchange='cekMekanik(this);'>".$optKar."</select></td>\r\n</tr>\r\n<tr>\r\n\t<td>".$_SESSION['lang']['namamekanik']." 5</td>\r\n\t<td>:</td>\r\n\t<td>
<select id=namaMekanik5 name=namaMekanik5 tipe=mechanic style=width:150px; onchange='cekMekanik(this);'>".$optKar."</select></td>\r\n</tr>\r\n<tr>\r\n\t<td>".$_SESSION['lang']['notransaksi'].' '.$_SESSION['lang']['gudang']."</td>\r\n\t<td>:</td>\r\n\t<td><input type=text id=noTranGudang disabled class=myinputtext size=25 onkeypress=\"return tanpa_kutip(event);\">\r\n\t    <img src=images/zoom.png title='".$_SESSION['lang']['find']."' id=tmblCariNoGudang class=resicon onclick=cariNoGudang('".$_SESSION['lang']['find']."',event)>\r\n\t </td>\r\n</tr>\r\n\r\n\r\n\r\n\r\n\r\n\r\n<tr>\r\n<td colspan=3 id=tmblHeader>\r\n</td>\r\n</tr>\r\n</table>\r\n</fieldset>";
echo "\r\n";
CLOSE_BOX();
