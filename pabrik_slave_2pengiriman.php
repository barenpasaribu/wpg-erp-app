...
switch ($proses) {
    ...
    case 'preview':
        ...
        $sTimbangan = "select kodebarang,notransaksi,kodeorg,jumlahtandan1 as jjg,beratbersih as netto,kodecustomer,jammasuk,jamkeluar,beratmasuk, substr(tanggal,1,10) as tanggal,supir,nokendaraan,nodo,nokontrak,kgpembeli,beratbersih,beratkeluar,nosipb from ".$dbname.".pabrik_timbangan where  millcode='".$kdPabrik."' and kodebarang!='40000003' ".$whr.' and  tanggal >= '.$tgl_1.'000001 and tanggal<='.$tgl_2."235959 order by tanggal asc";
...

{
...

    {
        ...
        if ('' !== $kdPabrik && '' !== $unit) {
            ...
            $this->Cell($width, $height, $_SESSION['lang']['rPengiriman'].' : '.$kdPabrik.' '.$_SESSION['lang']['periode'].' :'.$tgl_1.'-'.$tgl_2, 0, 1, 'C');

        } else {

            $this->Cell($width, $height, $_SESSION['lang']['rPengiriman'].' : '.$kdPabrik.' '.$_SESSION['lang']['periode'].' :'.tanggalnormal($tgl_1).' - '.tanggalnormal($tgl_2), 0, 1, 'C');
        ...
        $this->Cell(10 / 100 * $width, $height, $_SESSION['lang']['materialname'], 1, 0, 'C', 1);
        $this->Cell(5 / 100 * $width, $height, $_SESSION['lang']['tanggal'], 1, 0, 'C', 1);
        $this->Cell(15 / 100 * $width, $height, 'Transporter', 1, 0, 'C', 1);
        ...
        $this->Cell(5 / 100 * $width, $height, $_SESSION['lang']['beratnormal'], 1, 0, 'C', 1);
        $this->Cell(10 / 100 * $width, $height, $_SESSION['lang']['sopir'], 1, 0, 'C', 1);
        $this->Cell(10 / 100 * $width, $height, $_SESSION['lang']['nodo'], 1, 0, 'C', 1);
        $this->Cell(11 / 100 * $width, $height, $_SESSION['lang']['NoKontrak'], 1, 1, 'C', 1);
...

    {
        ...
        while ($rData = mysql_fetch_assoc($qList)) {
            ...
            $pdf->Cell(15 / 100 * $width, $height, $optSupp[$rData['kodecustomer']], 1, 0, 'L', 1);
            ...
            $pdf->Cell(5 / 100 * $width, $height, number_format($rData['netto'], 2), 1, 0, 'R', 1);
            $pdf->Cell(10 / 100 * $width, $height, $rData['supir'], 1, 0, 'L', 1);
            $pdf->Cell(10 / 100 * $width, $height, $rData['nosipb'], 1, 0, 'L', 1);
            $pdf->Cell(11 / 100 * $width, $height, $rData['nokontrak'], 1, 1, 'L', 1);
        ...
        $pdf->Cell(64 / 100 * $width, $height, 'Total', 1, 0, 'R', 1);

        $pdf->Cell(5 / 100 * $width, $height, number_format($subtota, 2), 1, 0, 'R', 1);

        $pdf->Cell(31 / 100 * $width, $height, '', 1, 1, 'C', 1);
    ...
    case 'excel':
        ...
        if (0 < $brs) {
            ...
            $tab .= '<tr class=rowcontent ><td colspan=7 align=right>Total (KG)</td><td align=right>'.number_format($subtota, 2).'</td><td colspan=7 align=right>&nbsp;</td></tr>';
