...
switch ($proses) {
    ...
    case 'generate_no':
        if ($notransaksi !='') {
            $svhc = 'select kodevhc,jenisvhc,tahunperolehan,detailvhc from '.$dbname.".vhc_5master where kodeorg='".$codeOrg."'";
            $qvhc = mysql_query($svhc);
            while ($rvhc = mysql_fetch_assoc($qvhc)) {
                $optVhc .= "<option value='".$rvhc['kodevhc']."' ".(($rvhc['kodevhc'] === $kdJenis ? 'selected' : '')).'>'.$rvhc['kodevhc'].'['.$rvhc['detailvhc'].']</option>';
            }
