...
//	exit();
    ...
    //cek dipakai atau tidak, potongan HK diambil dari sdm_hktdkdibayar_vw
	...
	foreach ($resgjh as $idx => $val) {
		$pengali= 1;
		/*
		if (substr($_SESSION['empl']['lokasitugas'],0,3)=='MPS' || substr($_SESSION['empl']['lokasitugas'],0,3)=='CDS' || substr($_SESSION['empl']['lokasitugas'],0,3)=='SHL'){
			$pengali= 2;
		}
		*/
		...
		$readyData[] = ['kodeorg' => $_SESSION['empl']['lokasitugas'], 'periodegaji' => $param['periodegaji'], 'karyawanid' => $val['karyawanid'], 'idkomponen' => 20, 'jumlah' => $tdkdibayar[$val['karyawanid']], 'pengali' => $pengali, 'updateby' => $_SESSION['standard']['userid'], 'updatetime' => date('Y-m-d H:i:s')];
		*/
	}
...
//	$res1 = mysql_fetch_assoc($resx);
	...
	$persenjhtpt = $jmspersh[0]['jhtpt'] / 100; // tdk dipakai dimana mana
	$persenjppt = $jmspersh[0]['jppt'] / 100; // tdk dipakai dimana mana
	...
	foreach ($tjms as $key => $nilai) {
		...
		if ($nojms[$key] != '-'){
			...
			if ($gabthnbln < 57.01) {
				...
				if ($nilaibatasmax < $nilaix) {
					$readyData[] = ['kodeorg' => $_SESSION['empl']['lokasitugas'], 'periodegaji' => $param['periodegaji'], 'karyawanid' => $key, 'idkomponen' => 57, 'jumlah' => $nilaibatasmaxpt * $persenjhtpt, 'pengali' => 1, 'updateby' => $_SESSION['standard']['userid'], 'updatetime' => date('Y-m-d H:i:s')];
				} else {
					$readyData[] = ['kodeorg' => $_SESSION['empl']['lokasitugas'], 'periodegaji' => $param['periodegaji'], 'karyawanid' => $key, 'idkomponen' => 57, 'jumlah' => $nilaix * $persenjhtpt, 'pengali' => 1, 'updateby' => $_SESSION['standard']['userid'], 'updatetime' => date('Y-m-d H:i:s')];
				}
			} else {
		...
		//Jika ada yg berbeda antara gapok dgn dasar perhitungan BPJS KES - FA 20200212
		// ambil dari sdm_bpjs nominal lain
		...
		*/
    	...
    	if ($sisa[$val[0]] < 0) {
			...
			*/
            ...
            $list2 .= '<td align=right style="background-color:lightgrey">'.number_format($jmspersh[$val[0]], 0, ',', '.').'</td>';
