...
class rTable
{
    ...
    public function prepTable()
    {
        ...
        if ($this->_page > 1) {
            $theTable .= " style='cursor:pointer' onclick=\"newGoToPages(1,".$this->_rowPerPage.','.$where.')"';
        }

        $theTable .= '>&nbsp;';
        $theTable .= "<img src='images/".$_SESSION['theme']."/prev.png'";
        if ($this->_page > 1) {
            $theTable .= " style='cursor:pointer' onclick=\"newGoToPages(".($this->_page - 1).','.$this->_rowPerPage.','.$where.')"';
        }

        $theTable .= '>&nbsp;';
        $theTable .= makeElement('pages', 'select', $this->_page, ['style' => 'width:50px', 'onchange' => 'newChoosePage(this,'.$this->_rowPerPage.','.$where.')'], $optPage).'&nbsp;';
        ...
        if ($this->_page < $this->_totalPage) {
            $theTable .= " style='cursor:pointer' onclick=\"newGoToPages(".$this->_totalPage.','.$this->_rowPerPage.','.$where.')"';
        }
