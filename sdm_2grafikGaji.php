...
echo open_body();
?>
<script language=javascript1.2 src='js/sdm_2grafikGaji.js'></script>
<?
...
if($periode!='' && $unit!=''){
  for($j=1;$j<13;$j++){
    ...
    $sql= mysql_query("SELECT SUM(totbruto) as total FROM sdm_gaji_rekap2_vw WHERE periodegaji='".$periode1."' AND kodeorg='".$unit."'") or die(mysql_error());
...
<table align="center">
  <tr>
    ...
    <?php
      foreach ($data as $key) {
        echo "<tr class=rowcontent><td>".$key['periode']."</td><td align='right'>".number_format($key['jumlah'])."</td></tr>";
      }
    ?>
    </table>
    </td>
...
Highcharts.chart('kontainer', {
    ...
    title: {
        text: 'GRAFIK GAJI KARYAWAN PERIODE <?php echo $periode; ?> '
    },
