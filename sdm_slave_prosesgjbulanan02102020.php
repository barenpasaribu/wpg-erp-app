...
*/
	...
	*/
	/*
	$strgjh = 'select a.karyawanid,sum(jumlah)/'.$hari_bagi.' as gjperhari from '.$dbname.'.sdm_5gajipokok a left join '.$dbname.'.datakaryawan b on a.karyawanid=b.karyawanid where a.tahun='.substr($tanggal1, 0, 4)." and b.lokasitugas='".$_SESSION['empl']['lokasitugas']."' and (b.tanggalkeluar>='".$tanggal2."' or b.tanggalkeluar is NULL or b.tanggalkeluar='0000-00-00') and a.idkomponen in (1,2,15,61) and sistemgaji='Bulanan' group by a.karyawanid ";
	*/
	
	// FA 20200930 - komponen gaji ambil hanya dari gaji pokok (1) dan tunj jabatan (2)
	$strgjh = 'select a.karyawanid,sum(jumlah)/'.$hari_bagi.' as gjperhari from '.$dbname.'.sdm_5gajipokok a left join '.$dbname.'.datakaryawan b on a.karyawanid=b.karyawanid where a.tahun='.substr($tanggal1, 0, 4)." and b.lokasitugas='".$_SESSION['empl']['lokasitugas']."' and (b.tanggalkeluar>='".$tanggal2."' or b.tanggalkeluar is NULL or b.tanggalkeluar='0000-00-00') and a.idkomponen in (1,2) and sistemgaji='Bulanan' group by a.karyawanid ";
	...
	*/
	...
	foreach ($tjms as $key => $nilai) {
		if ('H' == substr($_SESSION['empl']['lokasitugas'], -1)) {
			...
			if (substr($_SESSION['empl']['lokasitugas'],0,3)=='LSP' || substr($_SESSION['empl']['lokasitugas'],0,3)=='SSP' || substr($_SESSION['empl']['lokasitugas'],0,3)=='MPS'){
				...
				$queryusiapensiun = selectQuery($dbname, 'sdm_ho_hr_jms_porsi', '*', "id='usiapensiun' and lokasiresiko='M'");
			} else {
				...
				$queryusiapensiun = selectQuery($dbname, 'sdm_ho_hr_jms_porsi', '*', "id='usiapensiun' and lokasiresiko='E'");
			}
			// ----------------------------------------------------------
		}
		...
		if ('R' == substr($_SESSION['empl']['lokasitugas'], -1)) {
			...
			$queryusiapensiun = selectQuery($dbname, 'sdm_ho_hr_jms_porsi', '*', "id='usiapensiun' and lokasiresiko='R'");
		}
		...
		$nilaibatasmaxpt = $angkapersenlokres[0]['jmppt'];
		$nilaibatasmax3 = $angkapersenlokres[0]['bpjsmpt'];
		...
		$nilaibatasmax = $angkapersenlokres[0]['jmpk'];
		$nilaibatasmax2 = $angkapersenlokres[0]['bpjsmk'];
		
		$nusiapensiun= 57.01;
		$angkausiapensiun = fetchData($queryusiapensiun);
		$nusiapensiun = $angkausiapensiun[0]['value'];
		...
		if ($nojms[$key] != '-' && $nojms[$key] != ''){
			...
			//FA-20200405
			// Jika umur kurang dari 55 tahun 1 bulan, lanjut. Jika lebih, jumlah => 0 (Khusus JHT) -> GAK JADI
			if ($gabthnbln < 99) {
				// JHT Karyawan
		...
		if ($isKes==1){
			...
			if ('' != $nobpjskes[$key]) {
				//$nilaibatasmax2 = 12000000; //kenapa harus hardcode, padahal ada di tabel sdm_ho_hr_jms_porsi, kolom bpjsmk
	...
	and sistemgaji='Bulanan' group by a.karyawanid";
	...
	and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar is NULL or b.tanggalkeluar='0000-00-00') 
	and a.unit like '".$_SESSION['empl']['lokasitugas']."%' and a.tanggal>='".$tanggal1."' and a.tanggal<='".$tanggal2."' 
	and sistemgaji='Bulanan' and c.isharian=1 group by a.karyawanid";
	...
	*/
			...
			if ($val[0] == $bar['karyawanid']) {
				...
				*/
				...
				if (8 == $comp[$bar['idkomponen']] || 8 == $bar['idkomponen']) {
					$potonganbpjskes[$val[0]] += ceil($bar['jumlah']);
				}
    ...
    $list0 .= '<td style="background-color:#efc6b1;font-weight:bold"><b>JP Perusahaan</b></td>
	</tr></thead><tbody>';
    $negatif = false;
    $list1 = '';
	...
	$qTunjLL = 'select kodeorg,karyawanid,sum(jumlah) as jumlah_pph21 from '.$dbname.".sdm_gaji where substr(periodegaji,1,4) ='".substr($param['periodegaji'],0,4)."' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' and idkomponen in (24,71,72) and periodegaji<'".$param['periodegaji']."' group by kodeorg,karyawanid";
    $resTunjLL = fetchData($qTunjLL);
    foreach ($resTunjLL as $key => $val) {
    ...
    $qTunjRU = 'select karyawanid,jumlah from '.$dbname.".sdm_gaji where periodegaji='".$param['periodegaji']."' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' and idkomponen=54";
    $resTunjRU = fetchData($qTunjRU);
    foreach ($resTunjRU as $key => $val) {
    	$rapelupah[$val['karyawanid']] = $val['jumlah'];
    }
	// THR
	$qThr = 'select karyawanid,jumlah from '.$dbname.".sdm_gaji where periodegaji='".$param['periodegaji']."' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' and idkomponen=14";
    $resThr = fetchData($qThr);
    foreach ($resThr as $key => $val) {
    	$thrvalue[$val['karyawanid']] = $val['jumlah'];
    }
	//BONUS
	$qBonus = 'select karyawanid,jumlah from '.$dbname.".sdm_gaji where periodegaji='".$param['periodegaji']."' and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' and idkomponen=13";
    ...
    foreach ($id as $key => $val) {
		...
		// Biaya Jabatan ke-3 (per bulan, masih Hardcoded)
   		$biayajab[$val[0]] = floor($totgajibruto[$val[0]] * 0.05);
    	if ($biayajab[$val[0]] > 500000) {
		...
		if ($nrapel > 0){
			...
			foreach ($resx as $idz => $valx) {
				...
				while ($barx = mysql_fetch_object($resx)) {
					...
					$pph21_bnsrpl= $pph21_bonus_rapel - $pph21rapel_setahun; // buat cek aja
...
if ($idxx=='0000000877'){
	echo "warning pph21 bonus rapel= ".$pph21_bonus_rapel." /pph21 rapel setahun= ".$pph21rapel_setahun." /pph21 rapel sebulan= ".$pphrapel." /zz_bonus rapel= ".$zz_bonus_rapel." /pph21 bonus=".$pph21_bnsrpl." /pph21 bonus lalu= ".$pph21_bonuslalu." /by jab bns rapel=".$biayajabbonus_rapel." /bruto rapel sebulan=".$bruto." /bruto rapel setahun=".$brut." /bonusnya aja=".$bonus_lalu." /netto bonus rapel=".$netto_bonus." /pkp bonus rapel=".$pkpbonus_rapel." /jhtx=".$jhtkarx." /jpx=".$jpkarx;
	exit();
}
*/
...
//			$pph21rapel[$val[0]] = $pphrapel_x + $pph21_bonus_rapel + $pph21_thr_rapel; // FA 20200503
			$pph21rapel[$val[0]] = $pphrapel_x + $pph21_bonus_rapel; // FA 20200503
...
$pph21sekarang= $pphsetahun[$val[0]];
$pph21_akhirnya= $pph21sekarang - $akum;
if ($idxx=='0000000017'){
echo "warning XXX: bln= ".$nbln." /pph21 rapel setahun= ".$pph21rapel_setahun." /pphrapel= ".$pphrapel." /pph rapel akhir= ".$pp21." /pkp= ".$pkpx." /zz_bonus=".$zz_bonus_rapel." /pph21 bonus lalu=".$pph21_bonuslalu." /pph21 bonus rapel=".$pph21_bonus_rapel." /pph21 bulan ini= ".$pph21sekarang." /pph akum= ".$akum." /pph21 akhirnya= ".$pph21_akhirnya;
exit();
}
*/
		...
		if (($tanggalkeluar[$val[0]] != "" && $tanggalkeluar[$val[0]] != "0000-00-00") || $flag_desember == "Y"){
			...
			$qjht_jp = "select sum(jumlah) as tot_jht_jp from sdm_gaji where substr(periodegaji,1,4) ='".substr($param['periodegaji'],0,4)."' and periodegaji < '".$param['periodegaji']."' and karyawanid = '".$idxx."' and idkomponen in (5,9) group by karyawanid";
			$resjht_jp = mysql_query($qjht_jp);
			while ($barjht_jp = mysql_fetch_object($resjht_jp)) {
				$jht_jp = $barjht_jp -> tot_jht_jp;
			}

			$biayajab_resign = floor(($bruto_thrbonus) * 0.05); // by jab resign (Faktor kali dan Max By Jab, masih hardcoded)
			if ($biayajab_resign > (500000 * $pembagi_bulan)) {
				$biayajab_resign = 500000 * $pembagi_bulan;
			}
			if ($biayajab_resign < 0) {$biayajab_resign = 0;}

			$jht_jp = $jht_jp + $jhtkarypersen[$val[0]] + $jpkarypersen[$val[0]]; // FA 20200807
...
//if ($idxx=='0000000021'){
//if ($idxx=='0000000580'){
$jht_jp = $jht_jp + $jhtkarypersen[$val[0]] + $jpkarypersen[$val[0]];
if ($idxx=='0000000017'){
...
/* if ($idxx=='0000000310'){
	...
	exit();
    	...
    	if ($sisa[$val[0]] < 0) {
			...
			$list2 .= '<td align=right style="background-color:#efc6b1"><b>'.number_format($jpptpersen56[$val[0]], 0, ',', '.').'</b></td>
			</tr>';
