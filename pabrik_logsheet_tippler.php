...
if( isset($_POST['btnSubmit']) ){
	...
	{	
		...
		$qry="insert into ".$dbname.".tmp_triplerht values('".$nama_file."','".$kdOrg."','".$date."'); ";
		...
		{ 
			...
			// if (preg_match("///i", $data->val($i, 1))){
				 
			// 	  echo "warning Ok:1 ".$data->val($i, 1);
			// 	 exit();
			// } else {
 		// 		  echo $data->val($i, 1);
 		// 		  echo strtotime($data->val($i, 1));
			// 	 exit();;
			// }
			$tgl=$data->val($i, 1);	
			$shift_nik=$data->val($i, 2);				
			$jml_lorituang=$data->val($i, 3);
			$start_jt= $data->val($i, 4);			
			$stop_jt=$data->val($i, 5);
			...
			// menyimpan data xls ke db tmp_absensidt
			 $qry1="insert into ".$dbname.".tmp_triplerdt values('".$nama_file."','".$tgl."','".$shift_nik."','".$jml_lorituang."','".$start_jt."','".$stop_jt."','".$keterangan."',current_timestamp); ";				
		...
		*/
		...
		// */
		// $totx= $res1['totx'];
		$squery="SELECT * FROM tmp_triplerdt  where namafile='".$nama_file."' ";
...
if( $_POST['btnSubmitData']!=""){
	...
	if( $total_data > 0){
		...
		// get data karyawan
		$squery="SELECT a.*,b.kodeorganisasi as kodeorg FROM tmp_triplerdt a,tmp_triplerht b  where a.namafile=b.namafile and a.namafile='".$namafile."' ";
		// echo "<pre>";
		// print_r($squery);
		// die();
		$hasil = mysql_query($squery);
		while ($res = mysql_fetch_array($hasil)) { 
			...
			$queryCekAbsensiDT = "SELECT * FROM pabrik_tripler WHERE kodeorg = '".$kodeorg."' and tgl = '".$tgl."' and jam = '".$jam."'  ";
			...
			if($jumlahCekAbsensiDT > 0){
				$sDetIns = "UPDATE ".$dbname.".pabrik_kernel2 
				 ...
				 $sDetIns="insert into ".$dbname.".pabrik_tripler values('".$namafile."','".$tgl."','".$shift_nik."','".$jml_lorituang."','".$start_jt."','".$stop_jt."','".$keterangan."','".$kdOrg."',current_timestamp)"; 
			}
		...
		$emptyQueryTable2 = "TRUNCATE TABLE tmp_triplerdt";
...
OPEN_BOX('', 'Pabrik Logsheet Tippler Upload</b>');
