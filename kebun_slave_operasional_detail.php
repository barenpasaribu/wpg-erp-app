...
switch ($proses) {
    case 'getInsentif' :
        $where = $_POST['kodekegiatan'];
        $query = "SELECT rupiah from kebun_5psatuan where kodekegiatan='".$where."'";
        $queryAct = mysql_query($query);
        $data = mysql_fetch_object($queryAct);
        echo  $data->rupiah;
        break;
    case 'cekSisip':
...
$where = "notransaksi='".$param['notransaksi']."'";
$cols = 'nourut,nik,absensi,jjg,hasilkerja,jhk,umr,tarif_premi as insentif,a.insentif as totalpremi, COALESCE(c.rupiah, 0) AS totalInsentif';
$query = selectQuery($dbname, 'kebun_kehadiran', $cols, $where);
$query="SELECT
        a.nourut, a.nik, a.absensi, a.jjg, a.hasilkerja, a.jhk, a.umr, 
        a.tarif_premi as insentif,
        a.insentif as totalpremi,
        COALESCE(c.rupiah, 0) AS totalInsentif
        FROM kebun_kehadiran a
        JOIN kebun_prestasi b ON a.notransaksi = b.notransaksi
        LEFT JOIN kebun_5psatuan c ON b.kodekegiatan = c.kodekegiatan
        WHERE
        a.".$where."
";
// $query2 = mysql_query($query3);
$data = fetchData($query);
// print_r($data);
// die();
$nikList = '';
