...
switch ($method) {
	case 'insert_forward_pp':
		...
		$PP_MAX_APPROVAL=$res['jumlahpemberipersetujuan'];
		if ($res['close'] == $PP_MAX_APPROVAL) {
			...
			if ($index!=$PP_MAX_APPROVAL) {
				...
				$strx = "update $dbname.log_prapoht set " .
					"hasilpersetujuan$PP_MAX_APPROVAL='1'," .
					"komentar$PP_MAX_APPROVAL='" . $comment . "'," .
					"close='" . $PP_MAX_APPROVAL . "',tglp5='" . $tglSkrng . "' where nopp='" . $nopp . "'";
				if ($res = mysql_query($strx)) {
					mailCoy($user_id);
				} else {
	...
	case 'insert_close_pp':
		...
		$res = mysql_fetch_assoc($query);
		$index=$res['jumlahpemberipersetujuan'];
//		$res = mysql_fetch_assoc($query);
		// if (($res['persetujuan5'] != '') || ($res['persetujuan4'] != '') || ($res['persetujuan3'] != '') ) { //&& ($user_id == $res['persetujuan5'])) {
		if ($res["persetujuan$index"] != '' ) { //&& ($user_id == $res['persetujuan5'])) {
			// $index=5;
			// if ($res['persetujuan4'] != '') $index=4;
			// if ($res['persetujuan3'] != '') $index=3;
			$sql2 = "update $dbname.log_prapoht set ".
				"close=".$index.",".
				"komentar$index='" . $comment . "',".
				"hasilpersetujuan$index=1,".
				"tglp$index='" . $tglSkrng . "' ".
				"where nopp='" . $nopp . "'"; 
...
//			if (($res['close'] == 1) && ($res['dibuat'] != $user_id)) {
			//mendapatkan dia persetujuan keberapa			
			$c = 0;
			$str1 = "select* from $dbname.log_prapoht where nopp='" . $nopp . "'";
			$res1 = mysql_query($str1);
			while ($bar1 = mysql_fetch_object($res1)) {
				$sawal = $bar1->sawal;
				$sawalrp = $bar1->sawalrp;
				if( $bar1->persetujuan1 == $user_id ){
					$c = 1;
				}
				if( $bar1->persetujuan2 == $user_id ){
					$c = 2;
				}
				if( $bar1->persetujuan3 == $user_id ){
					$c = 3;
				}
				if( $bar1->persetujuan4 == $user_id ){
					$c = 4;
				}
				if( $bar1->persetujuan5 == $user_id ){
					$c = 5;
				}
			}
...
//					"status='3',ditolakoleh='$user_id',alasanstatus='$alsnDtolak' where " . $where;
				//FA-20190925 - jika tolak beberapa maka status PR detailnya jangan diberi 3, supaya bisa muncul di verifikasi
				$sql2 = "update $dbname.log_prapodt set ".
					"status='0',ditolakoleh='$user_id',alasanstatus='$alsnDtolak' where " . $where;
				if ($query2 = mysql_query($sql2)) {
					echo '';
				}
				else {
...
LEFT OUTER JOIN datakaryawan d5 ON d5.karyawanid=l.persetujuan5 ";
		if ($_SESSION['empl']['tipeinduk'] == 'HOLDING') {
			...
			//$str .=	" ORDER BY tanggal desc "; //hasilpersetujuan1,hasilpersetujuan2,hasilpersetujuan3,hasilpersetujuan4,hasilpersetujuan5 ASC ".
			$str .=	" ORDER BY tanggal desc,nopp desc "; //hasilpersetujuan1,hasilpersetujuan2,hasilpersetujuan3,hasilpersetujuan4,hasilpersetujuan5 ASC ".
		...
		if ($res = mysql_query($str)) {
			while ($bar = mysql_fetch_assoc($res)) {
				...
				if ($bar['close'] == $PP_MAX_APPROVAL) {
					// if (($bar['close'] == 5) || ($bar['close'] == 4) || ($bar['close'] == 3)) {
					$accept = 0;
					$i = 1;
