...
if( isset($_POST['btnSubmit']) ){
	...
	{	
		...
		$qry="insert into ".$dbname.".tmp_wtpexternalht values('".$nama_file."','".$kdOrg."','".$date."'); ";
		...
		{ 
			...
			$als_start= $data->val($i, 3);
			$als_stop=$data->val($i, 4);			
			$sisa_als=preg_replace("/[^0-9.]/", "", $data->val($i, 5));
			$als_kg= preg_replace("/[^0-9.]/", "", $data->val($i, 6));
			$als_ppm=preg_replace("/[^0-9.]/", "",$data->val($i, 7));
			$sa_start= $data->val($i, 8);
			$sa_stop=$data->val($i, 9);
			$sisa_sa=preg_replace("/[^0-9.]/", "", $data->val($i, 10));
			$sa_kg=preg_replace("/[^0-9.]/", "", $data->val($i, 11));
			$sa_ppm=preg_replace("/[^0-9.]/", "", $data->val($i, 12));
			$mf_start= $data->val($i, 13);
			$mf_stop= $data->val($i, 14);
			$sisa_mf=preg_replace("/[^0-9.]/", "", $data->val($i, 15));
			$mf_kg=preg_replace("/[^0-9.]/", "", $data->val($i, 16));
			$mf_ppm=preg_replace("/[^0-9.]/", "", $data->val($i, 17));
			$wbs1= $data->val($i, 18);
			$wbs2= $data->val($i, 19);
			$wbs1_2= $data->val($i, 20);
			$wbs2_2= $data->val($i, 21);
			$rwp1_start=preg_replace("/[^0-9.]/", "", $data->val($i, 22));
			$rwp1_stop=preg_replace("/[^0-9.]/", "", $data->val($i, 23));
			$rwp2_start=preg_replace("/[^0-9.]/", "", $data->val($i, 24));
			$rwp2_stop=preg_replace("/[^0-9.]/", "", $data->val($i, 25));
			$fmw_awal=preg_replace("/[^0-9.]/", "", $data->val($i, 26));
			$fmw_akhir=preg_replace("/[^0-9.]/", "", $data->val($i, 27));
			$fmw_prosesawal=preg_replace("/[^0-9.]/", "", $data->val($i, 28));
			$fmw_prosesakhir=preg_replace("/[^0-9.]/", "", $data->val($i, 29));
			$fmd_awal=preg_replace("/[^0-9.]/", "", $data->val($i, 30));
			$fmd_akhir=preg_replace("/[^0-9.]/", "", $data->val($i, 31));
			$keterangan=$data->val($i, 32);
			...
			// menyimpan data xls ke db tmp_absensidt
			 $qry1="insert into ".$dbname.".tmp_wtpexternaldt values('".$nama_file."','".$tgl."','".$shift_nik."','".$als_start."','".$als_stop."','".$sisa_als."','".$als_kg."','".$als_ppm."','".$sa_start."','".$sa_stop."','".$sisa_sa."','".$sa_kg."','".$sa_ppm."','".$mf_start."','".$mf_stop."','".$sisa_mf."','".$mf_kg."','".$mf_ppm."','".$wbs1."','".$wbs2."','".$wbs1_2."','".$wbs2_2."','".$rwp1_start."','".$rwp1_stop."','".$rwp2_start."','".$rwp2_stop."','".$fmw_awal."','".$fmw_akhir."','".$fmw_prosesawal."','".$fmw_prosesakhir."','".$fmd_awal."','".$fmd_akhir."','".$keterangan."',current_timestamp); ";				
		...
		*/
		...
		// */
		// $totx= $res1['totx'];
		$squery="SELECT * FROM tmp_wtpexternaldt  where namafile='".$nama_file."' ";
...
if( $_POST['btnSubmitData']!=""){
	...
	if( $total_data > 0){
		...
		// get data karyawan
		$squery="SELECT a.*,b.kodeorganisasi as kodeorg FROM tmp_wtpexternaldt a,tmp_wtpexternalht b  where a.namafile=b.namafile and a.namafile='".$namafile."' ";
		// echo "<pre>";
		// print_r($squery);
		// die();
		$hasil = mysql_query($squery);
		...
		$tgl= $res['tgl'];
			...
			$queryCekAbsensiDT = "SELECT * FROM pabrik_wtpexternal WHERE kodeorg = '".$kodeorg."' and tgl = '".$tgl."' and shift_nik = '".$shift_nik."'  ";
			...
			if($jumlahCekAbsensiDT > 0){
				$sDetIns = "UPDATE ".$dbname.".pabrik_wtpexternal 
							SET
								als_start = '".$als_start."',
								als_stop='".$als_stop."',
								sisa_als='".$sisa_als."',
								...
								wbs1_2='".$wbs1_2."',
								wbs2_2='".$wbs2_2."',
								rwp1_start='".$rwp1_start."',
								rwp1_stop='".$rwp1_start."',								
								rwp2_start='".$rwp2_start."',
								rwp2_stop='".$rwp2_start."',
								fmw_awal='".$fmw_akhir."',
								fmw_akhir='".$fmw_awal."',
								fmw_prosesawal='".$fmw_prosesawal."',
								fmw_prosesakhir='".$fmw_prosesakhir."',								
								fmd_awal='".$fmd_awal."',
								fmd_akhir='".$fmd_akhir."',
				 ...
				 $sDetIns="insert into ".$dbname.".pabrik_wtpexternal values('".$namafile."','".$tgl."','".$shift_nik."','".$als_start."','".$als_stop."','".$sisa_als."','".$als_kg."','".$als_ppm."','".$sa_start."','".$sa_stop."','".$sisa_sa."','".$sa_kg."','".$sa_ppm."','".$mf_start."','".$mf_stop."','".$sisa_mf."','".$mf_kg."','".$mf_ppm."','".$wbs1."','".$wbs2."','".$wbs1_2."','".$wbs2_2."','".$rwp1_start."','".$rwp1_stop."','".$rwp2_start."','".$rwp2_stop."','".$fmw_awal."','".$fmw_akhir."','".$fmw_prosesawal."','".$fmw_prosesakhir."','".$fmd_awal."','".$fmd_akhir."','".$keterangan."','".$kdOrg."',current_timestamp)"; 
			}
		...
		$emptyQueryTable2 = "TRUNCATE TABLE tmp_wtpexternaldt";
...
OPEN_BOX('', 'Pabrik Logsheet WTP External Upload</b>');
