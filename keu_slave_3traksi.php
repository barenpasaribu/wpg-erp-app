...
$str = 'select * from ' . $dbname . ".flag_alokasi where kodeorg='" .$param['kodeorg']. "' and periode='" . $param['periode'] . "' AND tipe='TRAKSI' ";
$res = mysql_query($str);

if (mysql_num_rows($res) > 0) {
		exit('Error: Alokasi TRAKSI Sudah Dilakukan');

}


$str1 = 'select * from '.$dbname.".vhc_runht where posting=0 and kodeorg like '".$_SESSION['empl']['lokasitugas']."%' and kodevhc in(select kodevhc from ".$dbname.".vhc_5master where kodetraksi like '".$_SESSION['empl']['lokasitugas']."%') and tanggal>='".$tgmulai."' and tanggal<='".$tgsampai."'";
...
$total_biaya = 0;
$total = 0.01;
$str1= "select kodevhc, kodeorg, noakun, (sum(debet)-sum(kredit)) as total from keu_jurnaldt_vw 
			where kodevhc != '' and noakun like '4%' and periode = '".$param['periode']."' AND kodevhc IN (SELECT distinct(kodevhc) FROM vhc_5master  WHERE kodeorg ='".$kodeorg."') AND left(kodeorg,3) =left('".$kodeorg."',3)
			group by kodevhc";
...
//	$kodeorg= $rows1['kodeorg'];
	...
	echo "<tr class=rowcontent id='row".$no."'><td>".$no."
			...
			<td id='jumlah".$no."' align=right>".$total."</td><td id='jenis".$no."'>ALKJAM</td>";

//ambil total jam kegiatan traksi berdasarkan kode vhcnya
	$jamunit= "select sum(c.jumlah) as jumkerja from 
				vhc_runht b inner join vhc_rundt c on b.notransaksi = c.notransaksi
				where b.posting = 1
...
// tarik data di transaksi kendaraan, dapatkan total jumlah kerjanya (KM/HM)
	...
	$str2= "select namakegiatan, noakun, b.kodevhc, c.alokasibiaya, sum(c.jumlah) as jumkerja from 
				...
				and b.notransaksi not in (select distinct nojurnal from keu_jurnalht)
				and b.tanggal >= '".$tgmulai."' AND b.tanggal<='".$tgsampai."' 
				group by b.kodevhc, c.alokasibiaya, jenispekerjaan";
	$res2 = mysql_query($str2);

	echo "<td>";
	echo "<table><tr class=rowheader><td width='75'> unit </td> <td width='75'> noakun </td>  <td width='150'> nama kegiatan </td><td width='100'> jumlah jam </td> <td width='120'> biaya / unit </td>";
	while ($rows2 = mysql_fetch_array($res2)) {
		$uangkerja_satuan= $total / $jamperunit * $rows2['jumkerja']; 

		echo "<tr class=rowcontent>
			<td id='alokasibiaya".$no."'>".$rows2['alokasibiaya']."</td>
			<td id='noakun".$no."'>".$rows2['noakun']."</td>
			<td id='namakegiatan".$no."'>".$rows2['namakegiatan']."</td>
