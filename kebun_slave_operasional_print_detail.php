...
$optSatKegiatan = makeOption($dbname, 'setup_kegiatan', 'kodekegiatan,satuan', '', 11);
...
switch ($proses) {
    case 'pdf':
        ...
        $optBlokLama = makeOption($dbname, 'setup_blok', 'kodeorg,bloklama', "kodeorg='".$rPres['kodeorg']."'", 11);
        $pdf->SetFont('Arial', '', 7);
        $pdf->SetFillColor(255, 255, 255);
        ...
        $pdf->Cell(15 / 100 * $width, $height, $_SESSION['lang']['totalpremi'], 1, 1, 'C', 1);
        $pdf->SetFont('Arial', '', 7);
        $pdf->SetFillColor(255, 255, 255);
        $no = 0;
        $totHk = 0;
        ...
        while ($rKhdrn = mysql_fetch_assoc($qKhdrn)) {
            ...
            $totpremi=$rKhdrn['hasilkerja']*$rKhdrn['insentif'];
            $pdf->Cell(15 / 100 * $width, $height, number_format($totpremi, 0), 1, 1, 'R', 1);
        ...
        $pdf->Cell(15 / 100 * $width, $height, number_format($totalPremi, 0), 1, 1, 'R', 1);
        $pdf->SetFillColor(255, 255, 255);
        $pdf->Ln(30);
        $pdf->SetFont('Arial', 'B', 9);
        ...
        $pdf->Cell(17 / 100 * $width, $height, $_SESSION['lang']['kodebarang'], 1, 0, 'C', 1);
        ...
        while ($rMat = mysql_fetch_assoc($qMat)) {
            ...
            $pdf->Cell(17 / 100 * $width, $height, $optNamaBrg[$rMat['kodebarang']], 1, 0, 'L', 1);
        ...
        $pdf->SetFillColor(255, 255, 255);
        $pdf->SetFont('Arial', 'B', 8);
        $sAsis = 'select distinct nikmandor,nikmandor1,nikasisten,keranimuat,tanggal,kodeorg,updateby from '.$dbname.".kebun_aktifitas where notransaksi='".$param['notransaksi']."'";
        $qAsis = mysql_query($sAsis) ;
        $rAsis = mysql_fetch_assoc($qAsis);
        $pdf->ln(35);
        $pdf->Cell(100 / 100 * $width, $height, $rAsis['kodeorg'].','.tanggalnormal($rAsis['tanggal']), 0, 1, 'R', 0);
        $pdf->ln(35);
        ...
        $pdf->Cell(33 / 100 * $width, $height, $_SESSION['lang']['dibuatoleh'], 0, 1, 'C', 0);
        $pdf->ln(65);
        $pdf->SetFont('Arial', 'U', 8);
        $pdf->Cell(33 / 100 * $width, $height, $RnamaKary[$rAsis['nikasisten']], 0, 0, 'C', 0);
        $pdf->Cell(33 / 100 * $width, $height, $RnamaKary[$rAsis['nikmandor']], 0, 0, 'C', 0);
        $pdf->Cell(33 / 100 * $width, $height, $RnamaKary[$rAsis['updateby']], 0, 1, 'C', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(33 / 100 * $width, $height, $_SESSION['lang']['asisten'], 0, 0, 'C', 0);
        $pdf->Cell(33 / 100 * $width, $height, $_SESSION['lang']['nikmandor'], 0, 0, 'C', 0);
        $pdf->Cell(33 / 100 * $width, $height, $_SESSION['lang']['kerani'], 0, 1, 'C', 0);
        $pdf->Output();
    ...
    case 'html':
        ...
        $tab .= '<td>'.$_SESSION['lang']['umr'].'</td>';
        $tab .= '<td>'.$_SESSION['lang']['upahpremi'].'</td>';
        $tab .= '<td>'.$_SESSION['lang']['totalpremi'].'</td>';
        $tab .= '</tr></thead><tbody>';
        while ($rKhdrn = mysql_fetch_assoc($qKhdrn)) {
            $tab .= '<tr class=rowcontent>';
            $tab .= '<td>'.$optNamaKary[$rKhdrn['nik']].'</td>';
            $tab .= '<td>'.$rKhdrn['absensi'].'</td>';
            $tab .= '<td  align=right>'.number_format($rKhdrn['jjg']).'</td>';
        ...
        $tab .= '<td  align=right>'.number_format($totalPremi, 0).'</td>';
        $tab .= '</tr>';
        $tab .= '</table><br />';
        $sMat = 'select distinct * from '.$dbname.".kebun_pakaimaterial where notransaksi='".$param['notransaksi']."'";
        $qMat = mysql_query($sMat) ;
