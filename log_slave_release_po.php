...
switch ($method) {
case 'approve_po':
	$sql = "select karyawanid from setup_approval where applikasi='POAPP1' and kodeunit= '".$_SESSION['empl']['lokasitugas']."'";
	$qstr = mysql_query($sql);
	$rstr = mysql_fetch_assoc($qstr);
	if ($rstr['karyawanid'] != $user_id){
		echo 'Warning: Anda tidak berhak melakukan Approval 1, hubungi Admin';
		exit();
	}
	...
	if (($res['persetujuan1'] != '') || ($res['persetujuan2'] != '0000000000')) {
		...
		if (((int)$res['stat_release'] == 0) && ($res['useridreleasae'] == '0000000000')) {
			$unopo = "	update $dbname.log_poht 
						set 
							useridapprovepo='" . $rlse_user_id . "', 
							tglapprovepo='" . $this_date . "',
							wktapprovepo=current_time 
						where 
							nopo='" . $nopo . "' ";
...
case 'release_po':
	...
	// FA 20200210
	$sql = "select karyawanid from setup_approval where applikasi='POAPP2' and kodeunit= '".$_SESSION['empl']['lokasitugas']."'";
	$qstr = mysql_query($sql);
	$rstr = mysql_fetch_assoc($qstr);
	if ($rstr['karyawanid'] != $user_id){
		echo 'Warning: Anda tidak berhak melakukan Release PO (Approval 2), hubungi Admin';
		exit();
	}
...
case 'un_release_po':
	...
	if (($res['stat_release'] == 1) && ($res['useridreleasae'] == $rlse_user_id) ) {
		...
		echo 'Warning: Anda tidak berhak melakukan Unrelease PO No. ' . $nopo;
		exit();
	}
...
case 'list_new_data_release_po':
	...
	// $str = 'SELECT * FROM ' . $dbname . '.log_poht where kodeorg like \''.$_SESSION['empl']['kodeorganisasi'].'%\' and statuspo>1 ' . $add . '   GROUP BY tanggal DESC  limit ' . $offset . ',' . $limit . ' ';
	$str = 'SELECT * FROM ' . $dbname . '.log_poht where kodeorg like \''.$_SESSION['empl']['kodeorganisasi'].'%\' and statuspo>1 ' . $add . '   ORDER BY tanggal DESC, nopo DESC  limit ' . $offset . ',' . $limit . ' ';
	// print_r($str);
	// die();
			...
			if (1 < $bar['statuspo']) {
				...
				*/
				if( $need_approval == "Y"){
					...
					$qry="select notransaksi from log_transaksiht where nopo='".$bar['nopo']."'";
					$sq=mysql_query($qry);
					$has=mysql_num_rows($sq);
					if($has==0){
					$disbled2 = '<td><button class=mybutton onclick="un_release_po(\'' . $bar['nopo'] . '\',\'' . $this_date . '\') " >' . $_SESSION['lang']['un_release_po'] . ' </button></td>';
					}else{
					$disbled2 = '<td></td>';
...
case 'cari_rpo':
	...
	$strx = 'select * from ' . $dbname . '.log_poht where statuspo>1 ' . $add . ' and right(nopo,3)=\'' . $_SESSION['empl']['kodeorganisasi'] . '\' ' . $where . 'order by nopo desc';
	$sql2 = 'select count(*) as jmlhrow from ' . $dbname . '.log_poht where statuspo>1 ' . $add . '  and right(nopo,3)=\'' . $_SESSION['empl']['kodeorganisasi'] . '\' ' . $where . 'order by nopo desc';
