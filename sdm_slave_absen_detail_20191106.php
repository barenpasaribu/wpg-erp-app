...
switch ($proses) {
    case 'createTable':
        ...
        $where = ' 21=21'; //FA 20191023
        if (4 < strlen($idAbn[0])) {
            $where .= " and subbagian='".$idAbn[0]."'  and (tanggalkeluar>".$tgl.' or tanggalkeluar is NULL)';
            $ha = 'select karyawanid,nik,namakaryawan,subbagian from '.$dbname.'.datakaryawan where '.$where.' and karyawanid is not null ';
        } else {
            $where .= " and lokasitugas='".$idAbn[0]."' and (subbagian IS NULL or subbagian='0' or subbagian='')and (tanggalkeluar>".$tgl.' or tanggalkeluar is NULL)';
            $ha = 'select karyawanid,nik,namakaryawan,subbagian from '.$dbname.'.datakaryawan where '.$where.' and karyawanid is not null ';
    ...
    case 'getKary':
        ...
        $tab .= '</tr></thead><tbody>';
        if ('6' == strlen($_POST['unit'])) {
            $wher = "subbagian='".$_POST['unit']."'  and (tanggalkeluar>".tanggaldgnbar($_POST['tanggalcr']).' or tanggalkeluar is NULL)  and tipekaryawan!=5';
            ...
            $wher = "lokasitugas='".$_POST['unit']."' and (subbagian is null or subbagian='') and tipekaryawan!=5   and (tanggalkeluar>".tanggaldgnbar($_POST['tanggalcr']).' or tanggalkeluar is NULL)';
            $sDt = 'select karyawanid,nik,namakaryawan,subbagian from '.$dbname.".datakaryawan\r\n                             where ".$wher." and karyawanid is not null and (namakaryawan like '%".$_POST['nmkary']."%' or nik like '%".$_POST['nmkary']."%')";
