<?php
	...
	*/
    ...
    left join ".$dbname.".sdm_5jabatan b on a.kodejabatan=b.kodejabatan 
    left join ".$dbname.".sdm_5departemen c on a.bagian=c.kode 
    where a.lokasitugas = '".$lokasitugas."' and ((a.tanggalkeluar >= '".$tangsys1."' and a.tanggalkeluar <= '".$tangsys2."') or a.tanggalkeluar is NULL) and isduplicate=0 order by namakaryawan asc";
    ...
    and 
    '".$tangsys2."' 
    and a.kodeorg like '".$lokasitugas."' ORDER BY tanggal DESC";
    $res = mysql_query($str);
    ...
    function dates_inbetween($date1, $date2)
    {
        ...
        for ($x = 1; $x < $days_diff; ++$x) {
			...
			//echo ' date after adding 1 day: ' . $stop_date.'<br/>';
            $dates_array[] = $stop_date;
			//echo date('Y-m-d', $date1 + $day * $x).'<br/>';
			//echo date('Y-m-d', $date1).'+'.($day * $x).'<br/>';
        }
/* 		for ($x = 1; $x < $days_diff; ++$x) {
