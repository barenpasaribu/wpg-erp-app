...
if ('' !== $namabarang) {
    ...
    if ('' === $errorDBX) {
        ...
        while ($bar = mysql_fetch_object($resp)) {
            ...
            ++$nomor[$bar->kodegudang];
            $num = $nomor[$bar->kodegudang].'-GI-'.$bar->kodegudang;
            $dataMat['header'][] = ['tipetransaksi' => '5', 'notransaksi' => $num, 'tanggal' => $dataH[0]['tanggal'], 'kodept' => $_SESSION['empl']['kodeorganisasi'], 'untukpt' => $_SESSION['empl']['kodeorganisasi'], 'nopo' => '', 'nosj' => '', 'keterangan' => 'Material BKM ', 'statusjurnal' => '1', 'kodegudang' => $bar->kodegudang, 'user' => $_SESSION['standard']['userid'], 'namapenerima' => '0', 'mengetahui' => $_SESSION['standard']['userid'], 'idsupplier' => '', 'nofaktur' => '', 'post' => '1', 'postedby' => $_SESSION['standard']['userid'], 'untukunit' => $_SESSION['empl']['lokasitugas'], 'notransaksireferensi' => $param['notransaksi'], 'gudangx' => '', 'lastupdate' => date('Y-m-d H:i:s')];
            $dataMat['detail'][] = ['notransaksi' => $num, 'kodebarang' => $bar->kodebarang, 'satuan' => $bar->satuan, 'jumlah' => $bar->kwantitas, 'jumlahlalu' => $saldo[$bar->kodegudang][$bar->kodebarang], 'hargasatuan' => '0', 'kodeblok' => $kodeblok, 'waktutransaksi' => date('Y-m-d H:i:s'), 'updateby' => $_SESSION['standard']['userid'], 'kodekegiatan' => $kodekegiatan, 'kodemesin' => '', 'statussaldo' => 1, 'hargarata' => $harga[$bar->kodegudang][$bar->kodebarang]];
        }
        $str = 'select a.*,b.namabarang,b.satuan from '.$dbname.".kebun_pakaimaterial a\r\n          left join ".$dbname.".log_5masterbarang b on a.kodebarang=b.kodebarang\r\n          where a.notransaksi='".$param['notransaksi']."' and a.kodegudang!=''";
