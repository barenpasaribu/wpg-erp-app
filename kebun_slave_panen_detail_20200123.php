			...
			if ($optDenda[$paramBlok] == 1) {
				...
				if (($xhasilKg > $basis || $isDobel == 1) && $isgabunghasil !=2){
					if ($optRp[$paramBlok] !=0) {
						...
						for ($awl = 0; $awl <= $JmlhRow; $awl++) {
							...
							if ($lbhbasis > $batasatas || (($totlbhbasis + $lbhbasis) > $batasatas && $awl == 0)){
								...
								$premi = $premi + ($lbhbasis * $rupiah);
									echo "warning: lbh".$lbhbasis."/rup".$rupiah;
									exit();
							...
							if ($metode==1 && $lbhbasis<$batasatas) {
						...
						// Khusus CDS - setelah blok ke-1, semua hasil digabungkan utk dpt rate premi panen (FA 20200115)
						if ($isgabunghasil== 3) {
							if ($totlbhbasis == 0){
								$str = "select hasilkg,rupiah from kebun_5premipanen where 
									hasilkg+hasilpanen < ".$tothasilkerja_ori." and hasilkg+lebihbasiskg > ".$tothasilkerja_ori
									." and bulanawal<=".$month." and bulanakhir>=".$month
									." and kodeorg='".$paramBlok."' limit 1";
							} else {
								$xtotlbhbasis= $totlbhbasis + round($hasilKg);
								$str = "select rupiah from kebun_5premipanen where 
									hasilpanen < ".$xtotlbhbasis." and lebihbasiskg > ".$xtotlbhbasis
									." and bulanawal<=".$month." and bulanakhir>=".$month
									." and kodeorg='".$paramBlok."' limit 1";
							}
							//echo "warning: ".$str;
							...
							$rows = mysql_fetch_assoc($qstr);
							if ($totlbhbasis == 0){
//								echo "warning: hasilkg= ".$rows['hasilkg']." / tothasilkerja= ".$tothasilkerja_ori;
//								exit();
								$jumlahlbhbasis= round($tothasilkerja_ori) - $rows['hasilkg'];
								$premi = $rows['rupiah'] * $jumlahlbhbasis;
							} else {
								$premi = $rows['rupiah'] * round($hasilKg); 
							}
							
							// FA 20200107 - premi lebih basis blok sebelumnya ditambah preminya,
							// jika ada selisih dgn rate yg baru
							//$selisih = ($totlbhbasis * $rupiah) - $totpremilbhbasis;
							$selisih = ($totlbhbasis * $rupiah) - $totpremilbhbasis;
							if ($isgabunghasil == 3 && $selisih > 0) {
								//$totlbhbasis -> Kg-nya, $totpremilbhbasis -> Rupiah-nya
								$premi = $premi + $selisih;	
							}	
						}
						$insentif = $premi;
