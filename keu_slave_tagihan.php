...
switch ($proses) {
	case 'preview':
        ...
        $sCob = 'select * from '.$dbname.".pabrik_timbangan where  notransaksi in(".$notransaksi.") and trpcode='".$kodesupplier."' and IsPosting=1 and millcode like '%".$_SESSION['empl']['induklokasitugas']."%' order by notransaksi ASC";
      	// echo $sCob;
        ...
        if ($row > 0) {
            ...
            while ($hsl = mysql_fetch_assoc($res)) {
                ...
                $nopo = $hsl['nospb'];
              	$sSPBantrian = 'select * from '.$dbname.".pabrik_antriantb where  nospb = '".$nopo."' and kodeorg = '".$_SESSION['org']['kodeorganisasi']."' ";
...
//	echo "warning".$where;
	...
	$cols = 'noinvoice,noinvoicesupplier,kodeorg,tanggal,updateby,nopo,(select namasupplier from log_5supplier a where supplierid=kodesupplier),keterangan,(nilaiinvoice+nilaippn-perhitunganpph) as nilaiinvoice,postingby,tanggalposting,posting';
	$order = 'tanggal DESC, noinvoice, posting';
...
case 'getPo':
	...
	switch ($jenisInvoice) {
		...
		case 'kontrak':
			...
			if ($_SESSION['empl']['tipelokasitugas'] == 'HOLDING') {
                ...
                $sPo = 'select distinct notransaksi as nopo,nilaikontrak as nilaipo,koderekanan as kodesupplier from ' . $dbname . '.log_spkht where kodeorg in (select distinct kodeorganisasi from ' . $dbname . '.organisasi where induk=\'' . $_SESSION['org']['kodeorganisasi'] . '\')  ' . $where . ' and (notransaksi not in(select nopo from keu_tagihanht) OR notransaksi IN (SELECT notransaksi FROM (SELECT notransaksi, SUM(jumlahrealisasi) as jumlahrealisasi FROM log_baspk GROUP BY notransaksi) a , (SELECT nodok, SUM(jumlah) as jumlahkasbank  FROM keu_kasbankdt GROUP BY nodok) b 
                    WHERE a.notransaksi=b.nodok AND jumlahrealisasi>jumlahkasbank )) order by tanggal desc';
		...
		case 'sj':
			...
			$sPo = 'select distinct pabrik_timbangan.nospb as nopo, pabrik_timbangan.notransaksi as notransaksi, pabrik_timbangan.trpcode as kodesupplier, log_5supplier.namasupplier as namasupplier, date(pabrik_timbangan.tanggal) as tgl from ' . $dbname . '. pabrik_timbangan, log_5supplier ' . "\r\n\t\t\t\t\t" . '   where  millcode like \''.$_SESSION['empl']['induklokasitugas'].'%\'  and pabrik_timbangan.trpcode=log_5supplier.supplierid and substr(pabrik_timbangan.notransaksi,1,1)=\'M\' and pabrik_timbangan.notransaksi not in(select notiket from keu_tagihandt a inner join keu_tagihanht b on a.noinvoice=b.noinvoice where kodeorg like \''.$_SESSION['empl']['induklokasitugas'].'%\')  ' . $where . '  order by date(pabrik_timbangan.tanggal),pabrik_timbangan.nospb desc ';
	...
	while ($rPo = mysql_fetch_assoc($qPo)) {
		...
		//	saveLog($sCek);
			...
			if ($jenisInvoice == 'sj') {
			$no += 1;
			$dat .= "<tr class=\'rowcontent\'><td> <input type=checkbox   id=chk".$no."  class=myinputtext onchange=pilihchk('".$rPo['notransaksi']."','".$no."','".$rPo['kodesupplier']."')></td><td>". $no . "</td>";
			$dat .= '<td>' . $rPo['nopo'] . '</td>';
			$dat .= '<td>' . $optNmsupp[$rPo['kodesupplier']] . '</td>';
			$dat .= '<td>' . $rPo['tgl'] . '</td>';
			$dat .= '<td align=right>' . $rPo['nilaipo'] . '</td>';						
