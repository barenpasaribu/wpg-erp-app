...
$where = ' tanggal>='.$period['start']." and substr(nojurnal,10,4)='".$_SESSION['empl']['lokasitugas']."' and kodejurnal='M'".' and revisi=0';
...
$header = [$_SESSION['lang']['kodeabs'], $_SESSION['lang']['nojurnal'], $_SESSION['lang']['tanggal'], $_SESSION['lang']['noreferensi'], $_SESSION['lang']['matauang'], $_SESSION['lang']['debet'], $_SESSION['lang']['kredit']];
...
foreach ($resTab as $key => $row) {
    ...
    foreach ($row as $col => $dat) {
        ...
        if ('totaldebet' == $col) {
            $table .= "<td id='".$col.'_'.$key."' onclick='passEditHeader(".$key.")' align=right ".$dbgr." title='".$_SESSION['lang']['selisih'].' '.(int) ($rCekData['selisih'])."'>".number_format($dtplus, 2).'</td>';
        } else {
            if ('totalkredit' == $col) {
                $table .= "<td id='".$col.'_'.$key."' onclick='passEditHeader(".$key.")' align=right ".$dbgr." title='".$_SESSION['lang']['selisih'].' '.(int) ($rCekData['selisih'])."'>".number_format($dtmin, 2).'</td>';
            } else {
...
$els[] = [makeElement('tanggal', 'label', $_SESSION['lang']['tanggal']), makeElement('tanggal', 'text', '', ['style' => 'width:80px', 'readonly' => 'readonly', 'onkeypress' => 'return tanpa_kutip(event)', 'disabled' => 'disabled'])];
$els[] = [makeElement('noreferensi', 'label', $_SESSION['lang']['nodok']), makeElement('noreferensi', 'text', '', ['style' => 'width:80px', 'maxlength' => '20', 'onkeypress' => 'return tanpa_kutip(event)', 'disabled' => 'disabled'])];
$els[] = [makeElement('matauang', 'label', $_SESSION['lang']['matauang']), makeElement('matauang', 'select', '', ['style' => 'width:70px', 'disabled' => 'disabled'], $optCurr)];
