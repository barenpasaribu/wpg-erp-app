...
switch ($proses) {
    case 'list':
        ...
        if ('Bulanan' == $param['jnsGaji']) {
            ...
            $where1 = "lokasitugas='".$_SESSION['empl']['lokasitugas']."' and sistemgaji='".$param['jnsGaji']."' and (tanggalkeluar>'".$row['tanggalmulai']."' or tanggalkeluar is NULL or tanggalkeluar ='0000-00-00') and '".$param['agama'].'\' like concat("%",agama,"%")'; // alokasi=0 and 
            ...
            if ('Harian' == $param['jnsGaji']) {
                ...
                $where2 = "lokasitugas='".$_SESSION['empl']['lokasitugas']."' and sistemgaji='".$param['jnsGaji']."' and (tanggalkeluar>'".$row['tanggalmulai']."' or tanggalkeluar is NULL or tanggalkeluar ='0000-00-00') and '".$param['agama'].'\' like concat("%",agama,"%")'; // alokasi=0 and 
        ...
        if ('Bulanan' == $param['jnsGaji']) {
            foreach ($resBln as $key => $row) {
				...
				if( $param['jenis'] == $id_komponen_thr){
					$sGaji = 'select karyawanid,idkomponen,pengali,jumlah from '.$dbname.".sdm_gaji where karyawanid='".$row['karyawanid']."'\r\n                    and idkomponen in (select component from ".$dbname.".sdm_ho_thr_setup ) and kodeorg='".$_SESSION['empl']['lokasitugas']."' and periodegaji='".$param['periodegaji']."'";
				}else{
					$sGaji = 'select karyawanid,idkomponen,pengali,jumlah from '.$dbname.".sdm_gaji where karyawanid='".$row['karyawanid']."'\r\n                    and idkomponen in (select component from ".$dbname.".sdm_ho_bonus_setup ) and kodeorg='".$_SESSION['empl']['lokasitugas']."' and periodegaji='".$param['periodegaji']."'";
				}
                ...
                if ('' != $rGaji['jumlah'] && 0 != $rGaji['jumlah']) {
					...
					if( $param['jenis'] == $id_komponen_thr){
						$sGaji2 = 'select karyawanid,idkomponen,sum(jumlah) as jumlah from '.$dbname.".sdm_5gajipokok where karyawanid='".$row['karyawanid']."'\r\n                    and idkomponen in (select component from ".$dbname.".sdm_ho_thr_setup ) and tahun=".$param['tahun'];
					}else{
						$sGaji2 = 'select karyawanid,idkomponen,sum(jumlah) as jumlah from '.$dbname.".sdm_5gajipokok where karyawanid='".$row['karyawanid']."'\r\n                    and idkomponen in (select component from ".$dbname.".sdm_ho_bonus_setup ) and tahun=".$param['tahun'];
					}                    
        ...
        foreach ($resGj as $kary => $row) {
            ...
            if (isset($row[$param['jenis']])) {
                ...
                if ($msKerja < 3) {
					...
					$flag_disnaker_thr_baru = "Y";
					if( $flag_disnaker_thr_baru == "Y" ){
						$row[1]['pengali'] = 1;
					}else{
        ...
        foreach ($dataShow as $key => $row) {
            if ('' != $row['id']) {
                ...
                foreach ($row as $head => $cont) {
					if( $head == "gajipokok"){
						 $table .= "<td id='".$head.'_'.$listdt."' value='".number_format($data[$key][$head],0,".","")."'>";
					}else{
                    ...
                    if ('pengali' == $head) {
                        ...
                        if ('jumlah' == $head) {
                            $table .= makeElement($head.'_'.$listdt.'_text', 'textnum', number_format($cont,0,".",""), ['style' => 'width:70px']);
                        } else {
							if( $head == "masakerja"){
								if( floor($cont) > 0){
									$table .= floor($cont)." tahun";
								}else{
									$table .= floor($cont*12)." bulan";
								}
								
							}elseif( $head == "gajipokok" ){
								$table .= number_format($cont,0,".",",");
							}else{
								$table .= $cont;
							}                            
                        }
    ...
    case 'post':
		...
		$query1 = insertQuery($dbname, 'sdm_gaji', $data,array("kodeorg","periodegaji","karyawanid","idkomponen","jumlah","pengali"));
