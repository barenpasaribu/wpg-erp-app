...
while ($rvhc = mysql_fetch_assoc($qvhc)) {
    $optVhc .= "<option value='".$rvhc['kodevhc']."'>".$rvhc['kodevhc'].$rvhc['detailvhc']."</option>";
}
$svhc2 = 'select kodeorg from '.$dbname.'.vhc_5master where  left(kodeorg,3)=\''.$_SESSION['empl']['kodeorganisasi'].'\' group by kodeorg';
...
where left(kodeorganisasi,3)='".$_SESSION['empl']['kodeorganisasi']."' and (
s.alias like '%Kepala Mekanik%' or s.alias like '%Pelaksana Bengkel%' or s.alias like '%Mekanik%' or 
s.alias like '%Asisten Bengkel%' or s.alias like '%WELDER%' )  order by namakaryawan ";
$n = mysql_query($i);
