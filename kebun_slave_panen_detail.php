...
switch ($proses) {
                ...
                <fieldset>
                    ...
                    <table>
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <label for='penalti1'>Buah mentah dipanen</label>
                                </td>
                                <td>:</td>
                            ...
                            <tr>
                                <td>
                                    <label for='penalti2'>Brondolan tinggal di piringan</label>
                                </td>
                                <td>:</td>
                            ...
                            <tr>
                                <td>
                                    <label for='penalti3'>TBS panen tertinggal</label>
                                </td>
                                <td>:</td>
                            ...
                            <tr>
                                <td>
                                    <label for='penalti4'>Pelepah tidak disusun</label>
                                </td>
                                <td>:</td>
                            ...
                            <tr>
                                <td>
                                    <label for='penalti5'>Buah matang tidak dipanen</label>
                                </td>
                                <td>:</td>
                            ...
                            <tr>
                                <td>
                                    <label for='penalti6'>Pelepah gantung/sengkleh</label>
                                </td>
                                <td>:</td>
                            ...
                            <tr>
                                <td>
                                    <label for='penalti7'>Tangkai panjang</label>
                                </td>
                                <td>:</td>
                        ...
                        <table class='sortable' cellspacing='1' border='0' id='prestasiTable'>
                            <thead id='thead_ftPrestasi'>
                                ...
                                <td id='head_nik' align='center' style='width:300px'>NIK</td>
                                    ...
                                    <td id='head_penalti1' align='center' style='width:100px'>Buah mentah dipanen</td>
                                    <td id='head_penalti2' align='center' style='width:100px'>Brondolan tinggal di piringan</td>
                                    <td id='head_penalti3' align='center' style='width:100px'>TBS panen tertinggal</td>
                                    <td id='head_penalti4' align='center' style='width:100px'>Pelepah tidak disusun</td>
                                    <td id='head_penalti5' align='center' style='width:100px'>Buah matang tidak dipanen</td>
                                    <td id='head_penalti6' align='center' style='width:100px'>Pelepah gantung/sengkleh</td>
                                    <td id='head_penalti7' align='center' style='width:100px'>Tangkai panjang</td>
    ...
    case 'updTahunTanam':
        ...
        //        }
                ...
                $monthh = $tgl[1];
                $year = $tgl[2];
                         $tglpanen = $year.'-'.$monthh.'-'.$date;   
                $fieldno = $param['kodeorg'];   
                $karyawanid = $param['karyawanid']; 
                $sqlNik = 'select * from '.$dbname.".datakaryawan where karyawanid='".$karyawanid."'";  
                 $queryNik = mysql_query($sqlNik);  
                 $resNik = mysql_fetch_assoc($queryNik);    
                 $nik = $resNik['nik']; 
                 $sql = 'select * from '.$dbname.".setup_remotetimbangan where lokasi='Android'";   
                 $query = mysql_query($sql);    
                 $res = mysql_fetch_assoc($query);  
                  error_reporting(E_ALL ^ E_DEPRECATED);    
                 $corn = mysql_connect($res['ip'].':'.$res['port'], $res['username'], $res['password']);    
                    $dbnm = $res['dbname']; 
                     $sPanenJjg = 'select sum(TOTKELBUAHQTY) as JMLJJG,TRANSID from '.$dbnm.".transpanenheader where EMPCODE='".$nik."' and date(LASTUPDATED)='".$tglpanen."' AND FIELD_NO='".$fieldno."' and SATUANID='' group by SUBSTR(TRANSID,-1) DESC LIMIT 1 ";   
                    
                    
                    $qPanenJjg = mysql_query($sPanenJjg,$corn); 
                    $rPanenJjg = mysql_fetch_assoc($qPanenJjg); 
                    $rowJjg = mysql_num_rows($qPanenJjg);   
                    if($rowJjg != 0 ){  
                        $jjgjml = $rPanenJjg['JMLJJG']; 
                    }else{  
                        $jjgjml = '0';  
                    }   
                      $sPanenBrd = 'select sum(TOTKELBUAHQTY) as JMLBRD,TRANSID from '.$dbnm.".transpanenheader where EMPCODE='".$nik."' and date(LASTUPDATED)='".$tglpanen."' AND FIELD_NO='".$fieldno."' and SATUANID='KG' group by SUBSTR(TRANSID,-1) DESC LIMIT 1 ";    
                    $qPanenBrd = mysql_query($sPanenBrd,$corn); 
                    $rPanenBrd = mysql_fetch_assoc($qPanenBrd); 
                    $rowBrd = mysql_num_rows($qPanenBrd);   
                    if($rowJjg != 0 ){  
                        $brdjml = $rPanenBrd['JMLBRD']; 
                    }else{  
                        $brdjml = '0';  
                    }   
                       $sPanenMentah = 'select sum(KELBUAHQTY) as JMLMENTAH,a.TRANSID from '.$dbnm.".transpanenheader a,".$dbnm.".transpanendetail b where a.TRANSID=b.TRANSID and a.EMPCODE='".$nik."' and date(a.LASTUPDATED)='".$tglpanen."' AND a.FIELD_NO='".$fieldno."' and b.KELBUAHID='02' group by SUBSTR(a.TRANSID,-1) DESC LIMIT 1"; 
                    
                    $qPanenMentah = mysql_query($sPanenMentah,$corn);   
                    $rPanenMentah = mysql_fetch_assoc($qPanenMentah);   
                    $rowMentah = mysql_num_rows($qPanenMentah); 
                    if($rowMentah != 0 ){   
                        $mentahjml = $rPanenMentah['JMLMENTAH'];    
                    }else{  
                        $mentahjml = '0';   
                    }   
                        $sPanenTP = 'select sum(KELBUAHQTY) as JMLTP,a.TRANSID from '.$dbnm.".transpanenheader a,".$dbnm.".transpanendetail b where a.TRANSID=b.TRANSID and a.EMPCODE='".$nik."' and date(a.LASTUPDATED)='".$tglpanen."' AND a.FIELD_NO='".$fieldno."' and b.KELBUAHID='52' group by SUBSTR(a.TRANSID,-1) DESC LIMIT 1";    
                    $qPanenTP = mysql_query($sPanenTP,$corn);   
                    $rPanenTP = mysql_fetch_assoc($qPanenTP);   
                    $rowTP = mysql_num_rows($qPanenTP); 
                    if($rowTP != 0 ){   
                        $tpjml = $rPanenTP['JMLTP'];    
                    }else{  
                        $tpjml = '0';   
                    }   
                    
                      $sPanenJT = 'select sum(KELBUAHQTY) as JMLJT,a.TRANSID from '.$dbnm.".transpanenheader a,".$dbnm.".transpanendetail b where a.TRANSID=b.TRANSID and a.EMPCODE='".$nik."' and date(a.LASTUPDATED)='".$tglpanen."' AND a.FIELD_NO='".$fieldno."' and b.KELBUAHID='32' group by SUBSTR(a.TRANSID,-1) DESC LIMIT 1";  
                    $qPanenJT = mysql_query($sPanenJT,$corn);   
                    $rPanenJT = mysql_fetch_assoc($qPanenJT);   
                    $rowJT = mysql_num_rows($qPanenJT); 
                    if($rowJT != 0 ){   
                        $jtjml = $rPanenJT['JMLJT'];    
                    }else{  
                        $jtjml = '0';   
                    }   
                     $sPanenBTK = 'select sum(KELBUAHQTY) as JMLBTK,a.TRANSID from '.$dbnm.".transpanenheader a,".$dbnm.".transpanendetail b where a.TRANSID=b.TRANSID and a.EMPCODE='".$nik."' and date(a.LASTUPDATED)='".$tglpanen."' AND a.FIELD_NO='".$fieldno."' and b.KELBUAHID='35' group by SUBSTR(a.TRANSID,-1) DESC LIMIT 1"; 
                    $qPanenBTK = mysql_query($sPanenBTK,$corn); 
                    $rPanenBTK = mysql_fetch_assoc($qPanenBTK); 
                    $rowBTK = mysql_num_rows($qPanenBTK);   
                    if($rowBTK != 0  ){ 
                        $btkjml = $rPanenBTK['JMLBTK']; 
                    }else{  
                        $btkjml = '0';  
                    }   
                     $sPanenPT = 'select sum(KELBUAHQTY) as JMLPT,a.TRANSID from '.$dbnm.".transpanenheader a,".$dbnm.".transpanendetail b where a.TRANSID=b.TRANSID and a.EMPCODE='".$nik."' and date(a.LASTUPDATED)='".$tglpanen."' AND a.FIELD_NO='".$fieldno."' and b.KELBUAHID='15' group by SUBSTR(a.TRANSID,-1) DESC LIMIT 1";   
                    $qPanenPT = mysql_query($sPanenPT,$corn);   
                    $rPanenPT = mysql_fetch_assoc($qPanenPT);   
                    $rowJT = mysql_num_rows($qPanenPT); 
                    if($rowJT != 0 ){   
                        $ptjml = $rPanenPT['JMLPT'];    
                    }else{  
                        $ptjml = '0';   
                    }   
                  echo $btkjml; 
                echo $thntnm.'####'.number_format($rBjr['bjr'], 2).'####'.$basiskg.'####'.$pagi.'####'.$selisihjam.'####'.number_format($basiskg2, 2).'####'.$turunhujan.'####'.$jjgjml.'####'.$brdjml.'####'.$mentahjml.'####'.$tpjml.'####'.$jtjml.'####'.$btkjml.'####'.$ptjml;
	...
	//echo 'warning x1 tothasilkerja='.$tothasilkerja.' /hasilKg='.$hasilKg.' /xnorma='.$xnorma.' /isgabunghasil='.$isgabunghasil;
	//exit();
        } else {
	//echo 'warning x2 tothasilkerja='.$tothasilkerja.' /hasilKg='.$hasilKg.' /xnorma='.$xnorma.' /isgabunghasil='.$isgabunghasil;
	//exit();
		}
...
//          "and (hasilpanen between 0 and ".round($jumlahlbhbasis, 0, PHP_ROUND_HALF_UP).") ".
        ...
        //echo "warning: jumlbhbasis=".$jumlahlbhbasis." / hasilkg=".$hasilKg." / tothasilkerja=".$tothasilkerja." / xnorma=".$xnorma." /isgabunghasil=".$isgabunghasil." /isdobel=".$isDobel." / QUERY= ".$sPremi;
        //exit();
                ...
                if (($xhasilKg > $basis || $isDobel == 1) && $isgabunghasil !=2){
                    ...
                    //exit();
                        ...
                        for ($awl = 0; $awl <= $JmlhRow; $awl++) {
                            ...
                            if ($awl == 0 && $isDobel==1){
                                $str = $str." where hasilpanen <= ".($batasatas+1)." and lebihbasiskg>".$batasatas; 
								// FA 20200730 - ubah menjadi <= , dan menjadi $batasatas+1 
								// Kasus SPS, Dedi Purnomo tgl 25 Juni 2020. Lebih Basis nya sama dengan Batas Atas Basis 1
                            } else {
