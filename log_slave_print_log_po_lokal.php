<?php
	...
	//		$waktu_persetujuan1 = "";
			if( $bar->persetujuan1 != ""){
				$str2 = "select namakaryawan from ".$dbname.".datakaryawan where karyawanid like '%".$bar->persetujuan1."%'";
	...
	if ($rlopoht->nilaipo <= 0) {
		$gr_total = ($sb_tot - $nil_diskon) + $nppn - $npph + $rlopoht->misc + $rlopoht->ongkirimppn + $rlopoht->misc;
		// $updateGrandTotal = "update log_poht set nilaipo='".$gr_total."' where nopo='".$_GET['column']."'";
		// mysql_query($updateGrandTotal))
	}else{
		$gr_total = $rlopoht->nilaipo;
	}
	if (240 <= $akhirSubtot) {
	...
	$pdf->SetY($pdf->GetY());
	$pdf->SetX($pdf->GetX() + 134);
	$pdf->Cell(29, 5, 'PPh (10 %)', 0, 0, 'L', 1);
	$pdf->Cell(26, 5, number_format($rlopoht->pph, $desiomal, '.', ','), 0, 1, 'R', 1);
	$pdf->SetY($pdf->GetY());
	$pdf->SetX($pdf->GetX() + 134);
	$pdf->Cell(29, 5, 'Pembulatan', 0, 0, 'L', 1);
	$pdf->Cell(26, 5, number_format($rlopoht->misc, $desiomal, '.', ','), 0, 1, 'R', 1);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->SetY($pdf->GetY());
	$pdf->SetX($pdf->GetX() + 134);
