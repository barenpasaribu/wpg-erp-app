...
switch ($proses) {
	...
	$jjgDis="true"; //add by hadi 12/112019
        ...
        if ('false' == $jjgDis) {
            ...
            $x = 'select sum(a.totalkg)/sum(a.jjg) as bjr,tanggal FROM '.$dbname.'.kebun_spbdt a left join '.$dbname.".kebun_spbht b on a.nospb=b.nospb \r\n\t\t\t\twhere blok like '%".$kdAfd."%' and tanggal<='".$tanggal."' ";
        ...
        $totalpremi = $hasilkerja * $insentif;





        $res = ['hasilkerja' => number_format($hasilkerja, 2), 'jhk' => number_format($jhk, 2), 'umr' => number_format($umr, 2), 'insentif' => number_format($insentif, 2),'totalpremi' => number_format($totalpremi, 2)];
