...
switch ($_POST['aksi']) {
    ...
    case 'ambilPremiPanen':
        $str = 'select sum(a.upahpremi) as premi,a.nik  from '.$dbname.".kebun_prestasi a left join ".$dbname.".kebun_aktifitas b on a.notransaksi=b.notransaksi where a.kodeorg like '".$_SESSION['empl']['lokasitugas']."%' and b.tanggal=".tanggalsystem($_POST['tanggal'])." and b.nikmandor='".$_POST['nikmandor']."' group by a.nik";
        $res = mysql_query($str);
    ...
    case 'ambilList':
        ...
        if ('excel' == $_POST['proses']) {
            ...
            if (0 < strlen($tab)) {
                ...
                $handle2 = fopen('tempExcel/'.$nop_.'.xls', 'w');
                if (!fwrite($handle2, $tab)) {
                    echo "<script language=javascript1.2>
                        parent.window.alert('Can't convert to excel format');
                    </script>";
                    exit();
                }
                print_r("tempExcel/".$nop_.".xls");
            }
        } else {
            echo $stream;
        }
 ...
 */
        ...
        $str = "insert into ".$dbname.".kebun_premikemandoran (kodeorg, tanggal, karyawanid, jabatan, pembagi, premisumber, premikomputer, premiinput, updateby, postingby, posting)  values( '".$_SESSION['empl']['lokasitugas']."','".tanggalsystem($_POST['tanggal'])."','".$_POST['karyawanid']."','".$_POST['jabatan']."','".$_POST['pembagi']."','".$_POST['sumber']."','".$_POST['komputer']."','".$_POST['premi']."','".$_SESSION['standard']['userid']."', 0, 0)";
        if (mysql_query($str)) {
            break;
        }
    ...
    case 'ambilPremiKerani':
        ...
        $str1="select kodejabatan from datakaryawan where karyawanid='".$_POST['nikkerani']."'   ";
        $res1 = mysql_query($str1);
        $hasil= mysql_fetch_assoc($res1);

        echo $jumlahmandor.'#'.number_format($tPremiPanen, 0, '.', ',').'#'.$hasil['kodejabatan'];
