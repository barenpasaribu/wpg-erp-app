...
while ($rows1 = mysql_fetch_assoc($res1)) {
	...
	while ($rows2 = mysql_fetch_assoc($res2)) {
		...
		while ($rows3 = mysql_fetch_assoc($res3)) {
			...
			$data3 = [1, $rows3['noakun_kredit'], 'Alokasi Traksi '.$kodevhc,$rows1['kodevhc'], -$uang_jurnal, 'IDR', 1, '', $rows3['jenispekerjaan'], '', '', '', '', '', $rows3['kodevhc'], $rows3['notransaksi'], '', $rows3['notransaksi'], tanggalsystemw($rows3['tanggal']), substr($rows3['notransaksi'],1,4)];
