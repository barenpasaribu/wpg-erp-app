...
switch ($param['jenis']) {
    case 'global':
        ...
        $res = mysql_query($str);
        while ($bar = mysql_fetch_object($res)) {
            $kgbudget[$tahun.'-01'] = $bar->kg01 / 1000;
            $kgbudget[$tahun.'-02'] = $bar->kg02 / 1000;
            $kgbudget[$tahun.'-03'] = $bar->kg03 / 1000;
            $kgbudget[$tahun.'-04'] = $bar->kg04 / 1000;
            $kgbudget[$tahun.'-05'] = $bar->kg05 / 1000;
            $kgbudget[$tahun.'-06'] = $bar->kg06 / 1000;
            $kgbudget[$tahun.'-07'] = $bar->kg07 / 1000;
            $kgbudget[$tahun.'-08'] = $bar->kg08 / 1000;
            $kgbudget[$tahun.'-09'] = $bar->kg09 / 1000;
            $kgbudget[$tahun.'-10'] = $bar->kg10 / 1000;
            $kgbudget[$tahun.'-11'] = $bar->kg11 / 1000;
            $kgbudget[$tahun.'-12'] = $bar->kg12 / 1000;
        }
        if ('' === $kodeorg) {
            $str = 'select periode,sum(debet-kredit) as jumlah from '.$dbname.".keu_jurnalsum_vw where periode like '".$tahun."%'\r\n                    and kodeorg like '%E' and (noakun like '".$mayorTM."%' or noakun like '".$mayorPanen."%') group by periode order by periode";
        } else {
            $str = 'select periode,sum(debet-kredit) as jumlah from '.$dbname.".keu_jurnalsum_vw where periode like '".$tahun."%'\r\n                   and kodeorg='".$kodeorg."' and (noakun like '".$mayorTM."%' or noakun like '".$mayorPanen."%') group by periode order by periode";
        }
        ...
        if (0 < count($budget)) {
            foreach ($budget as $key => $val) {
                ...
                $nolLine[] = 0;
                $rpkgaktual[] = ($realisasi[$key] * 1000) / $netto[$key];
                $rpkgbudget[] = ($budget[$key] * 1000) / $kgbudget[$key];
            }
        } else {
            ...
            $rpkgbudget[] = 0;
            $rpkgbudget[] = 0;
            $rpkgaktual[] = ($realisasi[$tahun.'-01'] * 1000) / $netto[$tahun.'-01'];
            $rpkgaktual[] = ($realisasi[$tahun.'-02'] * 1000) / $netto[$tahun.'-02'];
            $rpkgaktual[] = ($realisasi[$tahun.'-03'] * 1000) / $netto[$tahun.'-03'];
            $rpkgaktual[] = ($realisasi[$tahun.'-04'] * 1000) / $netto[$tahun.'-04'];
            $rpkgaktual[] = ($realisasi[$tahun.'-05'] * 1000) / $netto[$tahun.'-05'];
            $rpkgaktual[] = ($realisasi[$tahun.'-06'] * 1000) / $netto[$tahun.'-06'];
            $rpkgaktual[] = ($realisasi[$tahun.'-07'] * 1000) / $netto[$tahun.'-07'];
            $rpkgaktual[] = ($realisasi[$tahun.'-08'] * 1000) / $netto[$tahun.'-08'];
            $rpkgaktual[] = ($realisasi[$tahun.'-09'] * 1000) / $netto[$tahun.'-09'];
            $rpkgaktual[] = ($realisasi[$tahun.'-10'] * 1000) / $netto[$tahun.'-10'];
            $rpkgaktual[] = ($realisasi[$tahun.'-11'] * 1000) / $netto[$tahun.'-11'];
            $rpkgaktual[] = ($realisasi[$tahun.'-12'] * 1000) / $netto[$tahun.'-12'];
            $targ[] = '?periode='.$tahun.'-01'.'&pks='.$kodeorg.'&jenis='.$level;
            $alts[] = ' Click to Drill';
            $targ[] = '?periode='.$tahun.'-02'.'&pks='.$kodeorg.'&jenis='.$level;
            $alts[] = ' Click to Drill';
        ...
        $totalProduksi = (@array_sum($realisasi1) * 1000) / @array_sum($netto1);
        $totalBudget = (@array_sum($budget1) * 1000) / @array_sum($kgbudget1);
        ...
        $graph->title->Set('BIAYA TANAMAN MENGHASILKAN VS BUDGET '.$param['pks']);
        $graph->yaxis->scale->SetGrace(30);
        $graph->y2axis->scale->SetGrace(30);
        $txt = new Text('Rp(juta) - Ton');
        ...
        $plot1->SetLegend('Biaya TM');
        $plot2->SetLegend('Budget Biaya TM');
        $plot3->SetLegend('Produksi(Ton)');
        $plot4->SetLegend('Budget Produksi(Ton)');
        ...
        $graph->AddY2($plot7);
        $graph->StrokeCSIM();
        echo '<br>Termasuk Biaya Panen';
        echo '<br>Total Luas TM :'.number_format($luas, 2).' Ha';
    ...
    case 'level1':
        ...
        while ($bar = mysql_fetch_assoc($res)) {
            ...
            $kgbudget[$bar['kodeorg']] = $bar[$index] / 1000;
        }
        $str = 'select kodeorg,sum(debet-kredit) as jumlah from '.$dbname.".keu_jurnalsum_vw where periode like '".$tahun."%'\r\n                    and kodeorg like '%E' and (noakun like '".$mayorTM."%' or noakun like '".$mayorPanen."%') group by kodeorg";
        $res = mysql_query($str);
        while ($bar = mysql_fetch_object($res)) {
        ...
        $totalProduksi = (@array_sum($realisasi1) * 1000) / @array_sum($netto1);
        $totalBudget = (@array_sum($budget1) * 1000) / @array_sum($kgbudget);
        $graph = new Graph(800, 400);
        $graph->img->SetMargin(50, 50, 50, 50);
        $graph->SetScale('textlin');
        $graph->SetShadow();
        ...
        $graph->xaxis->SetLabelAngle(90);
        $graph->title->Set('BIAYA TANAMAN MENGHASILKAN VS BUDGET '.$param['pks'].' Periode '.$tahun);
        $graph->yaxis->scale->SetGrace(30);
        $graph->y2axis->scale->SetGrace(30);
        $txt = new Text('Rp(juta) - Ton');
        $txt->SetPos(0.01, 0.1, 'left', 'bottom');
        ...
        echo '<br>Termasuk Biaya Panen<br><a href=javascript:history.back(-1)>Back</a>';
