...
if( isset($_POST['btnSubmit']) ){
	...
	{	
		...
		$qry="insert into ".$dbname.".tmp_wtpinternalht values('".$nama_file."','".$kdOrg."','".$date."'); ";
		...
		{ 
			...
			$abpp_start= $data->val($i, 3);
			$abpp_stop=$data->val($i, 4);			
			$sisa_abpp=preg_replace("/[^0-9.]/", "", $data->val($i, 5));
			$os_start= $data->val($i, 6);
			$os_stop=$data->val($i, 7);
			$sisa_os=preg_replace("/[^0-9.]/", "", $data->val($i, 8));
			$w33_kg=preg_replace("/[^0-9.]/", "", $data->val($i, 9));
			$w33_ppm=preg_replace("/[^0-9.]/", "", $data->val($i, 10));
			$w44_kg=preg_replace("/[^0-9.]/", "", $data->val($i, 11));
			$w44_ppm=preg_replace("/[^0-9.]/", "", $data->val($i, 12));
			$w45_kg=preg_replace("/[^0-9.]/", "", $data->val($i, 13));
			$w45_ppm=preg_replace("/[^0-9.]/", "", $data->val($i, 14));
			$w66_kg=preg_replace("/[^0-9.]/", "", $data->val($i, 15));
			$w66_ppm=preg_replace("/[^0-9.]/", "", $data->val($i, 16));
			$garam=preg_replace("/[^0-9.]/", "", $data->val($i, 17));
			$wbs1= $data->val($i, 18);
			$wbs2= $data->val($i, 19);
			$wbs1_2= $data->val($i, 20);
			$wbs2_2= $data->val($i, 21);
			$flowmeter_awals=preg_replace("/[^0-9.]/", "", $data->val($i, 22));
			$flowmeter_akhirs=preg_replace("/[^0-9.]/", "", $data->val($i, 23));
			$flowmeter_awalb=preg_replace("/[^0-9.]/", "", $data->val($i, 24));
			$flowmeter_akhirb=preg_replace("/[^0-9.]/", "", $data->val($i, 25));
			...
			// menyimpan data xls ke db tmp_absensidt
			 $qry1="insert into ".$dbname.".tmp_wtpinternaldt values('".$nama_file."','".$tgl."','".$shift_nik."','".$abpp_start."','".$abpp_stop."','".$sisa_abpp."','".$os_start."','".$os_stop."','".$sisa_os."','".$w33_kg."','".$w33_ppm."','".$w44_kg."','".$w44_ppm."','".$w45_kg."','".$w45_ppm."','".$w66_kg."','".$w66_ppm."','".$garam."','".$wbs1."','".$wbs2."','".$wbs1_2."','".$wbs2_2."','".$flowmeter_awals."','".$flowmeter_akhirs."','".$flowmeter_awalb."','".$flowmeter_akhirb."','".$keterangan."',current_timestamp); ";				
		...
		*/
		...
		// */
		// $totx= $res1['totx'];
		$squery="SELECT * FROM tmp_wtpinternaldt  where namafile='".$nama_file."' ";
		...
		while ($res = mysql_fetch_array($hasil)) { 
			...
			$abpp_start = $res['abpp_start'];
			$abpp_stop = $res['abpp_stop'];
			$sisa_abpp = $res['sisa_abpp'];
			$os_start = $res['os_start'];
			$os_stop = $res['os_stop'];;
			$sisa_os = $res['sisa_os'];
			$w33_kg = $res['w33_kg'];
			$w33_ppm = $res['w33_ppm'];
			...
			$wbs1_2 = $res['wbs1_2'];
			$wbs2_2 = $res['wbs2_2'];
			$flowmeter_awals = $res['flowmeter_awals'];
			$flowmeter_akhirs = $res['flowmeter_akhirs'];
			$flowmeter_awalb = $res['flowmeter_awalb'];
			$flowmeter_akhirb = $res['flowmeter_akhirb'];
...
if( $_POST['btnSubmitData']!=""){
	...
	if( $total_data > 0){
		...
		// get data karyawan
		$squery="SELECT a.*,b.kodeorganisasi as kodeorg FROM tmp_wtpinternaldt a,tmp_wtpinternalht b  where a.namafile=b.namafile and a.namafile='".$namafile."' ";
		// echo "<pre>";
		// print_r($squery);
		// die();
		$hasil = mysql_query($squery);
		...
		$tgl= $res['tgl'];
			...
			$queryCekAbsensiDT = "SELECT * FROM pabrik_wtpinternal WHERE kodeorg = '".$kodeorg."' and tgl = '".$tgl."' and shift_nik = '".$shift_nik."'  ";
			...
			if($jumlahCekAbsensiDT > 0){
				$sDetIns = "UPDATE ".$dbname.".pabrik_wtpinternal 
							SET
								abpp_start = '".$abpp_start."',
								abpp_stop='".$abpp_stop."',
								sisa_abpp='".$sisa_abpp."',
								os_start='".$os_start."',
								os_stop='".$os_stop."',
								...
								wbs1_2='".$wbs1_2."',
								wbs2_2='".$wbs2_2."',
								flowmeter_awals='".$flowmeter_awals."',
								flowmeter_akhirs='".$flowmeter_akhirs."',								
								flowmeter_awalb='".$flowmeter_awalb."',
								flowmeter_akhirb='".$flowmeter_akhirb."',
				 ...
				 $sDetIns="insert into ".$dbname.".pabrik_wtpinternal values('".$namafile."','".$tgl."','".$shift_nik."','".$abpp_start."','".$abpp_stop."','".$sisa_abpp."','".$os_start."','".$os_stop."','".$sisa_os."','".$w33_kg."','".$w33_ppm."','".$w44_kg."','".$w44_ppm."','".$w45_kg."','".$w45_ppm."','".$w66_kg."','".$w66_ppm."','".$garam."','".$wbs1."','".$wbs2."','".$wbs1_2."','".$wbs2_2."','".$flowmeter_awals."','".$flowmeter_akhirs."','".$flowmeter_awalb."','".$flowmeter_akhirb."','".$keterangan."','".$kdOrg."',current_timestamp)"; 
			}
		...
		$emptyQueryTable2 = "TRUNCATE TABLE tmp_wtpinternaldt";
...
OPEN_BOX('', 'Pabrik Logsheet WTP Internal Upload</b>');
