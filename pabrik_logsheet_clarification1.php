...
if( isset($_POST['btnSubmit']) ){
	...
	{	
		...
		$qry="insert into ".$dbname.".tmp_klarification1ht values('".$nama_file."','".$kdOrg."','".$date."'); ";
		...
		{ 
			...
			$tgl=$data->val($i, 1);	
			$jam=$data->val($i, 2);				
			$cst_cm=preg_replace("/[^0-9.]/", "", $data->val($i, 3));
			$cst_c=preg_replace("/[^0-9.]/", "", $data->val($i, 4));			
			$st_cm=preg_replace("/[^0-9.]/", "", $data->val($i, 5));
			$st_c=preg_replace("/[^0-9.]/", "", $data->val($i, 6));
			$ot1_cm=preg_replace("/[^0-9.]/", "", $data->val($i, 7));
			$ot1_c=preg_replace("/[^0-9.]/", "", $data->val($i, 8));
			$ot2_cm=preg_replace("/[^0-9.]/", "", $data->val($i, 9));
			$ot2_c=preg_replace("/[^0-9.]/", "", $data->val($i, 10));
			$recovery1=preg_replace("/[^0-9.]/", "", $data->val($i, 11));
			$recovery2=preg_replace("/[^0-9.]/", "", $data->val($i, 12));
			$pengiriman_cpo_awal= $data->val($i, 13);
			$pengiriman_cpo_akhir= $data->val($i, 14);
			$keterangan=$data->val($i, 15);
			...
			// menyimpan data xls ke db tmp_absensidt
			 $qry1="insert into ".$dbname.".tmp_klarification1dt values('".$nama_file."','".$tgl."','".$jam."','".$cst_cm."','".$cst_c."','".$st_cm."','".$st_c."','".$ot1_cm."','".$ot1_c."','".$ot2_cm."','".$ot2_c."','".$recovery1."','".$recovery2."','".$pengiriman_cpo_awal."','".$pengiriman_cpo_akhir."','".$keterangan."',current_timestamp); ";				
		...
		*/
		...
		// */
		// $totx= $res1['totx'];
		$squery="SELECT * FROM tmp_klarification1dt  where namafile='".$nama_file."' ";
...
if( $_POST['btnSubmitData']!=""){
	...
	if( $total_data > 0){
		...
		// get data karyawan
		$squery="SELECT a.*,b.kodeorganisasi as kodeorg FROM tmp_klarification1dt a,tmp_klarification1ht b  where a.namafile=b.namafile and a.namafile='".$namafile."' ";
		// echo "<pre>";
		// print_r($squery);
		// die();
		$hasil = mysql_query($squery);
		while ($res = mysql_fetch_array($hasil)) { 
			...
			$queryCekAbsensiDT = "SELECT * FROM pabrik_klarification1 WHERE kodeorg = '".$kodeorg."' and tgl = '".$tgl."' and jam = '".$jam."'  ";
			...
			if($jumlahCekAbsensiDT > 0){
				$sDetIns = "UPDATE ".$dbname.".pabrik_klarification1
							SET
								cst_cm = '".$cst_cm."',
								cst_c='".$cst_c."',
								st_cm = '".$st_cm."',
								...
								recovery1 = '".$recovery1."',
								recovery2='".$recovery2."',
								pengiriman_cpo_awal = '".$pengiriman_cpo_awal."',
								pengiriman_cpo_akhir='".$pengiriman_cpo_akhir."',
				 ...
				 $sDetIns="insert into ".$dbname.".pabrik_klarification1 values('".$namafile."','".$tgl."','".$jam."','".$cst_cm."','".$cst_c."','".$st_cm."','".$st_c."','".$ot1_cm."','".$ot1_c."','".$ot2_cm."','".$ot2_c."','".$recovery1."','".$recovery2."','".$pengiriman_cpo_awal."','".$pengiriman_cpo_akhir."','".$keterangan."','".$kdOrg."',current_timestamp)"; 
			}
		...
		$emptyQueryTable2 = "TRUNCATE TABLE tmp_klarification1dt";
...
OPEN_BOX('', 'Pabrik Logsheet Klarification 1 Upload</b>');
