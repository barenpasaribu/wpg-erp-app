<?php
    ...
    $optSatuanPenampang = array( 
                            "cm2" => "cm2",
                            "m2" => "m2"
                        );
    $optSatuanKerucut = array( 
                            "cm3" => "cm3",
                            "m3" => "m3"
                        );
    ...
    $els[] = [makeElement('luaspenampang', 'label', $_SESSION['lang']['luaspenampang']), makeElement('luaspenampang', 'textnum', 0, ['style' => 'width:200px', 'maxlength' => '10', 'onkeypress' => 'return angka_doang(event)'])];
    ...
    $els[] = [makeElement('satuanpenampang', 'label', $_SESSION['lang']['satuanpenampang']), makeElement('satuanpenampang', 'select', '', ['style' => 'width:200px'], $optSatuanPenampang)];
    $els[] = [makeElement('volumekerucut', 'label', $_SESSION['lang']['volumekerucut']), makeElement('volumekerucut', 'textnum', 0, ['style' => 'width:200px', 'maxlength' => '10', 'onkeypress' => 'return angka_doang(event)'])];
    ...
    $els[] = [makeElement('satuankerucut', 'label', $_SESSION['lang']['satuankerucut']), makeElement('satuankerucut', 'select', '', ['style' => 'width:200px'], $optSatuanKerucut)];
    $els[] = [makeElement('stsuhu', 'label', $_SESSION['lang']['stsuhu']), makeElement('stsuhu', 'textnum', 0, ['style' => 'width:200px', 'maxlength' => '3', 'onkeypress' => 'return angka_doang(event)'])];
    $els[] = [makeElement('mejaukur', 'label', 'Meja Ukur'), makeElement('mejaukur', 'text', 0, ['style' => 'width:200px', 'maxlength' => '10', 'onkeypress' => 'return angka_doang(event)'])];
    $els[] = [makeElement('density', 'label', 'Density'), makeElement('density', 'text', 0, ['style' => 'width:200px', 'maxlength' => '10', 'onkeypress' => 'return angka_doang(event)'])];
    $els[] = [makeElement('kadarair', 'label', 'Kadar Air'), makeElement('kadarair', 'text', 0, ['style' => 'width:200px', 'maxlength' => '10', 'onkeypress' => 'return angka_doang(event)'])];
    $els[] = [makeElement('rasiokerneltonut', 'label', 'Rasio Kernel to Nut'), makeElement('rasiokerneltonut', 'text', 0, ['style' => 'width:200px', 'maxlength' => '10', 'onkeypress' => 'return angka_doang(event)'])];
    $els[] = [makeElement('iskonversi', 'label', 'Is Konversi'), makeElement('iskonversi', 'select', 0, ['style' => 'width:200px'], $optIsKonversi)];
    
    
    $fieldStr = '##kodeorg##kodetangki##keterangan##komoditi##stsuhu##luaspenampang##satuanpenampang##volumekerucut##satuankerucut##mejaukur##density##iskonversi';
    ...
    foreach ($field as $hName) {
        if($hName == 'mejaukur'){
            $tables .= '<td>Meja Ukur</td>';
        }elseif ($hName == 'iskonversi') {
            $tables .= '<td>Is Konversi</td>';
        }elseif ($hName == 'density') {
            $tables .= '<td>Density</td>';
        }elseif ($hName == 'kadarair') {
            $tables .= '<td>Kadar Air</td>';
        }elseif ($hName == 'rasiokerneltonut') {
            $tables .= '<td>Rasio Kernel to Nut</td>';
        }else{
    ...
    foreach ($result as $row) {
        ...
        $tables .= "<td><img id='editRow".$i."' title='Edit' onclick=\"editRow('".$row['kodeorg']."','".$row['kodetangki']."','".$row['keterangan']."','".$row['komoditi']."',".$row['stsuhu'].", '".$row['luaspenampang']."','".$row['satuanpenampang']."','".$row['volumekerucut']."','".$row['satuankerucut']."','".$row['mejaukur']."','".$row['density']."','".$row['kadarair']."','".$row['rasiokerneltonut']."','".$row['iskonversi']."')\"\r\n    class='zImgBtn' src='images/001_45.png' /></td>";
