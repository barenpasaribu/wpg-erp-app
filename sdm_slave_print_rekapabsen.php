...
$PeriodeAbsen 	= isset($_GET['PeriodeAbsen']) ? $_GET['PeriodeAbsen'] : null;
$Perusahaan	  	= isset($_GET['Perusahaan']) ? $_GET['Perusahaan'] : null;
...
function GetKaryawanIjin($Karyawan, $Perusahaan, $Tanggal){
	...
	b.bagian, b.namakaryawan, b.lokasitugas, 
	c.kodeabsen
    FROM ".$dbname.".sdm_ijin a
    LEFT JOIN ".$dbname.".datakaryawan b on a.karyawanid = b.karyawanid
	LEFT JOIN ".$dbname.".sdm_jenis_ijin_cuti c on a.jenisijin = c.id
    WHERE substr(a.darijam,1,10) >= '".$Tanggal['TanggalAwal']."' and stpersetujuan1 = '1' and stpersetujuanhrd = '1' OR substr(a.sampaijam,1,10) >= '".$Tanggal['TanggalAwal']."' and stpersetujuan1 = '1' and stpersetujuanhrd = '1'
	and ".$WhereKaryawan." like '%".$Karyawan."%'
...
$stream.="<thead><tr>";
$stream.="<td rowspan=2 align=center>".$_SESSION['lang']['nourut']."</td>";
$stream.="<td rowspan=2 align=center>".$_SESSION['lang']['nik']."</td>";
$stream.="<td rowspan=2 align=center>".$_SESSION['lang']['namakaryawan']."</td>";
...
foreach($Absensi as $AKey => $AVal) {
	$stream.="<td align=center>".$AVal['kodeabsen']."</td>";
...
foreach($DataKaryawan as $DKKey => $DKVal) {
	...
	foreach($DKVal as $DKTKey => $DKTVal) {
		...
		$stream.="<td align=left>".$DKTVal['nik']."</td>";
		$stream.="<td align=left>".$DKTVal['nama']."</td>";
		foreach($DataCalender as $DCKey => $DCVal) {
			...
			if($DCVal['status'] == 'L' || $DCVal['status'] == 'MG' || $DCVal['status'] == 'C'){
				if($DataKaryawanIjin[$DKTKey][$DCVal['tanggal']]) {
					...
					$bgcolor = "style='background-color: #F8F8F8'";
					$pres='HL';
					$StatusAbsen['HL'] +=1;
				...
				if($DataKaryawanIjin[$DKTKey][$DCVal['tanggal']]) {
					...
					$pres='D';
					$StatusAbsen['D'] +=1;
				} else if($DataKaryawanSakit[$DKTKey][$DCVal['tanggal']]) {
					$bgcolor = "";
					$pres='S';
					$StatusAbsen['S'] +=1;
				} else {
		...
		foreach($Absensi as $AKey => $AVal) {
			$stream.="<td align=right>".$StatusAbsen[$AVal['kodeabsen']]."</td>";	
			$StatusAbsen[$AVal['kodeabsen']] = 0;
		}
		$stream.="</tr>";
	}
...
foreach($Absensi as $AKey => $AVal) {
	if ($Count % 2 == 0){
		...
		$stream.='<td colspan=5>'.$AVal['keterangan'].'</td>';
	} else {
		$stream.='<td width=40>'.$AVal['kodeabsen'].'</td>';
		$stream.='<td colspan=15>'.$AVal['keterangan'].'</td>';
		$stream.='</tr>';
	}
	$Count++;
}
