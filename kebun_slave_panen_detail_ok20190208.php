...
switch ($proses) {
	...
	case 'updUpah':
		...
		$xnik = '';
		$xkodeorg = '';
		$str = "select notransaksi,nik,kodeorg from kebun_prestasi where substr(notransaksi,1,8) = '".$subnotrans."' and nik = '".$param['nik']."' limit 1";
		...
		$totlbhbasis = 0;
		$totpremirajin = 0;
		$str = "select sum(jumlahlbhbasis) as totlbhbasis, sum(premirajin) as totpremirajin from kebun_prestasi where substr(notransaksi,1,8) = '".$subnotrans."' and nik = '".$param['nik']."'";
		$qstr = mysql_query($str);
		while ($rows = mysql_fetch_assoc($qstr)) {
			$totlbhbasis = $rows['totlbhbasis'];
			$totpremirajin = $rows['totpremirajin'];
		}
//		echo "warning sql".$str;
//		exit();
        ...
        if ($qwe == 'Sun'|| isset($optHariLbr[$_SESSION['empl']['regional']]) && $optHariLbr[$_SESSION['empl']['regional']] !='') {
            if ($_SESSION['empl']['regional']== 'KALTENG' || $_SESSION['empl']['regional'] == 'SULTRA' || $_SESSION['empl']['regional'] == 'SUMUT') {
                $is = 1;
                $sum = 0;
                $sPremi = 'select distinct * from '.$dbname.".kebun_5premipanen where kodeorg='".$_SESSION['empl']['regional']."' and tahuntanam='".$param['tahuntanam']."' order by lebihbasiskg";
                $qPremi = mysql_query($sPremi) ;
                while ($rPremi = mysql_fetch_assoc($qPremi)) {
                    if ($_SESSION['empl']['regional']=='KALTENG' || $_SESSION['empl']['regional']=='SUMUT' || $_SESSION['empl']['regional']=='SULTRA') {
                            ...
                            if ($hasilKg <= $basis || $basis <= $hasilKg) {
                                if ($optRp[$param['tahuntanam']] !=0) {
									...
									for ($awl = 0; $awl <= $JmlhRow; $awl++) { 
										...
										." and kodeorg='".$_SESSION['empl']['regional']."' limit 1";
										$qstr = mysql_query($str);
										while ($rows = mysql_fetch_assoc($qstr)) {
		...
		// Jika bukan hari Minggu 
            ...
            if ($_SESSION['empl']['regional']=='KALTENG' || $_SESSION['empl']['regional']=='SULTRA' || $_SESSION['empl']['regional'] == 'SUMUT') {
				...
				$lebihbasisx=0;
				$rupiahx=0;
                $sPremi = 'select distinct * from '.$dbname.".kebun_5premipanen where kodeorg='".$_SESSION['empl']['regional']."' and tahuntanam='".$param['tahuntanam']."' and (hasilpanen between 0 and ".round($jumlahlbhbasis, 0, PHP_ROUND_HALF_UP).') order by hasilkg desc';                
				$qPremi = mysql_query($sPremi) ;
                while ($rPremi = mysql_fetch_assoc($qPremi)) {
                    if ($_SESSION['empl']['regional']=='KALTENG' || $_SESSION['empl']['regional']=='SUMUT' || $_SESSION['empl']['regional'] == 'SULTRA') {
                        $basisKg2[$is] = $rPremi['lebihbasiskg'];
                            ...
                            if (($basis < $hasilKg) || ($isDobel = 1)){
                                if ($optRp[$param['tahuntanam']] !=0) {
                                   ...
                                   if ($isDobel == 0) {
									...
									// Premi Kerajinan berdasar lebih basis (Hardcoded)
