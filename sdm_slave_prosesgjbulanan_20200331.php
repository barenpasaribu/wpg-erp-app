...
//					$pembagi_bulan = 1; //pada desember dibagi satu
		...
		$pkpthr[$val[0]] = 0;
		if ($thrvalue[$val[0]] > 0) {
			$nilaibatasmax4 = 6000000; // batas biaya jab max dlm setahun
			$biayajabthr[$val[0]] = floor((($totgajibruto[$val[0]] * $pembagi_bulan)+ $thrvalue[$val[0]]) * 0.05);
			...
			*/
			...
			$gjnettosetahunbonus[$val[0]] = ($totgajibruto[$val[0]] * $pembagi_bulan) + $thrvalue[$val[0]] - ($biayajabthr[$val[0]] + ($jhtkarypersen[$val[0]] * $pembagi_bulan) + ($jpkarypersen[$val[0]] * $pembagi_bulan));
		...
		if ($bonusvalue[$val[0]] > 0) {
			$nilaibatasmax4 = 6000000; // batas biaya jab max dlm setahun
			$biayajabbonus[$val[0]] = floor((($totgajibruto[$val[0]] * $pembagi_bulan)+ $bonusvalue[$val[0]]) * 0.05);
			...
			*/
			...
			$gjnettosetahunbonus[$val[0]] = ($totgajibruto[$val[0]] * $pembagi_bulan) + $bonusvalue[$val[0]] - ($biayajabbonus[$val[0]] + ($jhtkarypersen[$val[0]] * $pembagi_bulan) + ($jpkarypersen[$val[0]] * $pembagi_bulan));
...
$ptkpbon = $ptkp[$val[0]];
if ($idxx=='0000000003'){
echo "warning: /netto setahun bonus= ".$x." /netto setahun= ".$y." /biaya jab bonus= ".$z." /pkpbonus= ".$pkpbon." /ptkp= ".$ptkpbon;
exit();
}
*/
...
*/
		...
		if ($nrapel > 0){
			...
			while ($barx = mysql_fetch_object($resx)) {
				...
				$netto = ($netto + round($nrapel/$nbln) - $byjab) * $pembagi_bulan;
				$pkpx = floor(($netto - $ptkp[$val[0]]) / 1000) * 1000;

/*
$xptkp= $ptkp[$val[0]];
if ($idxx=='0000000005'){
echo "warning: pkp= ".$pkpx." /ptkp= ".$xptkp." /netto= ".$netto;
exit();
}
*/
			...
			if ('' == $npwp[$val[0]]) {
				$pphrapel= $pphrapel + ($pphrapel * 20/100);
			}
			$pphrapel_x = $pphrapel - $akumulasi_pph21[$val[0]];
			$pph21rapel[$val[0]] = $pphrapel_x;
...
if ($idxx=='0000000005'){
echo "warning: bln= ".$nbln." /pph rapel setahun= ".$pph21rapel_setahun." /pphrapel= ".$pphrapel." /pph akum= ".$akum." /pph rapel akhir= ".$pp21;
exit();
}
*/
		...
		*/
		...
		if( ($tanggalkeluar[$val[0]] != "" && $tanggalkeluar[$val[0]] != "0000-00-00") || $flag_desember == "Y"){ 
			$pphsetahun[$val[0]] = $pphsetahun[$val[0]] - $akumulasi_pph21[$val[0]];
//			$pphsetahun[$val[0]] = $zz - $akumulasi_pph21[$val[0]]; // FA 20200310
		}
