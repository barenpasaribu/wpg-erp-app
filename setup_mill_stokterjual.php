...
//$str = 'select KGPOTSORTASI from mstrxtbs where PRODUCTCODE="'. $productcode .'" AND DATEOUT="'. $txtTgl.'"';
$str = 'select SUM(NETTO) as NETTO from ' . $dbname2 . '.`mstrxtbs` where SPBNO="'. $no_trans .'" AND PRODUCTCODE="'. $productcode .'" AND SPBNO="'. $no_trans .'" AND DATEOUT LIKE"%'. $txtTgl.'%" GROUP BY DATEOUT';
