...
switch ($proses) {
    case 'showDetail':
        ...
        $cols = 'nik,kodeorg,bjraktual,tahuntanam,tarif,norma,hasilkerjakg,brondolan,hasilkerja,jumlahlbhbasis,'.'umr,premilebihbasis,premihadir,premirajin,premibrondol,upahpremi,upahkerja,penalti1,penalti2,penalti3,penalti4,penalti5,penalti6,penalti7,rupiahpenalty,luaspanen';
        ...
        $theForm2->addEls('nik', $_SESSION['lang']['nik'], '', 'selectsearch', 'L', 40, $optKary);
        $theForm2->_elements[0]->_attr['onchange'] = 'updUpah()';
        $theForm2->addEls('kodeorg', $_SESSION['lang']['kodeorg'], '', 'selectsearch', 'L', 40, $optOrg, null, null, null, 'ftPrestasi_kodeorg');
        ...
        $theForm2->_elements[8]->_attr['title'] = 'Hasil Kerja (JJG) * BJR [Kebun - Setup - Tabel BJR]';
        $theForm2->addEls('jumlahlbhbasis', $_SESSION['lang']['jumlahlbhbasis'], '0', 'textnum', 'R', 10);
        $theForm2->_elements[9]->_attr['disabled'] = 'disabled';
        $theForm2->addEls('umr', $_SESSION['lang']['upahharian'], $Umr[0]['nilai'] / 25, 'textnum', 'R', 15);
        $theForm2->_elements[10]->_attr['disabled'] = 'disabled';
        $theForm2->addEls('premilebihbasis', $_SESSION['lang']['premilebihbasis'], '0', 'textnum', 'R', 15);
        $theForm2->_elements[11]->_attr['disabled'] = 'disabled';
        $theForm2->addEls('premihadir', $_SESSION['lang']['premihadir'], '0', 'textnum', 'R', 15);
        $theForm2->_elements[12]->_attr['disabled'] = 'disabled';
        $theForm2->addEls('premirajin', $_SESSION['lang']['premirajin'], '0', 'textnum', 'R', 15);
        $theForm2->_elements[13]->_attr['disabled'] = 'disabled';
        $theForm2->addEls('premibrondol', $_SESSION['lang']['premibrondol'], '0', 'textnum', 'R', 15);
        $theForm2->_elements[14]->_attr['disabled'] = 'disabled';
        $theForm2->addEls('upahpremi', $_SESSION['lang']['totalpremi'], '0', 'textnum', 'R', 15);
        $theForm2->_elements[15]->_attr['disabled'] = 'disabled';
        $theForm2->addEls('upahkerja', $_SESSION['lang']['totupprem'], '0', 'textnum', 'R', 15);
        ...
        $theForm2->_elements[17]->_attr['onblur'] = "updDenda('BM');";
        $theForm2->_elements[18]->_attr['onblur'] = "updDenda('TP');";
        $theForm2->_elements[19]->_attr['onblur'] = "updDenda('TD');";
        $theForm2->_elements[20]->_attr['onblur'] = "updDenda('BT');";
        $theForm2->_elements[21]->_attr['onblur'] = "updDenda('PT');";
        $theForm2->_elements[22]->_attr['onblur'] = "updDenda('TM');";
        ...
        $formTab2->_noEnable = '##tahuntanam##bjraktual##upahkerja##upahpremi##rupiahpenalty##hasilkerjakg##norma##tarif##umr##jumlahlbhbasis##premirajin##premihadir##premibrondol##premilebihbasis##hasilkerja';
        ...
        break;
    case 'add':
        $cols = ['nik', 'kodeorg', 'bjraktual', 'tahuntanam', 'tarif', 'norma', 'hasilkerjakg', 'brondolan', 'hasilkerja', 'jumlahlbhbasis', 'umr', 'premilebihbasis', 'premihadir', 'premirajin', 'premibrondol', 'upahpremi', 'upahkerja', 'penalti1', 'penalti2', 'penalti3', 'penalti4', 'penalti5', 'penalti6', 'penalti7', 'rupiahpenalty', 'luaspanen', 'notransaksi', 'kodekegiatan', 'statusblok', 'pekerjaanpremi'];
        $data = $param;
        unset($data['numRow']);
        $data['kodekegiatan'] = '0';
        $data['statusblok'] = 0;
    ...
    case 'updTahunTanam':
        ...
        echo $thntnm.'####'.number_format($rBjr['bjr'], 2).'####'.$basiskg;
    ...
    case 'updUpah':
        ...
        $hasilKg = $param['hslkrjkg'];
        $hasilJjg = $param['hslkrjkg'] / $param['bjraktual'];
        $jumlahlbhbasis = $param['hslkrjkg'] - $param['norma'];
        $firstKary = $param['nik'];
        ...
        if ('Sun' === $qwe || isset($optHariLbr[$_SESSION['empl']['regional']]) && '' !== $optHariLbr[$_SESSION['empl']['regional']]) {
            $basis = 0;
            if ('KALTENG' === $_SESSION['empl']['regional'] || 'SULTRA' === $_SESSION['empl']['regional'] || 'SUMUT' === $_SESSION['empl']['regional']) {
                $is = 1;
                $sum = 0;
                $sPremi = 'select distinct * from '.$dbname.".kebun_5premipanen where kodeorg='".$_SESSION['empl']['regional']."' and tahuntanam='".$param['tahuntanam']."' and (hasilpanen between 0 and ".$jumlahlbhbasis.') order by hasilkg desc';
                $qPremi = mysql_query($sPremi) ;
                while ($rPremi = mysql_fetch_assoc($qPremi)) {
                    if ('KALTENG' === $_SESSION['empl']['regional'] || 'SUMUT' === $_SESSION['empl']['regional'] || 'SULTRA' === $_SESSION['empl']['regional']) {
                ...
                if (0 !== $basis) {
                    if (1 === $optDenda[$param['tahuntanam']]) {
                        if ($hasilKg <= $basis) {
                            $upah = $uphHarian;
                            $insentif = 0;
                            ...
                            if ($basis < $hasilKg) {
                                if (0 !== $optRp[$param['tahuntanam']]) {
                                    $upah = $uphHarian;
                                    $insentif = 0;
                                    $penguranglbhbasis = $jumlahlbhbasis;
                                    for ($awl = 1; $awl <= $JmlhRow; ++$awl) {
                                        $qw = 1;
                                        $penguranglbhbasis = $jumlahlbhbasis - $basisKg2[$awl - 1];
                                        if ($basisKg2[$awl - 1] <= $jumlahlbhbasis) {
                                            if (3 <= $JmlhRow) {
                                                if ($awl === $JmlhRow) {
                                                    if ($penguranglbhbasis < $basisKg2[$awl]) {
                                                        $penguranglbhbasis += $basisKg2[$awl - 2];
                                                        $insentif += $penguranglbhbasis * $rupiah[$awl - 1];
                                                    } else {
                                                        $penguranglbhbasis -= $basisKg2[$awl - 2];
                                                        $premjin += $premiRajin[$awl];
                                                        $insentif += $penguranglbhbasis * $rupiah[$awl];
                                                    }
                                                } else {
                                                    if ($penguranglbhbasis < $basisKg2[$awl]) {
                                                        $insentif += $basisKg2[$awl - 2] * $rupiah[$awl - 2];
                                                        $premjin += $premiRajin[$awl];
                                                    } else {
                                                        $insentif += $basisKg2[$awl] * $rupiah[$awl];
                                                        $premjin += $premiRajin[$awl];
                                                    }
                                                }
                                            }
            ...
            echo $upah.'####'.number_format($basis, 0).'####'.$insentif.'####'.$hasilJjg.'####'.$premihadir.'####'.$jumlahlbhbasis.'####'.$premjin.'####'.$premibrondol.'####'.$totprem.'####'.$totuppremx;
        }
