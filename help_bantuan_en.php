...
$offset = $page * $limit;
$q = 'select count(*) as jmlhrow from '.$dbname.'.guidance_english order by kode asc';
$query = mysql_query($q) || exit(mysql_error($conns));
while ($jsl = mysql_fetch_object($query)) {
    $jmlbrs = $jsl->jmlhrow;
}
$q2 = 'select * from '.$dbname.'.guidance_english order by kode asc,tentang,modul,isi limit '.$offset.','.$limit.' ';
