...
$query1 = selectQuery($dbname, 'datakaryawan', 'karyawanid,namakaryawan,jms,statuspajak,npwp', 'tipekaryawan in(1,2,6) and '."lokasitugas='".$_SESSION['empl']['lokasitugas']."' and "."(tanggalkeluar>='".$tanggal1."' or tanggalkeluar is NULL) and alokasi=0 and sistemgaji='Bulanan'"." and ( tanggalmasuk<='".$tanggal2."' or tanggalmasuk='0000-00-00' or tanggalmasuk is null)");
...
foreach ($resgjh as $idx => $val) {
    ...
    $readyData[] = ['kodeorg' => $_SESSION['empl']['lokasitugas'], 'periodegaji' => $param['periodegaji'], 'karyawanid' => $val['karyawanid'], 'idkomponen' => 37, 'jumlah' => $tdkdibayar[$val['karyawanid']], 'pengali' => 1];
}
...
foreach ($res1 as $idx => $val) {
    if ($id[$val['karyawanid']][0] == $val['karyawanid']) {
        ...
        if ((1 == $val['idkomponen'] || 2 == $val['idkomponen'] || 31 == $val['idkomponen']) && '' != $nojms[$val['karyawanid']]) {
            $tjms[$val['karyawanid']] += $val['jumlah'];
        }
    }
}
foreach ($tjms as $key => $nilai) {
    if ('KBL' == $tipekaryawan[$key] || 'Kontrak' == $tipekaryawan[$key]) {
...
foreach ($id as $key => $val) {
    ...
    foreach ($readyData as $dat => $bar) {
        if ($val[0] == $bar['karyawanid'] && (1 == $comp[$bar['idkomponen']] || 37 == $bar['idkomponen'])) {
