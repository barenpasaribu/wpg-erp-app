...
if( isset($_POST['btnSubmit']) ){
	...
	{	
		...
		$qry="insert into ".$dbname.".tmp_power2ht values('".$nama_file."','".$kdOrg."','".$date."'); ";
		...
		{ 
			...
			$tgl=$data->val($i, 1);				
			$hm_bolah_t1=preg_replace("/[^0-9.]/", "", $data->val($i, 2));
			$hm_bolah_t2=preg_replace("/[^0-9.]/", "", $data->val($i, 3));
			
			$hm_kolah_t1=preg_replace("/[^0-9.]/", "", $data->val($i, 4));
			$hm_kolah_t2=preg_replace("/[^0-9.]/", "", $data->val($i, 5));
			$kwh_bolah_t1=preg_replace("/[^0-9.]/", "", $data->val($i, 6));;
			$kwh_bolah_t2=preg_replace("/[^0-9.]/", "", $data->val($i, 7));;
			$kwh_kolah_t1=preg_replace("/[^0-9.]/", "", $data->val($i, 8));;
			$kwh_kolah_t2=preg_replace("/[^0-9.]/", "", $data->val($i, 9));;
			...
			// menyimpan data xls ke db tmp_absensidt
			 $qry1="insert into ".$dbname.".tmp_power2dt values('".$nama_file."','".$tgl."','".$hm_bolah_t1."','".$hm_bolah_t2."','".$hm_kolah_t1."','".$hm_kolah_t2."','".$kwh_bolah_t1."','".$kwh_bolah_t2."','".$kwh_kolah_t1."','".$kwh_kolah_t2."','".$keterangan."',current_timestamp); ";				
		...
		*/
		...
		// */
		// $totx= $res1['totx'];
		$squery="SELECT * FROM tmp_power2dt  where namafile='".$nama_file."' ";
		...
		while ($res = mysql_fetch_array($hasil)) { 
			...
			$tgl= $res['tgl'];
			$hm_bolah_t1= $res['hm_bolah_t1'];
			$hm_bolah_t2 = $res['hm_bolah_t2'];
			$hm_kolah_t1 = $res['hm_kolah_t1'];
			$hm_kolah_t2 = $res['hm_kolah_t2'];
			$kwh_bolah_t1 = $res['kwh_bolah_t1'];
			$kwh_bolah_t2 = $res['kwh_bolah_t2'];;
			$kwh_kolah_t1 = $res['kwh_kolah_t1'];
			$kwh_kolah_t2 = $res['kwh_kolah_t2'];
...
if( $_POST['btnSubmitData']!=""){
	...
	if( $total_data > 0){
		...
		// get data karyawan
		$squery="SELECT a.*,b.kodeorganisasi as kodeorg FROM tmp_power2dt a,tmp_power2ht b  where a.namafile=b.namafile and a.namafile='".$namafile."' ";
		// echo "<pre>";
		// print_r($squery);
		// die();
		$hasil = mysql_query($squery);
		while ($res = mysql_fetch_array($hasil)) { 
			...
			$queryCekAbsensiDT = "SELECT * FROM pabrik_power2 WHERE kodeorg = '".$kodeorg."' and tgl = '".$tgl."' ";
			...
			if($jumlahCekAbsensiDT > 0){
				$sDetIns = "UPDATE ".$dbname.".pabrik_power2 
							SET
								hm_bolah_t1 = '".$hm_bolah_t1."',
								hm_bolah_t2='".$hm_bolah_t2."',
								hm_kolah_t1='".$hm_kolah_t1."',
								hm_kolah_t2='".$hm_kolah_t2."',
								kwh_bolah_t1='".$kwh_bolah_t1."',
								kwh_bolah_t2='".$kwh_bolah_t2."',
								kwh_kolah_t1='".$kwh_kolah_t1."',
								kwh_kolah_t2='".$kwh_kolah_t2."',
				 ...
				 $sDetIns="insert into ".$dbname.".pabrik_power2 values('".$namafile."','".$tgl."','".$hm_bolah_t1."','".$hm_bolah_t2."','".$hm_kolah_t1."','".$hm_kolah_t2."','".$kwh_bolah_t1."','".$kwh_bolah_t2."','".$kwh_kolah_t1."','".$kwh_kolah_t2."','".$keterangan."',current_timestamp,'".$kodeorg."'); ";
			}
		...
		$emptyQueryTable2 = "TRUNCATE TABLE tmp_power2dt";
...
OPEN_BOX('', 'Pabrik Logsheet Power 2 Upload</b>');
