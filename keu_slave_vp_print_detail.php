...
$col1 = 'noakun,jumlah,namaakun';
$cols = array('noakun', 'jumlah', 'namaakun');
$where = 'novp=\'' . $param['novp'] . '\'';
$query = selectQuery($dbname, 'keu_vpdt_vw', $col1, $where);
...
$pdf->Cell((40 / 100) * $width, $height, 'DATE OF INVOICE RECEIVED', 0, 0, 'L', 1);
...
$pdf->Cell((30 / 100) * $width, $height, $dataD['debet'][0]['noakun'] . ' ' . $dataD['debet'][0]['namaakun'], 0, 0, 'L', 1);
$pdf->Cell((28 / 100) * $width, $height, number_format($dataD['debet'][0]['jumlah'], 0), 0, 1, 'R', 1);

if (isset($dataD['debet'][1])) {
	$onDebet = true;
	$pdf->Cell((30 / 100) * $width, $height, $dataD['debet'][1]['noakun'] . ' ' . $dataD['debet'][1]['namaakun'], 0, 0, 'L', 1);
	...
	$pdf->Cell((25 / 100) * $width, $height, $dataD['kredit'][0]['noakun'] . ' ' . $dataD['kredit'][0]['namaakun'], 0, 0, 'L', 1);
...
if ($onDebet) {
	if (isset($dataD['debet'][2])) {
		$onDebet = true;
		$pdf->Cell((30 / 100) * $width, $height, $dataD['debet'][2]['noakun'] . ' ' . $dataD['debet'][2]['namaakun'], 0, 0, 'L', 1);
	...
	++$currCredit;
	$pdf->Cell((5 / 100) * $width, $height, '', 0, 0, 'L', 1);
	$pdf->Cell((25 / 100) * $width, $height, $dataD['kredit'][1]['noakun'] . ' ' . $dataD['kredit'][1]['namaakun'], 0, 0, 'L', 1);
...
if ($onDebet) {
	if (isset($dataD['debet'][3])) {
		$onDebet = true;
		$pdf->Cell((30 / 100) * $width, $height, $dataD['debet'][3]['noakun'] . ' ' . $dataD['debet'][3]['namaakun'], 0, 0, 'L', 1);
	...
	++$currCredit;
	$pdf->Cell((5 / 100) * $width, $height, '', 0, 0, 'L', 1);
	$pdf->Cell((25 / 100) * $width, $height, $dataD['kredit'][$currCredit]['noakun'] . ' ' . $dataD['kredit'][$currCredit]['namaakun'], 0, 0, 'L', 1);
...
if ($onDebet) {
	if (isset($dataD['debet'][4])) {
		...
		while ($i < $tmpDebet) {
			$pdf->Cell((30 / 100) * $width, $height, $dataD['debet'][$i]['noakun'] . ' ' . $dataD['debet'][$i]['namaakun'], 0, 0, 'L', 1);
...
while ($i < $tmpKredit) {
	$pdf->Cell((5 / 100) * $width, $height, '', 0, 0, 'L', 1);
	$pdf->Cell((25 / 100) * $width, $height, $dataD['kredit'][$i]['noakun'] . ' ' . $dataD['kredit'][$i]['namaakun'], 0, 0, 'L', 1);
