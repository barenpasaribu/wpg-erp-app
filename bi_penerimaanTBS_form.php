...
switch ($param['jenis']) {
    case 'global':
        $str = 'select sum(beratbersih) as totalnetto from '.$dbname.".pabrik_timbangan where millcode='".$pks."'\r\n                  and kodebarang='40000003' and tanggal like '".$tahun."%'";
        $res = mysql_query($str);
        while ($bar = mysql_fetch_object($res)) {
            $total = $bar->totalnetto;
        }
        $str = 'select sum(beratbersih) as tbsluar from '.$dbname.".pabrik_timbangan where millcode='".$pks."'\r\n                  and kodebarang='40000003' and (kodeorg is null or kodeorg='') and tanggal like '".$tahun."%'";
        $res = mysql_query($str);
        $tbsluar = 0;
        while ($bar = mysql_fetch_object($res)) {
            $tbsluar = $bar->tbsluar;
