...
//	echo "warning".$where;
	...
	$header = array('No Transaksi', $_SESSION['lang']['noinvoice'] . ' Supplier', $_SESSION['lang']['pt'], $_SESSION['lang']['tanggal'], 'Last Update', $_SESSION['lang']['nopo'], $_SESSION['lang']['keterangan'], $_SESSION['lang']['subtotal'], 'postingby', 'Posting Date', 'No VP');
...
case 'getPo':
	...
	switch ($jenisInvoice) {
		case 'po':
			...
			$sPo = 'select distinct nopo,(subtotal+misc+ongkosangkutan-nilaidiskon) as nilaipo, (ppn+ongkirimppn+miscppn) as ppn,kodesupplier,closed,pph from ' . $dbname . '.log_poht where kodeorg=\'' . $_SESSION['org']['kodeorganisasi'] . '\' ' . $where . ' ' . $addlokal . '  order by tanggal desc';
...
function formHeader($mode, $data)
{
	...
	if (empty($data)) {
		...
		$data['perhitunganpph'] = number_format($data['perhitunganpph'], 2);
...
//					   array('style' => 'width:150px'), $optPpn));
	...
	$els[] = array(makeElement('nilaippn', 'label','Nilai PPn'), makeElement('nilaippn', 'textnum', $data['nilaippn'], array('style' => 'width:150px', 'onchange' => 'this.value=remove_comma(this);this.value = _formatted(this)')));
	$els[] = array(makeElement('pph', 'label', 'PPh'), makeElement('pph', 'select', $data['pph'],
		array('style' => 'width:150px'), $optPph));
	$els[] = array(makeElement('perhitunganpph', 'label', 'Nilai PPh'), makeElement('perhitunganpph', 'textnum', $data['perhitunganpph'], array('style' => 'width:150px', 'onchange' => 'this.value=remove_comma(this);this.value = _formatted(this)')));
