...
if( $_POST['btnSubmitData']!=""){
	...
	if( $total_data > 0){
						...
						if(!empty($lembur_hari_libur)){	
							...
							if(!empty($resultquerytgl)){
								if($resultquerytgl['ishariraya']){
									...
									// Bukan Hari Raya
									$sDetIns = "INSERT INTO ".$dbname.".sdm_lemburdt (`kodeorg`,`tanggal`,`karyawanid`,`tipelembur`,`jamaktual`,`uangmakan`,`uangtransport`,`uangkelebihanjam`) values ('".$kdOrg."','".$tanggal."','".$karyawanid."','2','".$lembur_hari_libur."','0','0','".$ungLbhjm."') ON DUPLICATE KEY UPDATE tipelembur='2', jamaktual='".$lembur_hari_libur."', uangkelebihanjam='".$ungLbhjm."'";
								}
							}
						}
						mysql_query($sDetIns);			
		...
		// mengosongkan tmp
		$emptyQueryTable1 = "TRUNCATE TABLE tmp_absensiht";
		$emptyQueryTable2 = "TRUNCATE TABLE tmp_absensidt";
