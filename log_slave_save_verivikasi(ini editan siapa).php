...
case 'refresh_data':
	...
	//$sql = 'select count(*) as jmlhrow FROM ' . $dbname . '.log_prapodt b LEFT JOIN ' . $dbname . '.log_prapoht a ON a.nopp = b.nopp ' . "\r\n" . '               WHERE a.close >= \'3\' and b.status=\'0\' and b.create_po=\'0\' and substr(tanggal,1,4)=\'' . $thnSkrng . '\' and SUBSTR(b.nopp, 16, 3) like \''.$_SESSION['empl']['kodeorganisasi'].'%\' '.$where.' ORDER BY purchaser asc,a.tglp5,a.tglp4,a.tglp3,a.tglp2,a.tglp1 desc';
	$sql = 'select count(*) as jmlhrow FROM ' . $dbname . '.log_prapodt b LEFT JOIN ' . $dbname . '.log_prapoht a ON a.nopp = b.nopp LEFT JOIN (select nopp,kodebarang,sum(jumlahpesan) as jumlahpesan from log_podt group by nopp,kodebarang) c on (b.nopp=c.nopp and b.kodebarang=c.kodebarang) ' . "\r\n" . '               WHERE a.close >= \'3\' and b.status=\'0\' and (b.create_po=\'0\' or (b.create_po=\'1\' and b.realisasi > c.jumlahpesan) or (b.create_po=\'1\' and c.jumlahpesan is null)) and substr(tanggal,1,4)=\'' . $thnSkrng . '\' and SUBSTR(b.nopp, 16, 3) like \''.$_SESSION['empl']['kodeorganisasi'].'%\' '.$where.' ORDER BY purchaser asc,a.tglp5,a.tglp4,a.tglp3,a.tglp2,a.tglp1 desc';
...
/*
	...
	*/
	...
	WHERE a.close >= '3' and b.status IN ('0','3') and b.create_po='0' and substr(tanggal,1,4)='" . $thnSkrng . "' and SUBSTR(b.nopp, 16, 3) like '".$_SESSION['empl']['kodeorganisasi']."%'  ".$where;
	$str .= " ORDER BY a.tanggal desc,nopp desc limit $offset,$limit";
	*/
	...
	LEFT JOIN  $dbname.log_prapoht a ON a.nopp = b.nopp
	left join $dbname.log_prapodt_vw as c on (b.nopp=c.nopp and b.kodebarang=c.kodebarang)
	left join (select nopp,kodebarang,sum(jumlahpesan) as jumlahpesan from $dbname.log_podt group by nopp,kodebarang) as d on (b.nopp=d.nopp and b.kodebarang=d.kodebarang)
	WHERE a.close >= '3' and b.status IN ('0','3') and (b.create_po='0' or (b.create_po='1' and b.realisasi > d.jumlahpesan) or (b.create_po='1' and d.jumlahpesan is null)) and substr(tanggal,1,4)='" . $thnSkrng . "' and SUBSTR(b.nopp, 16, 3) like '".$_SESSION['empl']['kodeorganisasi']."%'  ".$where;
	$str .= " ORDER BY a.tanggal desc,nopp desc limit $offset,$limit";
