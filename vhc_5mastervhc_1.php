...
echo open_body();
echo "\r\n<script language=javascript1.2 src='js/vhc.js?v=".mt_rand()."'></script>\r\n";
...
$str = 'select kodeorganisasi,namaorganisasi from '.$dbname.".organisasi where tipe='TRAKSI' and kodeorganisasi like '".$_SESSION['empl']['lokasitugas']."%' order by namaorganisasi desc";
$res = mysql_query($str);
$opttraksi = '';
while ($bar = mysql_fetch_object($res)) {
    $opttraksi .= "<option value='".$bar->kodeorganisasi."'>".$bar->namaorganisasi.'</option>';
}
$str = 'select kodeorganisasi,namaorganisasi from '.$dbname.".organisasi where tipe  in('KANWIL','HOLDING','KEBUN','PABRIK','TRAKSI')and (length(kodeorganisasi)=4||length(kodeorganisasi)=6) order  by namaorganisasi";
$res = mysql_query($str);
