...
if (isTransactionPeriod()) {
	...
	case 'getHeader':
		...
		$sdata = 'select * from ' . $dbname . '.log_mrisht where untukunit like \''.$_SESSION['empl']['lokasitugas'].'%\' and notransaksi!=\'\' ' . $whr . '  AND post=\'1\'  ';
		...
		$s = 'select * from ' . $dbname . '.log_transaksiht where nomris = \''.$rdata['notransaksi'].'\' ';
		$sht = 'select distinct untukunit as kebun,tanggal,kodegudang from ' . $dbname . '.log_mrisht where notransaksi=\'' . $rdata['notransaksi'] . '\'';
		...
		$sPrdStr = 'select distinct * from ' . $dbname . '.setup_periodeakuntansi ' . "\r\n" . '                      where tutupbuku=0 and kodeorg=\'' . $rht['kodegudang'] . '\'';
		...
		$periode = substr($rPrdStr['tanggalmulai'],0,6);
		$tglht =substr($rdata['tanggal'],0,6) ;
		if ($periode == $tglht){

		
		($q = mysql_query($s)) || true;
			while ($r = mysql_fetch_assoc($q)) {
				$postr = $r['post'];
			}
			if ($postr =='0' or mysql_num_rows($q) == 0){
	...
	case 'getDetail':
		...
		while ($rDet = mysql_fetch_assoc($qDet)) {
			$SumPosting = 'select sum(a.jumlah) as jumlahpost from ' . $dbname . '.log_transaksidt as a, ' . $dbname . '.log_transaksiht as b where a.notransaksi=b.notransaksi and a.kodebarang=\'' . $rDet['kodebarang'] . '\' and a.nomris=\'' . $param['notransaksi'] . '\' and b.post=1';
			($spst = mysql_query($SumPosting)) || true;
			while ($jmlpost = mysql_fetch_assoc($spst)) {
				$totalpost = $jmlpost['jumlahpost'];
			}
			if (empty($totalpost)){
				$totalpost=0;
			}
			...
			$tab .= '<td  align=right><input type=text  id=jmlhPengeluara_' . $no . ' onkeypress=\'return angka_doang(event)\'  style=width:100px class=myinputtextnumber />' . "\r\n" . '                   <input type=hidden  id=kegId_' . $no . ' value=\'' . $rDet['kodekegiatan'] . '\' />' . "\r\n" . '  <input type=hidden  id=jmlhPengeluaranPosting_' . $no . ' value=\'' . $totalpost . '\' />' . "\r\n" . '                   </td>';
		...
		echo $tab . '####' . $rht['kebun'] . '####' . tanggalnormal($rht['tanggal']) . '####' . $param['notransaksi'] . '####' . $rht['kodegudang'] . '####' . tanggalnormal($rPrdStr['tanggalmulai']) . '####' . tanggalnormal($rPrdStr['tanggalsampai']) . '####' . tanggalsystem(tanggalnormal($rPrdStr['tanggalmulai'])) . '####' . tanggalsystem(tanggalnormal($rPrdStr['tanggalsampai'])). '####' . $tanggalsekarang;
	...
	case 'saveData':
		...
		if($qty =='0' || $qty==''){
			exit('error:Jumlah tidak boleh Nol atau Kosong !');
		}

		if (mysql_num_rows($res) == 1) {

			$wher = 'kodebarang=\'' . $kodebarang . '\' and notransaksi=\'' . $nodok . '\' and kodeblok=\'' . $blok . '\'';
			...
			if (mysql_query($str)) {
				...
				if (mysql_query($str)) {
					...
					if (!mysql_query($supdate)) {
						...
						echo ' naon, (update status on log_mrisdt)' . addslashes(mysql_error($conn));
		...
		if ($status == 2) {
			...
			if (mysql_affected_rows($conn) < 1) {
				...
				echo ' Gagal, (update detail on status 2) / ' . $str;
				...
				if (!mysql_query($supdate)) {
					...
					echo ' Gagal, (update detail on status 2) / ' . $supdate;
	...
	case 'updateData':
		...
		$sqlnotrans = 'select  * from ' . $dbname . '.log_transaksidt where kodebarang=\'' . $kodebarang . '\' and nomris=\'' . $notransaksi . '\' and jumlah!=0 order by waktutransaksi desc limit 1';
		($sql = mysql_query($sqlnotrans)) || true;
		while ($r = mysql_fetch_assoc($sql)) {
			$notrans = $r['notransaksi'];
			$jmlhold = $r['jumlah'];
		}
		if($qty < $jmlhold){
		...
		if(mysql_query($str)){
			$sqljmlh = 'select  sum(jumlah) as jumlah from ' . $dbname . '.log_transaksidt where kodebarang=\'' . $kodebarang . '\' and nomris=\'' . $notransaksi . '\'';
			($sjml = mysql_query($sqljmlh)) || true;
				while ($s = mysql_fetch_assoc($sjml)) {
			$jmlhrealisasi = $s['jumlah'];
				}
				 
				$hwr = 'kodebarang=\'' . $kodebarang . '\' and notransaksi=\'' . $notransaksi . '\' ' . 'and kodeblok=\'' . $blok . '\'';
		$supdate = 'update ' . $dbname . '.log_mrisdt set' . "\r\n" . '                                          jumlahrealisasi=\'' . $jmlhrealisasi . '\' where ' . $hwr . '';
		mysql_query($supdate);
		}
		}else{
			echo ' Error: Jumlah inputan tidak boleh melebihi jumlah transaksi terakhir !';
		}
	break;
	...
	$SumPosting = 'select sum(a.jumlah) as jumlahpost from ' . $dbname . '.log_transaksidt as a, ' . $dbname . '.log_transaksiht as b where a.notransaksi=b.notransaksi and a.kodebarang=\'' . $rDet['kodebarang'] . '\' and a.nomris=\'' . $param['notransaksi'] . '\' and b.post=1';
			...
			$tab .= '<td  align=right>' . "\r\n" . '                       <input type=text  id=jmlhPengeluara_' . $no . ' onkeypress=\'return angka_doang(event)\'  style=\'width:100px\' value=\'' . $rDet['jumlah'] . '\' class=myinputtextnumber />' . "\r\n" . '                       <input type=hidden  id=kegId_' . $no . ' value=\'' . $rDet['kodekegiatan'] . '\' />' . "\r\n" . '<input type=hidden  id=jmlhPengeluaranPosting_' . $no . ' value=\'' . $totalpost . '\' />' . "\r\n" . '                       </td>';
