...
$inpEdit1 = $_POST['inpEdit1'];
$inpEdit2 = $_POST['inpEdit2'];
...
switch ($method) {
    ...
    case 'getForm':
        ...
        while ($d = mysql_fetch_assoc($n)) {
            ++$no;
            $a .= "\r\n\t\t\t\t<tr id=row".$no.">\r\n\t\t\t\t\t<td><input type=hidden value='".$d['id']."'  id=id".$no." disabled onkeypress=\"return angka_doang(event);\"   class=myinputtextnumber style=\"width:50px;\"></td>\r\n\t\t\t\t\t<td>".$d['namaitem']."</td> \r\n\t\t\t\t\t<td>:</td>\r\n\t\t\t\t\t<td> Shift 1<input type=text id=1inp".$no." onkeypress=\"return angka_doang(event);\"  value=0 class=myinputtextnumber style=\"width:50px;\"> %</td><td> Shift 2<input type=text id=2inp".$no." onkeypress=\"return angka_doang(event);\"  value=0 class=myinputtextnumber style=\"width:50px;\"> %</td><td><input type=text id=inp".$no." onkeypress=\"return angka_doang(event);\" onclick=totS(".$no.")  value=0 class=myinputtextnumber style=\"width:50px;\"> %</td>\r\n\t\t\t\t</tr>";
        ...
        break;
    case 'savedata':
        $str = 'insert into '.$dbname.".pabrik_kelengkapanloses (`kodeorg`,`tanggal`,`id`,`nilai`,`updateby`,`nilai1`,`nilai2`,`shift1`,`shift2`)\r\n\t\tvalues ('".$kodeorg."','".$tgl."','".$id."','".$inp."','".$_SESSION['standard']['userid']."','".$inp1."','".$inp2."','".$shift1."','".$shift2."')";
        if (mysql_query($str)) {
        } else {
            $str = 'update '.$dbname.".pabrik_kelengkapanloses set nilai='".$inp."',`updateby`='".$_SESSION['standard']['userid']."' where kodeorg='".$kodeorg."' and tanggal='".$tgl."' and id='".$id."' and nilai1='".$inp1."' and nilai2='".$inp2."' and shift1='".$shift1."' and shift2='".$shift2."'";
            if (mysql_query($str)) {
            } else {
        ...
        break;
    case 'loadData':
        echo "\r\n\t<div id=container>\r\n\t\t\r\n\t\t<table class=sortable cellspacing=1 border=0>\r\n\t     <thead>\r\n\t\t\t <tr class=rowheader>\r\n\t\t\t \t <td align=center>".$_SESSION['lang']['nourut']."</td>\r\n\t\t\t \t <td align=center>".$_SESSION['lang']['kodeorg']."</td>\r\n\t\t\t\t <td align=center>".$_SESSION['lang']['tanggal']."</td>\r\n\t\t\t\t \r\n\t\t\t\t <td align=center>".$_SESSION['lang']['produk']."</td>\r\n\t\t\t\t <td align=center>".$_SESSION['lang']['namabarang']."</td>\r\n\t\t\t\t \r\n\t\t\t\t <td align=center>".$_SESSION['lang']['satuan']."</td>\r\n\t\t\t\t <td align=center>".$_SESSION['lang']['standard']."</td>\r\n\t\t\t\t \r\n\t\t\t\t <td align=center>Realisasi 1</td><td align=center>Realisasi 2</td><td align=center>".$_SESSION['lang']['realisasi']."</td><td align=center>Shift 1</td><td align=center>Shift 2</td>\r\n\t\t\t\t <td align=center>".$_SESSION['lang']['updateby']."</td>\r\n\t\t\t\t \r\n\t\t\t\t <td align=center>".$_SESSION['lang']['action']."</td>\r\n\t\t\t </tr>\r\n\t\t</thead>\r\n\t\t<tbody>";
        if ('' !== $tglsch) {
        ...
        while ($d = mysql_fetch_assoc($n)) {
            ...
            echo '<td align=right>'.$d['nilai1'].'</td>';
            echo '<td align=right>'.$d['nilai2'].'</td>';
            echo '<td align=right>'.$d['nilai'].'</td>';
            echo '<td align=right>'.$optNmKar[$d['shift1']].'</td>';
            echo '<td align=right>'.$optNmKar[$d['shift2']].'</td>';
            ...
            if($d['posting'] == 1){
                ...
                echo "  <img src=images/application/application_edit.png class=resicon  caption='Edit' 
                    ...
                    '".$arrProduk[$d['id']]."','".$arrItem[$d['id']]."','".$d['nilai']."','".$d['id']."','".$d['nilai1']."','".$d['nilai2']."','".$d['shift1']."','".$d['shift2']."');\">
                    <img src=images/application/application_delete.png class=resicon 
                    caption='Delete' onclick=\"del('".$d['kodeorg']."','".tanggalnormal($d['tanggal'])."','".$d['id']."');\">";
                echo "  <img src=images/skyblue/posting.png class=resicon caption='Posting' 
                    onclick=\"posting('".$d['kodeorg']."','".tanggalnormal($d['tanggal'])."','".$d['id']."');\">";
            }
    ...
    case 'delete':
        ...
        break;
    case 'posting':
        $i = 'update '.$dbname.".pabrik_kelengkapanloses set posting=1 where kodeorg='".$kodeorg."' and tanggal='".$tgl."' and id='".$id."'";
        if (mysql_query($i)) {
            echo '';
        } else {
            echo ' Gagal,'.addslashes(mysql_error($conn));
        ...
        break;
    case 'update':
        $i = 'update '.$dbname.".pabrik_kelengkapanloses set nilai1='".$inpEdit1."',nilai2='".$inpEdit2."', nilai='".$inpEdit."',`updateby`='".$_SESSION['standard']['userid']."' where kodeorg='".$kodeorgEdit."' and tanggal='".$tglEdit."' and id='".$idEdit."'";
        if (mysql_query($i)) {
            echo $i;
        } else {
            echo ' Gagal,'.addslashes(mysql_error($conn));
