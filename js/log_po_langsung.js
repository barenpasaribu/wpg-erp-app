...
function pass2detail() {
    ...
    post_response_text('log_get_po_langsung_detail.php', param, respon);
}

function get_isi(kdorg,nm_org)
{
        param='kdorg='+kdorg;
        tujuan='log_get_po_langsung.php';
...
function cek_data()
{
		...
		try{
			...
			strUrl += '&kdbrg[]='+encodeURIComponent(trim(document.getElementById('kd_brg_'+i).value))
			  ...
			  +'&rjmlhDiminta[]='+encodeURIComponent(trim(document.getElementById('jumlah_'+i).value))
			  +'&hargasatuan[]='+encodeURIComponent(trim(document.getElementById('hargasatuan_'+i).value));
			}
			else
			{
			strUrl += '&kdbrg[]='+encodeURIComponent(trim(document.getElementById('kd_brg_'+i).value))
        ...
        param+=strUrl;
        tujuan='log_slave_get_user_id_log_pl.php';
        //alert(param);
        post_response_text(tujuan, param, respog);
        function respog()
        {
...
function addDetail(id) {
	...
	var rjmlhDiminta = document.getElementById('jumlah_'+id);
	var hargasatuan = document.getElementById('hargasatuan_'+id);
...
function addNewRow(body,onDetail) {
    ...
    if(!detail) {
        ...
        {
			jdl_ats_1 = 'Form Pencarian';
                // Create Row
        newRow.innerHTML += "<td onclick=\"searchBrg('"+jdl_ats_1+"','"+content_0+"<input id=nomor type=hidden value="+numRow+" />',event)\";><input id='kd_brg_"+numRow+"' type='text' class='myinputtext' style='width:120px' readonly='readonly' value='' /></td><td>"+"<input id='nm_brg_"+numRow+"' type='text' class='myinputtext' style='width:120px' readonly='readonly' value='' /></td><td onclick=\"searchBrg('"+jdl_ats_1+"','"+content_0+"<input id=nomor type=hidden value="+numRow+" />',event)\";><input id='sat_"+numRow+"' type='text' class='myinputtext' style='width:70px' readonly='readonly' value='' /><img src=images/search.png class=dellicon title='"+jdl_ats_0+"' onclick=\"searchBrg('"+jdl_ats_1+"','"+content_0+"<input id=nomor type=hidden value="+numRow+" />',event)\";></td>"+
		"<td><input id='jumlah_"+numRow+"' type='text' class='myinputtextnumber' style='width:70px' value='' onkeypress='return angka_doang(event)' /></td>"+"<td><input id='hargasatuan_"+numRow+"' type='text' class='myinputtextnumber' style='width:70px' value='' onkeypress='return angka_doang(event)' /></td>"+"<td><input id='ket_"+numRow+"' type='text' class='myinputtext' style='width:130px' onkeypress='return tanpa_kutip(event)' value='' /></td>"+"<td><img id='detail_add_"+numRow+"' title='Tambah' class=zImgBtn onclick=\"addDetail('"+numRow+"')\" src='images/save.png'/>"+
...
//perubahan indz
 function frm_ajun(nopp,kodeorg){//alert(nopp);
	// alert(stat);
	...
	a=confirm("Posting,  Are you sure?");
	if(a) {
		param='method=formPersetujuan'+'&nopp='+nopp+'&kodeorg='+kodeorg;
	//alert(param);
	tujuan='log_save_log_pl.php';//alert(param);
	post_response_text(tujuan, param, respog);	
	function respog()
	{
		  if(con.readyState==4)
