 ...
 * antlou2016
 * jakarta indonesia
 * anthesis-team@mitraagroservindo.com
 */
if (!window.qmad){
	qmad=new Object();
	qmad.binit='';
	...
	qmad.bhide='';
		...
		a.rcorner_size = 6;
		a.rcorner_border_color = "#A1A1A1";
		a.rcorner_bg_color = "#556175";
...
function tolltipJump(ev){
       ...
       if (document.getElementById('dynamic1')) {
	    ...
	    cont+="<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:"+height+"px;overflow:scroll'>";
	    cont+='<table class=sortable style="width:'+width+'px" cellspacing=1 border=0>';
		...
		for(x=0;x<step.length;x++){
			cont+='<tr class=rowcontent style="cursor:pointer;" onclick=jump('+step.options[x].getAttribute("id")+',event)><td>'+step.options[x].getAttribute("id")+'</td><td>'+step.options[x].text+"</td></tr>";
		}
		cont+='</tbody></table>';
	    cont+="</div>";
