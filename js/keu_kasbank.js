    ...
    for (var i = 0; i < datas.length; i++) {
        ...
        if (posted) {
            ...
            if (approved) {
                ...
                rows = rows + "<td><img src='images/skyblue/edit.png' class='zImgBtn' onclick=\"cekEdit('" + datas[i]['notransaksi'] + "','edit','" + datas[i]['postingprivilage'] + "');\" title='Edit'></td>";
...
function cekEdit(notrans, action, privilage) {
    ...
    if (privilage=='allow') {
        ...
        alert('Hanya user dengan departemen HO Accounting & Tax yang bisa edit data setelah di approve ');
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    if (result['action']=='approval') {
                        ...
                        if (result['userlogin']==result['disetujuioleh']) {
                            var answer = window.confirm("Apakah data akan di Approval?");
                            if (answer) {
...
function searchBatch(title, content, ev, tipetransaksi) {
    ...
    getFormBatch(tipetransaksi);

}

function getFormBatch(tipetransaksi) {

    param = '';

    tujuan = 'keu_slave_kasbank_detail.php';

    post_response_text(tujuan + '?' + 'proses=getFormbatch&tipetransaksi=' + tipetransaksi, param, respog);
        ...
        if (con.readyState == 4) {
            ...
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    document.getElementById('formPencarianbatch').innerHTML = con.responseText;
...
function searchPerdin(title, content, ev, tipetransaksi) {
    ...
    getFormPerdin(tipetransaksi);

}


function getFormPerdin(tipetransaksi) {

    param = '';

    tujuan = 'keu_slave_kasbank_detail.php';

    post_response_text(tujuan + '?' + 'proses=getFormperdin&tipetransaksi=' + tipetransaksi, param, respog);
        ...
        if (con.readyState == 4) {
            ...
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    document.getElementById('formPencarianperdin').innerHTML = con.responseText;
...
function findNoBatch(tipetransaksi) {
    ...
    } else post_response_text(tujuan + '?' + 'proses=getBatch', param, respog);
...
function findNoPerdin(tipetransaksi) {

    txt = trim(document.getElementById('no_transaksi').value);
    nik = document.getElementById('karyawanid').value;
    tipe = document.getElementById('tipe').value;

    param = 'txtfind=' + txt + '&tipe=' + tipe;
    ...
    } else post_response_text(tujuan + '?' + 'proses=getPerdin', param, respog);
...
function setPo(np, nilai, akn, ket, supp, nopo) {
	...
	*/
    ...
    l3 = document.getElementById('kodecustomer');

    for (a3 = 0; a3 < l3.length; a3++) {

        if (l3.options[a3].value == supp) {

            l3.options[a3].selected = true;
...
function setPo1(batch, np, nilai, akn, ket, supp, nopo) {

    document.getElementById('novp').value = batch;
    document.getElementById('keterangan1').value = np;
...
function setPo2(notransaksi, karyawanid, nilai) {

    document.getElementById('pdinas').value = notransaksi;
    ...
    document.getElementById('keterangan2').value = "Perjalan Dinas No " + notransaksi;
    ...
    for (a = 0; a < l.length; a++) {
        ...
        if (l.options[a].value == karyawanid) {
