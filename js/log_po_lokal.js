...
document.getElementById('form_po_lokal').style.display='block';
}
...
//if((parseFloat(jmlh_brg))>=(parseFloat(defult_tot)+1))
        ...
        else
        {
                ...
                else
                {
                        ...
                        if(jmlh_sub==0)
                        {
                                document.getElementById('total_'+id).value=0.00;
                        }
                        else
                        {
        ...
        tot.value=total;
        change_number(tot);
        if (document.getElementById('angDiskon').disabled == false) {
                calculate_angDiskon();
        }
        if (document.getElementById('diskon').disabled == false) {
                calculate_diskon();
        }
...
function plusAll(id)
{
        ...
        nPPh=document.getElementById('ppH').value;
        if(nPPh!="")
        {
                nilPph=(parseFloat((total-disc))*nPPh)/100;	
        ...
        grnd_tot=parseFloat((total-disc))+parseFloat(nilPpn)-parseFloat(nilPph);
...
function calculatePph(){
        ...
        { 
                if(nilP < 0 || nilP > 10){
                        ...
                        document.getElementById('hslPPh').value=0;
                        document.getElementById('pph').value=0;
                        document.getElementById('ppH').value=0;
                        return;
                }else{
                        dis.value=remove_comma(dis);
                        subTot.value=remove_comma(subTot);
                        ph=((parseFloat(subTot.value-dis.value))*nilP)/100;	
                        document.getElementById('hslPPh').innerHTML=ph;
                        document.getElementById('pph').value=ph;
                        document.getElementById('ppH').value=nilP;
                }
        }
        else
        {
...
function grandTotal()
{
        ...
        ongKirimPPn=document.getElementById('ongKirimPPn');
        var nilPph = 0;
        ...
        if(nilDiskon.value!=""||nilDiskon.value!=0)
        {
                ...
                nilPpn=document.getElementById('ppn').value;
                nilPph=document.getElementById('pph').value;
        }
        else
        {
                document.getElementById('diskon').value=0;
        ...
        nilaiOngkir = document.getElementById('ongKirim').value;
        nilaiOngkirPpn = document.getElementById('ongKirimPPn').value;
        nilaiMisc = document.getElementById('misc').value;

        if(nilaiOngkir != 0 || nilaiOngkir != '')
        {
                nilaiOngkir=document.getElementById('ongKirim');
		nilaiOngkir.value=remove_comma(nilaiOngkir);
                nilaiOngkir=nilaiOngkir.value;
        }
        else
        {
                document.getElementById('ongKirim').value = 0;
                nilaiOngkir = 0;
        }

        if(nilaiOngkirPpn != 0 || nilaiOngkirPpn != '')
        {
                nilaiOngkirPpn = document.getElementById('ongKirimPPn').value;  
        }
        else
        {
                document.getElementById('ongKirimPPn').value = 0;
                nilaiOngkirPpn = 0;
        }

        if(nilaiMisc != 0 || nilaiMisc != '')
        {
                nilaiMisc=document.getElementById('misc');
		nilaiMisc.value=remove_comma(nilaiMisc);
                nilaiMisc=nilaiMisc.value; 
        }
        else
        {
        ...
        grnd_tot = parseFloat((sb_tot.value-nilDiskon.value)) + parseFloat(nilPpn)
                - parseFloat(nilPph) + parseFloat(nilaiOngkir)
                + ((parseFloat(nilaiOngkir) * parseFloat(nilaiOngkirPpn)) / 100)
                + parseFloat(nilaiMisc);
        total=document.getElementById('grand_total');
...
function displayList()
{
		...
		document.getElementById('list_pp').style.display='none';
        //document.getElementById('form_po').style.display='none';
        document.getElementById('form_po_lokal').style.display='none';
        load_new_data();
        clear_all_data();
...
{
	if(confirm("Anda ingin menyimpan data ini ?"))
	{ 
		...
		for(i=0;i<row;i++)
		{
						...
						try{
										...
										{					
												...
												+'&kdbrg[]='+encodeURIComponent(trim(document.getElementById('kd_brg_'+i).value))
		...
		param+='&diskon='+disc+'&ppn='+nil_ppn+'&pph='+nil_pph+'&tgl_krm='+tgl_deliver+'&lok_kirim='+delivery_loc+'&cara_pembayarn='+cara_pem+'&nildiskon='+nil_diskon;
...
function deleteDetail(id) {
        ...
        document.getElementById('spek_brg_'+id).value = '';
        document.getElementById('sat_'+id).value = '';
        document.getElementById('jmlhDiminta_'+id).value = 0;
        document.getElementById('harga_satuan_'+id).value = '';
        document.getElementById('total_'+id).value = 0.00;
...
function calculateMiscPpn(idkebrp){
    ...
    nilP=document.getElementById('ppN').value;
    nilPh=document.getElementById('ppH').value;
    ...
    switch(idkebrp){
        case 0:
            ...
            //misSub=parseFloat(subtotal)+parseFloat(miscPPn.value); FA 20190306 - miscPPn sudah ada PPn nya
            misSub=parseFloat(subtotal);
            totPlppn=parseFloat(misSub)*(parseFloat(nilP)/100);
            totPlpph=parseFloat(misSub)*(parseFloat(nilPh)/100);
            if(isNaN(onkKrmdnppn)){
                grandtotal=parseFloat(misSub)+parseFloat(totPlppn)-parseFloat(totPlpph)+parseFloat(msc.value);
            }else{
                grandtotal=parseFloat(misSub)+parseFloat(totPlppn)-parseFloat(totPlpph)+parseFloat(msc.value)+(parseFloat(onkKrmdnppn)+parseFloat(onkrm.value));
            }
            document.getElementById('hslPPn').innerHTML=totPlppn;
            document.getElementById('hslPPh').innerHTML=totPlpph;
            document.getElementById('ppn').value=totPlppn;
            document.getElementById('pph').value=totPlpph;
        ...
        case 2:
            //ongKirim
            miscPPn2=document.getElementById('ppn');
            miscPPn=document.getElementById('miscNppn');
            miscPPn.value=remove_comma(miscPPn);
            miscPPn2.value=remove_comma(miscPPn2);
            diskon=disc.value;
            onkrm=document.getElementById('ongKirim');
            onkrm.value=remove_comma(onkrm);
            msc=document.getElementById('misc');
            ...
            totPlppn=parseFloat(misSub)*(parseFloat(nilP)/100);
            totPlpph=parseFloat(misSub)*(parseFloat(nilPh)/100);
            grandtotal=parseFloat(misSub)+parseFloat(totPlppn)-parseFloat(totPlpph)+parseFloat(msc.value)+parseFloat(totByKrm);
            grandTota=document.getElementById('grand_total');
            grandTota.value=remove_comma(grandTota);//
            grandTota.value=grandtotal;
            change_number(sb_tot);
