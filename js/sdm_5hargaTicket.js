...
function loadData()
{
	param='method=loadData';
        ...
        region=document.getElementById('tujuanHead').options[document.getElementById('tujuanHead').selectedIndex].value;
        if(thnBudget!='')
        {
...
function fillField(thnbdget,kdgol,tjn)
{
	
	param='method=getData'+'&thnBudget='+thnbdget+'&kdGol='+kdgol+'&region='+tjn;
	tujuan='sdm_slave_5hargaTicket';
        post_response_text(tujuan+'.php', param, respon);
	function respon() {
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					ar=con.responseText.split("###");
                                    ...
                                    pnjng=document.getElementById('region');
					...
					//document.getElementById('kdGol').value=ar[1];
					//document.getElementById('region').value=ar[2];
					document.getElementById('tktPes').value=ar[3];
					document.getElementById('tksi').value=ar[4];
                                        document.getElementById('airport').value=ar[5];
                                        document.getElementById('visa').value=ar[6];
                                        document.getElementById('byaLain').value=ar[7];
...
function cancelIsi()
{
	...
	document.getElementById('tktPes').value='0';
	document.getElementById('tksi').value='0';
	document.getElementById('airport').value='0';
        document.getElementById('visa').value='0';
        document.getElementById('byaLain').value='0';
...
function delData(thnbdget,kdgol,tjn)
{
	param='method=delData';
        param+='&thnBudget='+thnbdget+'&kdGol='+kdgol+'&region='+tjn;
	tujuan='sdm_slave_5hargaTicket';
...
function cariPage(num)
{
		...
		param+='&page='+num;
                ...
                tujuan='sdm_slave_5hargaTicket.php';
