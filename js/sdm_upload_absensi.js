...
function ProcessDataCSV(){
	var method 		= 'ProcessDataCSV'; 
	var StartDate	= trim(document.getElementById('StartDate').value); 
	var EndDate		= trim(document.getElementById('EndDate').value); 
	
	var param	= 'method='+method+'&StartDate='+StartDate+'&EndDate='+EndDate;
	var tujuan 	= 'sdm_slave_save_upload_absensi.php';
...
// START PROCESS DATA XLS //
function ProcessDataXLS(){
	var method 		= 'ProcessDataXLS'; 
	var StartDate	= trim(document.getElementById('StartDate2').value); 
	var EndDate		= trim(document.getElementById('EndDate2').value); 
	...
	function respog(){
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				//document.getElementById("buttonsave2").disabled = true;
					alert('Data Berhasil di Proses');
					//if(confirm('Mau Cek Karyawan yang cuti tapi dia masuk ga ?')) {
					//	RestoreCuti(StartDate, EndDate);
					//} else {
					//	alert('Yaudah.');
					//}
				}
...
// END PROCESS DATA XLS //

function RestoreCuti(StartDate, EndDate){
	var method 	= 'RestoreCuti'; 
	var param	= 'method='+method+'&StartDate='+StartDate+'&EndDate='+EndDate;
	var tujuan 	= 'sdm_slave_save_upload_absensi.php';
	post_response_text(tujuan, param, respog);	
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById("FieldsetListData").innerHTML = con.responseText;					
				}
			}
			else {
				alert('ERROR TRANSACTION,\n' + con.responseText);
...
function ProcessRestoreCuti(TotalRecord){
	var method 	= 'ProcessRestoreCuti'; 
	var ListRecords = new Array(TotalRecord);
	
	for(var c = 1; c <= TotalRecord; c++) {		
		ListRecords[c-1] = {
			'kodeorg' : document.getElementById('kodeorg-'+c).value,
			'karyawanid' : document.getElementById('karyawanid-'+c).value,
			'tanggalcuti' : document.getElementById('tanggalcuti-'+c).value
		}
	}
	
	var param	= 'method='+method+'&ListRecords='+JSON.stringify(ListRecords);
	var tujuan 	= 'sdm_slave_save_upload_absensi.php';
	post_response_text(tujuan, param, respog);	
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					alert('Udah di balikin cutinya.');
					document.getElementById("BtnRestoreCuti").disabled = true;
				}
			}
			else {
				alert('ERROR TRANSACTION,\n' + con.responseText);
...
function sEntryShift(){
	...
	if(confirm('Saving data for '+kode+', are you sure ?')) {post_response_text(tujuan, param, respog);}		
	function respog() {
		if(con.readyState==4) {
			if (con.status == 200) {
				busy_off();
