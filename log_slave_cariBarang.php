...
if (isTransactionPeriod()) {
	...
	if (mysql_num_rows($res) < 1) {
		...
		$no = 0;
		    	...
		    	$strses6 = 'select periode from '.$dbname.'.setup_periodeakuntansi a left join '.$dbname.".organisasi b 
					on a.kodeorg=b.kodeorganisasi where b.tipe like 'GUDANG%' and tutupbuku=0 and a.kodeorg like '".$gudang."%' ";
    			$resses6 = mysql_query($strses6);
			$hasil=mysql_fetch_assoc($resses6);
			$periodegudang=$hasil['periode'];

		while ($bar = mysql_fetch_object($res)) {
			$no += 1;
			$saldoqty = 0;
			$str1 = 'select saldoakhirqty from ' . $dbname . '.log_5saldobulanan where kodebarang=\'' . $bar->kodebarang . '\'' . "\r\n\t\t\t\t" . '       
				and periode=\''.$periodegudang.'\'	and kodegudang=\'' . $gudang . '\'';
			$res1 = mysql_query($str1);

			while ($bar1 = mysql_fetch_object($res1)) {
				$saldoqty = $bar1->saldoakhirqty;
			}
